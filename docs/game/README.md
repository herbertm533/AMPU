...
`/ingest-playtest` skill, which runs four roles in order.

## The four living docs
...
Lanes are intentionally crisp — context = WHAT (+gaps), mechanics = RULES,
technical = HOW, roadmap = ORDER — so the docs don't drift into duplicating each
other.
...
- **Living docs** — the four files above: synthesis *across* all digests.
...
3. The skill preprocesses the CSV → cleaned chunks, then runs:
   Game PM (digest + context) → Game Master (mechanics) → Tech Lead (technical
   guide + sequencing) → Project Manager (roadmap).
