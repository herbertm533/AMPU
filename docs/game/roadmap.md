...
founding-variant work.** A coverage/sourcing note, NOT a build item. **Total author-before-build RULES items: 13** (UNCHANGED — ★ batch-31 nets 0: 3 RECONCILED, no new hole)
...
Why the order is what it is — the tech-lead's binding calls (§9 batch-31 lead +
