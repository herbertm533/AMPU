...
multiplier. **Total author-before-build items now: 12** (DH-1, #10/#84, §25.9,
DH-12, DH-13, DH-14, DH-15, DH-25, DH-33, DH-41, **DH-49**, **DH-54**).
...
> - **DH-50 / DH-53 (NEW, batch 9)** — era-event-scheduling holes → **E15** (the
>   own-clock content schedule, #109); bill-catalog completeness → **E2**.
...
Why the order is what it is — the tech-lead's binding calls (§9 batch-9 lead +
§9.1.5 + §9.1.8 + §9.1.3 + §9.6 + §9.1.6 + §9.1.7 + §6.6.1). **Batch 9 is
CONFIDENCE + NEGATIVE-SCOPE + small placements, NOT a re-sequence** — the batch-9
leads come first (below), then the batch-8 confidence leads, then the three batch-7
leads, then the carried batch-5 leads. **The order itself is UNCHANGED from batch 7
— the top of the queue is still QW0 → K0/K2 → K3/K4 …**
...
This reflects **batches 1–9** — **eleven** ingested digests across **11
playtest threads**: the `f4c7c2c4` 1868 Gilded-Age multiplayer dry-run, the
...
war traversal (`drums`) corroborating across the span. **This batch-9 pass is
CONFIDENCE + NEGATIVE-SCOPE + small placements — NO re-sequence, NO new keystone:**
