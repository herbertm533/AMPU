...
| QW3 | **±3-per-phase meter-swing clamp (meter-model divergence; now confirmed live by `drums` #80)** | One-helper change: clamp every `NationalMeters` write to ±3 per phase (`types.ts:1399`). **Batch 5 extends this:** apply the same clamp to **cabinet ideology net-swings** and **per-phase ideology swings** at the same chokepoint (#80, `drums` POST 4574 — a live designer patch). Orthogonal to the full named-ordinal relabel (parked) and to E6's crisis/cascade rules (which build on it). Cheap balance/feel win. **★ Batch-10 — the ±3 cap is the SETTLED HALF of the decision-gated #18/#51 fork** (`dem1820`: a GA-published meter→enthusiasm→election rule moves enthusiasm boxes with a hard ±3 cap). **This QW3 clamp on `NationalMeters` writes is buildable NOW** (the cap value is settled). **What is NOT settled — and is BLOCKED on the #18/#51 human pick (parking lot) — is where the enthusiasm→election step BINDS and its STATE SCOPE** (every-state-unless-penalized vs ideology-leaning-states-only vs primaries-only): the shipped `calcStateVote` (`phaseRunners.ts:3685`, score `:3709-3711`) applies enthusiasm UNIFORMLY with **NO ±3 cap and NO per-state penalty** today. So: land the meter-write clamp here; the enthusiasm-application binding-point + per-state-scope clamp wait on the fork (recorded under "Decision-gated", folds into E20/E23). | — (the enthusiasm-application half waits on the #18/#51 pick) | XS | meter model §21.8/§22.1/§22.2 (`1772s`; `modern`; **`drums` #80 POST 4574**) + **#18/#51 settled ±3 cap (`dem1820`)** — CARRIED + EXTENDED, HI-CONF | ready (meter-write half) |
...
existing DH-25 career-track-eligibility call, listed there). **Total author-before-
build items: still 12** (DH-1, #10/#84, §25.9, DH-12, DH-13, DH-14, DH-15, DH-25,
DH-33, DH-41, **DH-49**, **DH-54**). **NEW this batch: a separate "Decision-gated"
category (2 items) — #52 player-controlled SCOTUS + #18/#51 meter→enthusiasm→election
— each a HUMAN design pick among 3 live models** (distinct from author-before-build:
the rules exist, three of them; a human must choose). See the **"Decision-gated"**
subsection below.
...
**★ Batch 10 — TWO forks each have 3 LIVE competing models; the build cannot pick
one on its own. These are NOT ready-to-build and are NOT author-before-build holes
(rules exist — three of them); a HUMAN must choose the canonical model first, then the
item folds into the engine row noted.** (Distinct from "Roadmap decisions" below,
which are tech-lead/planner calls already made.)
...
- **★ #18/#51 — Meter→enthusiasm→election: EVERY-STATE-UNLESS-PENALIZED vs
  IDEOLOGY-LEANING-STATES-ONLY vs PRIMARIES-ONLY (DECISION-GATED).** A `dem1820` GA
  published a concrete meter→enthusiasm→election processing rule but exposed a
  **GA-vs-designer model FORK** — three live models: Ted's **"non-enthusiasm meters
  move the enthusiasm boxes, which apply to EVERY state unless penalized"**, V's
  **"ideology-leaning states only"**, Matt's **"primaries only."** **A human must pick
  the canonical model for the election engine.** **SETTLED part (buildable now,
  model-agnostic):** meters move enthusiasm boxes + a **hard ±3 cap** — that cap is the
  **queued XS clamp QW3** (the meter-write half ships now). **UNSETTLED part (blocked on
  the pick):** the **BINDING POINT + STATE SCOPE** of the enthusiasm→election step.
  Verified shipped: `calcStateVote` (`phaseRunners.ts:3685`, score `:3709-3711`) applies
  enthusiasm **UNIFORMLY with NO ±3 cap and NO per-state penalty** — so both the cap's
  binding point and the per-state scope wait on this fork. **Folds into E20/E23
  (election math / enthusiasm engine) once chosen.** — #18/#51 (`dem1820` 3-model fork;
  shipped `phaseRunners.ts:3685,:3709-3711`; cap → **QW3**; binding → **E20/E23**).
...
Why the order is what it is — the tech-lead's binding calls (§9 batch-10 lead +
§9.1.9 + §9 batch-9 lead + §9.1.5 + §9.1.8 + §9.1.3 + §9.6 + §9.1.6 + §9.1.7 +
§6.6.1). **Batch 10 is PROMOTION + decision-gating + small placements, NOT a
re-sequence** — the batch-10 leads come first (below), then the batch-9 leads, then
the batch-8 confidence leads, then the three batch-7 leads, then the carried batch-5
leads. **The order itself is UNCHANGED — the top of the queue is QW0 → K0/K2 →
K3/K4 + the `scenarioBoot`/`BootSheet` pipeline → `scenario1788` (E1) …**
