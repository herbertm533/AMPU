...
Ted/vcczar), NOT the author-before-build list. **Total author-before-build items: 13**
(DH-1, #10/#84, §25.9, DH-12, DH-13, DH-14, DH-15, DH-25, DH-33, DH-41, **DH-49**,
**DH-54**, **#122 pardon mechanics batch-12**). **Decision-gated category — held at 2 by batch 11** (batch 10 introduced it
...
Why the order is what it is — the tech-lead's binding calls (§9 batch-13 lead +
...
§9.1.5 + §9.1.8 + §9.1.3 + §9.6 + §9.1.6 + §9.1.7). **Batch 13 (`oopscpu`) is E9
DE-RISK — it field-validates the CPU-handler specs before they're built and folds the
OC-* sub-gaps into the existing handler rows; NO re-sequence, NO new keystone, NO
keystone moves.** The batch-13 leads come first (below), then batch-12, then
batch-11, then batch-10, then batch-9, then batch-8, then batch-7, then the carried
batch-5 leads. **The order itself is UNCHANGED — the TOP OF THE QUEUE is QW0
(constant=4 + alt-state guard, ship FIRST) → K0/K2 → K3/K4 +
`scenarioBoot`/`BootSheet` → `scenario1788` (E1) … — but E1 now carries the
