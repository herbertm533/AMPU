...
the B8/B9 pair = one logical gate).**
...
Why the order is what it is — the tech-lead's binding calls (§9 batch-15 lead +
...
re-sequence, NO new keystone, NO new author-before-build items.** The batch-15 lead comes
first (below), then batch-14, then batch-13, then batch-12, then batch-11, then batch-10,
then batch-9, then batch-8, then batch-7, then the carried batch-5 leads. **The order
itself is UNCHANGED — the TOP OF THE QUEUE is QW0 (constant=4 + alt-state guard, ship
FIRST) → K0/K2 → K3/K4 + `scenarioBoot`/`BootSheet` → `scenario1788` (E1) … — E1 still
carries the DH-61 (boot-seed active wars, in K4) + DH-62 (pre-12A EC mode, in E4)
prerequisites, and the XS quick-win cluster now runs through QW18 (#143 = QW17, #153 = QW18).**
