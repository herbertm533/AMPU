...
> `modern`/`hd`/`drums`/`pop`/**`rep1800`** are the eight digests; **NEW** = first
...
is complete; the knowledge-base infra and seven ingestion batches are knowledge
...
they fold into the engine row noted. **Batch 7 adds ONE NEW author-before-build
item:** DH-33 impeachment-ruleset rewrite (folds into E29) — plus a separate
...
multiplier. **Total author-before-build items now: 9** (DH-1, #10/#84, §25.9,
DH-12, DH-13, DH-14, DH-15, DH-25, **DH-33**).
...
Why the order is what it is — the tech-lead's binding calls (§9.6 + §9.1.5 +
§9.1.6 + §9.1.7 + §9.1.3 + §6.6.1). **The three batch-7 leads come first
(below), then the carried batch-5 leads.**
...
This reflects **batches 1–7**: the `f4c7c2c4` 1868 Gilded-Age multiplayer
...
the only source for the Civil-War / Reconstruction / secession machinery), the
`e1776bbd` **all-CPU "Drums of War"** (7540 posts — the first explicit forum
record of CPU heuristics), the `c50d9da7` **"Era of Populism" 2012 fresh-modern
boot** (1172 posts — the canonical `BootSheet` + meter-driven endgame), and now
`rep1800` — the **"Era of Republicanism" 1800→1868 early-republic campaign**, the
first procedural record of the 1800–1856 early republic and the **predecessor of
batch-1's `gilded`**. **The 1800 campaign is now documented end-to-end** (1800→1868
here, 1868→Gilded in batch 1). This batch-7 re-sequence:
...
rewrite**, draft 3/3-canonical, Civil-War end-year, cross-era win total, **and
the batch-7 era-model open Qs — are the early sub-bands content-band markers or
enum values [tech-lead's call: markers first]; the exact scope of the
`territoryOwned` content-gate; which Reconstruction readmission fix E3b ships
(CPU bias #1 vs era-boundary auto-resolution #3 vs both — tech-lead's rec:
1+3]**) are tracked in `game-context.md` → Open questions.
