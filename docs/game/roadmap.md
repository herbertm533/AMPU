...
| **QW18** | **★ batch-15 #153 — global ×2 Command-gain multiplier (the UNBUILT half of the rookie-Command rule, `terror2000#POST 91`)** *(NEW, batch 15 — RULED, made official)* | **XS — one global multiplier on the Command-gain roll path** (`abilities.ts` `gainCommand`-equivalent + the committee/career-track expertise-grant sites). Ted (`terror2000#POST 91-93`) made TWO house-rules official: **(a)** nobody is born with Command — **all rookies enter at 0 Command** (already ships via #136) **AND every in-game Command-gain % is DOUBLED** to compensate (so Presidents emerge from in-game action, not real-world reputation — the **×2 multiplier is the only UNBUILT piece**); **(b)** rolling an already-held expertise on a grant = **NO new expertise (do NOT re-roll)** — already an invariant in the grant path. Pin the ×2 on every Command-gain % (the success→+1-Command rolls in E11/E13/E18 + draft-grant), gated nowhere (all eras). Pairs with **QW17** (#143 Command decay) — same Command economy, opposite direction (decay pares the pool, the ×2 lets contenders climb). No dependency; ship with the QW0 / XS-consistency cluster. **RULED by Ted (`terror2000#POST 91-93`).** | — | XS | gap **#153 / `terror2000#POST 91-93`** — NEW | ready |
...
Why the order is what it is — the tech-lead's binding calls (§9 batch-16 lead +
...
author-before-build or Decision-gated items (batch 16 nets 0).** **Batch 14 (`gild1868`) is
...
re-sequence, NO new keystone, NO new author-before-build items.** The batch-16 lead comes
first (below), then batch-15, then batch-14, then batch-13, then batch-12, then batch-11,
then batch-10, then batch-9, then batch-8, then batch-7, then the carried batch-5 leads.
