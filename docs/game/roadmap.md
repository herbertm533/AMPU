...
> **★ North-star / GM-replacement lens (read this first).** The entire backlog
> below is, read correctly, **the catalogue of work a human Game Master does by
> hand every turn** — and AMPU's core mandate is a **zero-human-GM game**. The
> conceptual charter lives in `game-context.md` → **"★ North-star: the app must
> subsume the human GM"** (cross-referenced from `technical-guide.md` §9). For a
> single VIEW that groups the existing items by **which half of the GM's job each
> retires** — ① referee/calculator vs. ② opponent — see the **"GM-replacement
...
> #214/#215/#220 + re-scopes DH-77) + `legisexecgov` (518fb253, the #221 3-PRIMITIVE content-authoring source —
> Legis-Prop / Pres-Action / Gov-Action) + `ampuelections` (92f0659b, the elections content doc, ~70 primitives
> — ORIGIN of NEW #241 variable term-length + #242 amendable articles) + `housepoll` (aed35c6e, the focus-reps-
> vs-every-seat poll — RESOLVES #219, fully AUTHORS #191) + `excelautomate` (79fbbb46, the spreadsheet GM-
> automation progress log — charter corroboration + ORIGIN of NEW DH-78). **NO new keystone, NO re-sequence,
> top-of-queue UNCHANGED (QW0 → K0/K2 → K3/K4 + scenarioBoot → E1).** 3 NEW gap rows #241/#242 + DH-78 (max gap
> #242, max bug DH-78) + a RECONCILE-HEAVY harvest: #214/#215/#220 UNBLOCKED, DH-77 RE-SCOPED, the `pv*0.1`
> double-count RULED OUT, #219 RESOLVED, #191 FULLY AUTHORED, #221 given its authoring source. **The two
> headlines are a TINY high-leverage REMOVAL and an EPIC becoming buildable.** **The value:**
> **(1) ★★★ HEADLINE A → REMOVE `pv*0.1` FROM ELECTIONS (debt #85; XS quick-win; `GM⇒App`; SUPERSEDES the
> batch-30/31 "re-tune `pv*0.1`" note).** `calcStateVote` scores each candidate `50 + baseLean*5 + partyPref*5 +
> enthusiasm*2 + pv*0.1 + factionBias + traitBonus + (Math.random()-0.5)*8` (`phaseRunners.ts:3709`, `pv =
> c.pvCache` `:3699`). The designer + room RULED **PV must NOT feed elections** — pricing PV into the vote
> DOUBLE-COUNTS the traits/abilities already in BOTH PV and the election roll (`groupthinkpv` POST 36-38; a
> highest-PV election-bonus proposal was REJECTED). **So the prior "re-tune `pv*0.1`" call is REPLACED by DELETE
> `+ pv * 0.1` at `:3709`** — the term should not exist. XS and high-leverage twice over: **(a)** it pairs with
> **QW19's election-determinism work at the SAME line** (route the `:3711` `Math.random` jitter through `rng.ts`
> in one touch — debt #1/#71); **(b)** it DECOUPLES the #214 PV reweight from elections so the reweight can
> re-fit freely without distorting margins. **★ Tech-lead judgment-call (one line, NOT auto-done):** the 2.9.1
> PRIMARY scorer's raw `pvCache` (`:3733`) is **KEPT** — the primary is candidate *selection* (the CPU picking
> its nominee = the AI-utility use the ruling ENDORSES); remove ONLY `:3709`. **→ `pv*0.1`-removal JOINS the
> cheap-fixes lane** (added as a new XS quick-win pairing with QW19/QW22/QW23). debt #85; game-mechanics §3.4.1.
> **(2) ★★★ HEADLINE B → THE PV EPIC IS NOW BUILDABLE END-TO-END — #214 + #215 + #220 UNBLOCKED (debt #85;
> `GM⇒App`; moves from GATED → READY-AFTER-#216).** The long-flagged gate — "the trait EFFECTS live in an
> un-ingested thread" — is LIFTED: `groupthinkpv` IS that thread (the EFFECTS/office-PV sibling of `revampPV`'s
> VALUES). Shipped state re-confirmed: flat ±4/−5 (`pv.ts:76-77`), `command×10` (`:74`), `Math.max(0)` (`:88`);
> the **trait-EFFECTS-in-PV layer is 0% built** (verified — `pv.ts` has NO trait→office-conditional logic).
> Direction now RULED: ability curve **Command×15 · Legislative/Governing/Admin×10 · Military/Judicial×5** per
> level (+ expertise +5 each); display **0–100, 50 = average pol**; worst trait = Incompetent −20 (the
> spreadsheet `lackey −53` was a TYPO → ruled −5, already correct in `pv.ts` → re-scopes DH-77). **Sequence the
> epic:** **#216 trait-name remap FIRST** (QW22, already queued, BLOCKS the rest) → then **#214 tiers (M-L
> re-fit) + #215 curve (M) + #220 0–100/floor-offset (XS+S) + remove `pv*0.1` (XS, headline A) + BUILD the
> trait-EFFECTS-in-PV layer (M-L** — office-conditional ability buffs, Crisis-*, Magician take-higher,
> Cosmopolitan/Provincial regional EV, Lackey behavioral lock-in; overlaps the §14.1.y #217 implementation
> engine). **The ONLY remaining human-call across the whole epic = the office-magnitude ↔ 0–100 display-scale
> reconciliation.** **★ Move the PV epic from the gated/Author-before-build bucket to ready-after-#216 (annotate;
> do NOT re-sequence the top-of-queue); office-PV is scoped to the active-draft/persuasion AI, NOT the vote
> (consistent with headline A).** debt #85; game-mechanics §3.4.1.
> **(3) ★★ #221 THE 3-PRIMITIVE CONTENT-MODEL REGISTRY — primary authoring source DELIVERED (L, downstream of
> K4; `GM⇒App`).** `legisexecgov` is vcczar's explicit crowdsourcing call for a registry of THREE content
> primitives: **Legislative Proposals** (federal+state bills incl. constitutional Amendments) / **Presidential
> Actions** (president-only, inactive pre-Constitution) / **Governor Actions** (state-scoped), with era/date
> gating, importance-curation (joke laws REJECTED so the CPU doesn't propose them), meter/payoff tradeoffs,
> prereq-chain ladders, and option-set/branching primitives + scripted events. **VERIFIED 0% shipped:** `grep
> -riE 'presAction|legisProp|govAction|scriptedEvent' src/` = ZERO; the only adjacent thing is the runtime
> `Legislation` INSTANCE record (`types.ts:1506`) built from tiny inline `templates` arrays. **#236/#237 both
> RIDE this** — neither lands before #221. **L (a new typed primitive registry + era-gating + the CPU-selection
> draw); folds into the era-content/K4 area, NOT a new keystone; stays where it is in the queue (downstream of
> K4) — now WITH its authoring source.** debt #97/#98 area; game-mechanics §15.1.
> **(4) ★ NEW BACKLOG — #242 AMENDABLE ARTICLES + #241 VARIABLE TERM LENGTH + DH-78 TRAIT-COMBO (debt #101/#102/
> #103; all `GM⇒App`).** **#242** (M, gated on #221): `ConstitutionalArticles` (`types.ts:1389-1397`) is written
> ONCE at the CC (`constitutionalConvention.ts:145`) and never mutated; the design wants it AMENDABLE mid-game
> (a Legis-Prop Amendment rewrites an article on 75%-state ratification) + NEW fields (suffrage/EC-toggle/
> houseSizeCap/term-length). Downstream of #221 (an Amendment IS a Legis-Prop) + #241; the data shape already
> exists, the build is the mutation PATH + new fields + a `DB_VERSION` default. **#241** (M, → K3's cadence):
> the election cadence is hardcoded `year%4`/`%2` (`phases.ts:53-58`), NO term-length field anywhere; a passed
> 6-/2-yr-presidency Amendment would break every predicate. This is the **SECOND independent reason to replace
> the year-predicates** (the FIRST is #92's era-advance) → build the predicate-replacement ONCE inside K3's
> `advanceEra`/`year%4`-replacement work and a per-office term-length field serves both; ties #92. **The cost is
> shared with K3 — no standalone epic.** **DH-78** (XS-S, gated on #216): `TraitElectionRule` (`types.ts:727`)
> has only `opponentConditional` — it CANNOT express a same-candidate two-trait AND-combo (Celebrity AND Military
> Leader → +1, `excelautomate` POST 50); add a `requiresAlsoHas: Trait[]` field + a consume-side check in
> `applyTraitElectionBonus` (`electionEffects.ts:16`); gated on #216 (which adds `Military Leader` to the union);
> verify it wasn't deliberately cut first. debt #101/#102/#103; game-mechanics §14.2, §2.3, §14.1.3.
> **(5) ★ THE HOUSE MODEL — #219 RESOLVED + #191 FULLY AUTHORED (S-M; debt #87/#73; `GM⇒App`).** `housepoll`:
> the community VOTED to **KEEP the focus-Rep abstraction; every-real-435-seat was REJECTED → parked "AMPU 2."**
> Shipped state: flat `representativeIds: string[]` (`types.ts:1327`) — NO size enum, NO EV-ladder, NO
> vote-weight, NO census. **#219 (RESOLVED):** US Reps = historical EVs−2 → Focus Reps via an **EV-derived 1-10
> ladder** (≤5→1 … ≥46→10, SUPERSEDES #55's flat `(EV−2)/5`) → **vote power = `floor(stateUSReps / focusReps)`,
> remainder → highest-Legislative rep**. **#191 (FULLY AUTHORED, `housepoll` POST 17-19):** a per-seat STATIC
> `deviantPartyBias` (overrides only the party term) + the minority-seat guarantee (5+ Focus Reps → a Bias+2
> minority seat) + the 25/25/50 new-seat / seat-gain / seat-loss rolls keyed to **era-gated growth lobbies**.
> Both ride a NEW **per-seat struct** + the #34 census + the row-74 gerrymander gov-action that SETS the bias.
> **S-M; folds into the census/apportionment + down-ballot-election clusters (#34/#55/#73/#74); not a new
> keystone. Mark #219 RESOLVED (keep focus-reps; every-435 = AMPU-2 parking) + #191 FULLY AUTHORED.** debt #87/
> debt #73; game-mechanics §29.5.
> **(6) ★ SHARPENING — #20 Gov Actions FLAT/state-agnostic.** `legisexecgov` confirms Gov Actions today are
> FLAT / state-agnostic (the Gov-Action primitive should be state-scoped with per-state variation) — sharpen the
> existing #20 gerrymander/gov-action umbrella row (debt #74); no re-size. **DH-77 RE-SCOPE reflected:** the PV
> floor-clamp fix (QW23) is re-scoped by `groupthinkpv` — worst trait is Incompetent −20 (not the spreadsheet
> `lackey −53` typo); the floor-OFFSET fix stands, the magnitude is corrected. game-mechanics §15.1, §3.4.1.
> **(7) ★★ GM-REPLACEMENT LENS additions (a VIEW, no re-sequence).** ① referee gains: **PV-as-AI-utility** —
> annotate that PV is the ② CPU's draft/steal/persuasion/leadership/primary-selection metric and is **NOT an
> election input** (the `pv*0.1` removal ENFORCES that boundary; the primary scorer's `:3733` raw `pvCache` is
> the endorsed AI-utility use, KEPT); the **#221 content-primitive registry** (the typed action pool the GM
> hand-curated, with the CPU-selection draw #70-#78 reading from it); the **House model** (the EV-ladder +
> vote-power-with-remainder = the seat-tally math the `excelautomate` GM ranked automation priority #1, and the
> focus-Rep abstraction is the deliberate ">435-seats is AMPU-2" tractability call); and **#241/#242** as the
> referee half of constitutional adjudication (amendable rules). **★ The `excelautomate` charter metric — the
> *">30 min/phase ⇒ automate"* GM-burden budget (game-mechanics §18.1)** — is the concrete quantified statement
> of the ① mandate; corroborates #114/DH-36 from the build side. Lens stays a VIEW; adding pointers re-sequenced
> nothing.
> **(8) ★ HYGIENE / CORPUS-COVERAGE.** **★ "Groupthinking the Political Value" is now INGESTED** (`groupthinkpv`,
> batch 33) — REMOVE it from the still-needed un-ingested list; the un-ingested set is now **1840-start
> (persistent-Whig ~100-yr run) + matthewyoung123-1772** (the rookie founding run). **NO gap closed (5 RECONCILED:
> #214/#215/#220 unblocked, DH-77 re-scoped, #219 resolved, #191 authored, the `pv*0.1` double-count ruled out),
> NO new keystone, NO re-sequence; top of queue UNCHANGED (QW0 → K0/K2 → K3/K4 + scenarioBoot → E1) — the ONE
> delta to the cheap-fixes lane is `pv*0.1`-removal as an XS quick-win, and the PV epic moves gated →
> ready-after-#216. Within-batch order: remove `pv*0.1` + #216 remap (cheap-fixes, now) → the PV epic (#214/#215/
> #220 + trait-EFFECTS layer) → #221 registry → #242 amendable articles + #241 cadence (inside K3) → the House
> model → DH-78.** debt #85, #87, #97/#98, #101-#103; technical-guide §9 batch-33 lead + §9.6 + §8 debt #101-#103
> + updated #85/#87/#73; game-mechanics §3.4.1, §15.1, §15.4(c), §29.5, §2.3, §14.2, §14.1.3, §18.1, §30.23.
>
> **★★★★ Batch-32 version — FIVE MIXED DATA/DESIGN/CHANGELOG threads (1 founding solo + 4 data/design/changelog),
> NO playtest: `tomorrowlist` (vcczar's to-do + crowdsourcing call, Feb 2022 — the ORIGIN of the alternate-
> government-form axis) + `businesslabor` (vcczar AUTHORS the Business/Labor policy GENRE — the origin of the
> stateful policy-genre framework) + `histpres` (vcczar's 47-real-president AMPU statlines, Washington→Biden, +
> the curated-pol record SCHEMA) + `georgemartha` (a 1772 SOLO — the gender/suffrage model; the ~9th 1772 source)
> + `calsleaderboard` (a ranking-record companion to the already-ingested `summer2021`). **NO new keystone, NO
> re-sequence, top-of-queue UNCHANGED (QW0 → K0/K2 → K3/K4 + scenarioBoot → E1).** This batch is **mostly NEW
> designed-but-unbuilt SURFACES + one DATASET-fill action — almost nothing is shipped.** 5 NEW gap rows #236–#240
> (max gap #240 = the `CURATED_ROWS` dataset fill) + NO new design-hole bug (max bug DH-77). Three of the four
> design threads are vcczar-authored — the strongest CONTENT-AUTHORING batch since batch 30/31. **The value:**
> **(1) ★★ HEADLINE / MOST ACTIONABLE → #240 THE `CURATED_ROWS` DATASET FILL (debt #100; a STANDALONE author/DATA
> task OFF the engine track; `GM⇒App` — removes the GM's statline math).** **★ CORRECTED FACT (a prior-role error
> is FIXED): `CURATED_ROWS` is NOT empty at runtime.** `seedDataset.mjs:275` `export const CURATED_ROWS = [];` is
> only the INITIALIZER — the loop at `:276-289` populates it from the `ROWS` tuple-array (`:38-233`), giving
> **143 curated entries at runtime** (runtime-verified `node -e` import → `CURATED_ROWS.length === 143`;
> `ERA_FIGURES` = 23 from `ERA_ROWS` `:242-267`). What is TRUE: `ROWS`/`CURATED_ROWS` hold the **1772+1856 marquee
> in-game roster, NOT the 47 presidents** — vcczar's presidential statlines (`histpres`, Washington→Biden) are
> simply NOT authored in. **So #240 = "ADD ~47 president statlines to the `ROWS` source array"** (which the loop
> folds into `CURATED_ROWS` as same-named OVERRIDES, beating the `congress-legislators` real-served rows per the
> `CLAUDE.md` merge precedence), then regenerate (`bash scripts/fetchLegislators.sh && node
> scripts/legislatorsToDataset.mjs && npm run build`) — **NOT "fill an empty array."** **★ HARD PREREQ — the #216
> trait-name remap (= QW22, already queued in the cheap-fixes lane):** 14/50 `histpres` trait names mismatch the
> shipped `Trait` union (`types.ts:62-117`) — `Charisma`→`Charismatic` + 13 with NO shipped analog (`Bookkeeper,
> Cop, Disharmonious, Geostrategist, Illicit, Incoherent, Jurisprudence, Lackey, Late Bloomer, Low Brow, Military
> Leader, Pliable, Teflon`) — an importer CANNOT author the tuples until the rename/cut map exists or they carry
> invalid `Trait` values. **★ The Misc/Demographics/Dynasty lines of each statline need the #99 data-model cluster
> first** (so the FULL statline waits on the schema; the skills/traits/ideology CORE can land once QW22 does).
> **★ HUMAN-CALL: trust the 2021-vintage raw skills vs. re-derive under the current #214/#215 PV curve** (every
> president maxes a single ability at **3** — no 4s/5s, consistent with #204). **S-M; a pure author/DATA task; NOT
> a build-queue change — runs standalone once the #216 remap (QW22) lands.** debt #100; game-mechanics §30.22.C.
> **(2) ★★ #236 THE ALTERNATE-GOVERNMENT-FORM SCENARIO AXIS (debt #97; DESIGNED, 0% shipped; `GM⇒App` — the
> REFEREE half).** VERIFIED wholly unbuilt: a `src/` scan for `fascist|communist|theocra|governmentForm` returns
> ONE hit — the `Theocrats` pressure-group faction STRING (`factions1856.ts:28`), NOT a USA-government-form state.
> The country itself BECOMES the regime — **Fascist / Communist / Theocratic** — each unlocking its own
> Pres-Action/Legis-Prop/Scripted-Event menu + candidate pool, plus a reusable cross-form **"authoritarian kit"**
> (ban-opposition-parties / empower-executive-via-amendment / ban-politicians-by-class / state-media /
> nationalize-sector / dissolve-Congress); Theocracy branches ×5 by denomination. Fix = a `game.governmentForm`
> flag that swaps in the form's menu + pool. **★ TWO HARD DEPENDENCIES keep it OFF the near-term queue:** (a) it
> RIDES the **unbuilt #221 per-administration content-primitive registry** — there is NO Pres-Action/Legis-Prop/
> Gov-Action/Scripted-Event CONTENT SYSTEM in `src/` (`grep -iE 'presAction|legisProp|govAction|scriptedEvent'
> src/` = ZERO), so there is nothing to attach a regime menu to; (b) **Theocracy is BLOCKED on the #239 religion
> demographics field** (the engine must assign a denomination per pol + form denominational party-splits). **★
> DISTINCT from #6/#354** (mid-era social-MOVEMENT factions / the `Theocrats` lobby — a movement-faction is a
> lobby; a government-form is a scenario branch). **★ HUMAN-CALL: the transition TRIGGER + the win/loss/
> persistent-branch END-STATE are a DESIGN ruling** (ties #88/#188 — the only game-overs in the corpus are mid-run
> meter/scripted LOSSES). **L; folds into the era-content / #221 content-primitive area, NOT a new keystone; gated
> on #221 + #239-religion + §24 amendments.** debt #97; game-mechanics §30.22.A, §24.x.
> **(3) ★★ #237 THE STATEFUL POLICY-GENRE FRAMEWORK (debt #98; DESIGNED, 0% shipped; `GM⇒App` — the REFEREE
> half).** VERIFIED: NO policy-genre/policy-flag store in `src/`. "Business"/"Labor" exist ONLY as `Expertise`
> tags (`types.ts:182-186`) + faction economic-lean SIGNS (`EXPERTISE_IDEOLOGY_LEAN`, `:421`) — static inputs,
> NOT a stateful system. A **policy genre** = a named family of discrete, toggleable, prerequisite-gated policies
> (Business/Labor is ONE; **Currency + Copyright are sibling genres**), each with a `*-Default` baseline
> (absence-of-policy IS a state), an `L/P/G/S` mechanism prefix, an era-band tag from the **SAME #92/#206
> vocabulary**, and `Preq:` chains gated on other policy flags / scripted-event state / **the SHIPPED `economic`
> meter** (`NationalMeters.economic`, `:1401` — read as a gate AND fed back into: Recession→Create-Government-Work,
> Depression→WPA/PWA) / party-type flags (Communist-major→Nationalize-Unions — **ties the #236 kit**). Fix = a
> `game.policies` flag store + the genre/prefix/era-tag/Preq schema + the economic-meter wiring + content for the
> 3 genres + the symmetric pro-business institutions (National Business Council / Sec. of Business). **★
> DEPENDENCY: like #236, the `L/P/G/S` entries ARE Legis-Props/Pres-Actions/Gov-Actions/Scripted-events → this
> RIDES the unbuilt #221 content-primitive registry**; couples to the SHIPPED EconStab economic engine (§29.7/
> §29.7.1, #116/#160). **★ DISTINCT from interest-cards/lobbies** (those nudge state industries; this is a
> national + per-state policy-FLAG axis with a baseline + prereq chains). **★ HUMAN-CALL (POST 12): the "no
> policy" default = flat "left to the states" vs computed-from-state-makeup.** **M-L; folds into the era-content/
> policy area alongside #236; gated on #221; couples to the SHIPPED economic engine.** debt #98; game-mechanics
> §30.22.B, §29.7, §29.7.1.
> **(4) ★ #238 / #239 / #194 THE CURATED DATA-MODEL CLUSTER (debt #99; DESIGNED/DATA, 0% shipped; `GM⇒App`) → an
> M data-model extension; the SCHEMA HALF of #240.** VERIFIED ABSENT on both data shapes: `Politician`
> (`types.ts:1251-1291`) and `ImportedDraftee` (`:1780-1793`) carry NO `gender`/`sex`, NO `spouseId`/`parentId`/
> `dynasty`/family edge, NO `religion`/`race`/`demographics`, NO `southernUnionist`/`canBeIndependent`/
> `partyFlipBudget`/`canMoveStates`. Build all three together as ONE data-model extension: **(#238 gender)**
> `Politician.gender` + a suffrage-keyed electoral-eligibility GATE (women→backroom/lobby-only pre-suffrage) + a
> spouse/family relationship EDGE enabling spousal Key-Advisor (Ted-endorsed); **(#239 curated-override flags)**
> `southernUnionist` (corroborates #121/#122) / `canBeIndependent` / `partyFlipBudget` (Trump ×3, Reagan-starts-
> Dem) / `canMoveStates` (atop the SHIPPED relocation `Politician.altState` `:1256-1258` — a curated
> AUTHORIZATION, not a new system) + the **race×religion×sex demographics triple — the `religion` field is the
> PREREQUISITE for the #236 Theocracy branch** (NOT low-priority flavor); **(#194 dynasty)** wire the `histpres`
> turnkey **dynasty SEED TABLE** (~37 named dynasties incl. multi-president Adams/Harrison/Roosevelt/Bush) into
> the existing §30.18.E #194 lineage spec (the family edge is shared with #238). **★ Build the relationship-edge +
> the demographics tuple TOGETHER (the religion field gates #236-Theocracy).** **★ This cluster is the SCHEMA HALF
> of #240** (debt #100 is the DATA half — the statlines carry exactly these Misc/Demographics/Dynasty lines).
> **M; folds into the dataset/roster work; needs a save-migration (`DB_VERSION` bump).** debt #99; game-mechanics
> §30.22.C, §30.18.E, §3.1.
> **(5) ★ SHARPENINGS (annotate existing rows; no re-doc, no re-size).** **#194** gains a TURNKEY dynasty seed
> table (`histpres`, ~37 named dynasties) — the lineage spec now has its data. **#216/#214** — `calsleaderboard`
> is the cleanest NARRATIVE confirmation that AMPU influence tracks **office-weighted PV** (Presidency ≫
> doc-authorship/CC-Pres > governor/Congress > faction-leader; office LOSS = instant PV collapse) AND that **raw
> command is OVER-CREDITED early** (a 0-domestic war-hero holds #1 on pure command then slides) — the
> **decreasing-Command-returns target of the #215 curve** (the shipped flat `command*10` at `pv.ts:74`; the flat
> ±4/−5 per-trait at `:76-77` is the #214 target; skill-cap-3 per #204). Confirms the office-weighting is RIGHT;
> folds into debt #85. **#206/#221 Era-of-Future NON-tech content** — BOTH `tomorrowlist` + `businesslabor`
> confirm the Future band is an actively-authored content frontier, explicitly NON-tech (e-democracy, USCoin,
> drones, lab-grown meat, AI/robot-employee restrictions); reconfirms #206 (Future DOUBLY UNBUILT) + #221; no new
> architecture. **#115/#24** `georgemartha` argues for a curated "what-if" draft-pool toggle (women + Toussaint),
> broader than the shipped male-only `ERA_FIGURES` (`seedDataset.mjs:293`); ties #238 (gender makes drafted women
> PLAYABLE); folds into #115. **#158/#159/#114/DH-36** founding corroborations (Rev-War-Ends auto-prompt /
> Convention per-article ambiguity / solo-burden — `georgemartha` is the ~9th 1772 source); pure corroboration.
> **(6) ★ GM-REPLACEMENT LENS — all four NEW surfaces are `GM⇒App` (a VIEW, no re-sequence).** ① referee gains the
> **government-form axis (#236)** + the **policy-genre framework (#237)** as referee-content systems (the GM
> hand-adjudicated regime transitions + hand-tracked which policies are active + their `economic`-meter effects).
> The **data-model cluster (#238/#239)** is the OPPONENT/referee boundary (the GM hand-decided who may go
> Independent / flip parties / stay Unionist → data-driven gates the CPU + engine read), and the **dataset fill
> (#240)** is content-authoring that removes statline math from the GM — both noted as FOUNDATIONAL. None can be
> left to a human; they are exactly the work the north-star removes.
> **(7) ★ HYGIENE / SEQUENCING.** **Within-batch order (binding): the #216 remap (QW22, already queued) → #240
> core skills/traits fill (standalone) → #99 data-model cluster (schema) → #240 full Misc/Demographics fill;
> #236/#237 wait on #221 in the era-content area.** **NO gap closed, NO new keystone, NO re-sequence; top of queue
> UNCHANGED (QW0 → K0/K2 → K3/K4 + scenarioBoot → E1).** debt #97-#100; technical-guide §9 batch-32 lead + §9.6 +
> §8 debt #97-#100; game-mechanics §30.22.A, §30.22.B, §30.22.C, §30.18.E, §29.7, §24.x, §3.1, §3.4.1.
>
> **★★★★ Batch-31 version — FIVE RULES/FEEDBACK/TEST/DATA threads, NO playtest (a RULES/FEEDBACK/TEST
> batch — no playtest exercise; and a RECONCILE-HEAVY batch): `rulebook` (`b9e4f534`, "AMPU Rules for
> Playtest" — the CANONICAL RULEBOOK, ~24 systems, designer @vcczar; the single most authoritative mechanics
> spec in the corpus; STOPS before the election phases the author withheld as "massive") + `apptdeepdive`
> (`b9028796`, "Deep Dive Feedback: Presidential Appointments & Confirmation Process" — the Sept-2022
> DESIGN-ORIGIN of the appointment/confirmation ruleset, @MrPotatoTed-led, vcczar-final) + `primary2020`
> (`74f1da28`, "The 2020 Democratic Primary: Robust Testing Forum" — a May-2022 balance/edge-case STRESS-TEST,
> NOT a played-forward game) + `playtestdraft` (`38ecfaeb`, "AMPU Playtest Draft" — a founding-era 1772/1774
> DRAFT-PROCESS record) + `bestmoments` (`4f90149c`, "AMPU Best Playtest Moments" — a HIGHLIGHTS/design-intent
> digest). **NO new keystone, NO re-sequence, top-of-queue UNCHANGED (QW0 → K0/K2 → K3/K4 + scenarioBoot →
> E1).** 8 NEW gap rows #228–#235 (max gap #235) + NO new design-hole bugs (max bug DH-77). **A RECONCILE-HEAVY
> batch:** #199 SUPERSEDED, #183 RESOLVED, #182 CONFIRMED, #172/DH-76 SHARPENED, #92/#206 era-table, #50/#51 +
> #179/#134 canonical algos pinned, POST-143 designed→built CLOSURE. **The value:**
> **(1) ★★★ HEADLINE → THE CABINET/SCOTUS CONFIRMATION SUBSYSTEM is now FULLY SPECCED + WHOLLY UNBUILT (debt
> #91; `GM⇒App`, the REFEREE half) — it GROWS the existing cabinet/appointments epic (debt #50, E16+E14c) from
> S-M to M-L; it is NOT a new keystone and NOT a duplicate row.** VERIFIED absent end-to-end:
> `runPhase_2_3_1_Cabinet` (`phaseRunners.ts:2158`) auto-picks the top scored+PV candidate (`:2187`), assigns
> the seat (`:2191`), logs `"… confirmed as ${seat}"` (`:2197-2198`) with **NO vote / committee / party-block /
> filibuster / cloture / nuclear-option / auto-confirm-gate / Top-Four / promise**; SCOTUS has the SAME gap at
> `runPhase_2_8_2_CourtMgmt` (`:3667`, no vote); `grep -iE 'nuclearOption|cloture|filibuster|topFour' src/` =
> ZERO. The game-master pinned the **complete 12-rule procedure (game-mechanics §9.3.11)** from the two PRIMARY
> sources `rulebook` + `apptdeepdive`: auto-confirm gate (Admin≥3 ∧ ¬Controversial ∧ ¬IronFist-SML ∧ ¬Top-Four
> → full hearing) → hearing at the **#172 era-keyed threshold** (default 60% cabinet+SCOTUS; Nuclear-Option →
> simple majority) → SML party-wide block [5 reasons; cap = opposing leader's Legislative; blocked → Pres picks
> a concern-alleviating replacement, auto-confirmed] → **Puritan filibuster + Manipulation-cloture [the DH-76
> half finally supplied]** → president's tools [higher-Legislative compel / Iron-Fist compel / convert
> N=Legislative moderate/pliable senators] → vote [Admin-1 50/50-flip = the DH-23 root cause; trait→vote 75%] →
> **#199 defeated-nominee fallout = the 33/33/34 blame roll + already-Controversial→lifetime-cabinet-ban,
> SUPERSEDING the Controversial→Incompetent cascade** → SML 5-name recovery → office-vacancy cascades → the
> #181 promise. **★ Build flag:** the two "Top-Four" sets DIFFER (always-vote = State/Defense/Treasury/**War**;
> cross-party-cap = State/War-Def/**AG**/Treasury). **★ SIZING (binding):** debt #50 was scoped S-M (boot flag +
> per-track threshold + SML enact/repeal); the canonical spec adds the gate + block + filibuster/cloture +
> fallout + vacancy cascades + promise → **re-size to M-L and re-scope E16 to own the whole §9.3.11 flow; do NOT
> create a duplicate confirmation epic — GROW E16.** **GATED on K2** (the SML enact/repeal Nuclear-Option row +
> the president's-tools rows are ActionRegistry entries) **+ K5** (CPU votes / SML block / cross-party support
> are CpuController handlers). debt #91; game-mechanics §9.3.11.
> **(2) ★ THE 15-ERA TRIGGER-YEAR TABLE → reinforces K3/K4 (debt corroboration on #92/#206; the data-model
> input).** The rulebook (`rulebook#§A POST 2`) prints the canonical **15-era trigger-year calendar** (Independence
> 1774 … Populism 2012 → Era-of-the-Future = after the current real-world cycle) and states eras "auto-update
> when their trigger date is reached." The shipped `Era` enum is the 4-value `independence/federalism/
> nationalism/modern` (`types.ts:1337`); the 15 named bands collapse into those 4 mechanical buckets → the table
> is the **authoritative band-label spec K4's per-era content-band registry keys on** (the `{era, startYear}` /
> trigger-cycle map), RECONFIRMING the batch-29/#206 push toward the #92 two-level content-band model (the 15
> labels become content-band DATA, not 15 enum values; the transition predicate stays game-state-gated per #92,
> with "trigger date reached" as the EARLIEST a band opens). Reconciles with the modernday 14-band map (#2) +
> the nuke "Neocons = rebrand, not a band" finding via the two-level model. **★ Annotate K3/K4/#92; no re-size,
> no new keystone — it is the data the registry consumes.** game-mechanics §27.1.2.
> **(3) ★ #182 CONFIRMED → reinforces K2 (debt #92; `GM⇒App`).** `command` was a single-thread inference (batch
> 30); the rulebook makes it authoritative (`rulebook#POST 30`): *"The President may take as many executive
> actions per half-term as he has Command Points."* Re-confirmed `command` is ONLY a PV input today (`pv.ts:74`
> flat `command*10`) + VP/eligibility gates; the exec phase is a flat `chance(0.5)` (`phaseRunners.ts:3635`) over
> 4 hardcoded one-shots. Build = replace the coin-flip with a **command-sized loop over the K2 ActionRegistry
> exec-action library** (one of K2's 6 libraries) + the persistent active-action set + green/yellow/white
> lifecycle (debt #23/#62). **★ Annotate K2 (no longer a single-thread inference); S budget loop / M
> library+lifecycle; pairs #179/#217.** debt #92; game-mechanics §11.2.1.
> **(4) ★ #179/#134 LINGERING + #50/#51 CARD-ASSIGNMENT — canonical algorithms now PINNED (debt #93; `GM⇒App`)
> → annotate the lingering/card-assignment epics, no re-size.** **#179/#134:** the shipped lingering pass is a
> flat deterministic `drift(skill)` fixed by Admin tier (`phaseRunners.ts:3266-3273`); `rulebook#POST 20` is the
> canonical algorithm — a per-meter positive%-vs-negative% PIT → roll-if-it-moves → roll winning-direction
> (top-2/bottom-2 = 25% no-change, else 50%) + the cabinet-save admin ladder (Efficient doubles; Incompetent
> can't roll positive) + the officer→meter map + the **decay numbers** (tariffs/income-tax decay 10%/2yr once
> age≥10, DO NOTHING at age 20 → crisis; volatility at 2/4/8-yr marks; crises/booms never >10 yrs). M (a real
> roll engine; #134's 7-step strict ordering already lives at E16/§30.21). **#50/#51:** the card layer is partial
> (`Faction.*Cards`, `types.ts:1298-1300`, no assignment engine); `rulebook#POST 10` is the canonical engine —
> plurality-of-own-pols assignment (every faction MUST hold ≥1 ideology card) + interest/lobby compatibility-ban
> tables + a 5-pol FLOOR + a cascade to the next-eligible faction; corroborated by `playtestdraft` (cards derive
> from the draft, #24). M (the card-assignment pass feeds the enthusiasm reshuffle). Both fold into existing
> clusters (E6/E16/E23); neither is a keystone. debt #93; game-mechanics §30.21.
> **(5) ★ NEW ELECTION DELTAS — #228 spoiler / #230 swing-state cap / #231 modern-Moderate (debt #94).**
> **#228** (wholly unbuilt — no `spoiler`/`thirdParty` drain in `src/`; `GM⇒App`): a third-party VOTE-DRAIN keyed
> on party-of-origin + shared-ideology + incumbency with the Charisma region→nationwide escalation, plus the fix
> to the current incumbent-only mis-attribution (a 3rd party from the CHALLENGER must drain the challenger). M;
> pairs #48 (3rd-party trigger/PV) + #214 (Charisma valuation). **#230** (`calcStateVote` `phaseRunners.ts:3685`
> scores all states identically; the `(Math.random()−0.5)*8` jitter at **`:3711`** can flip safe states): cap the
> competitive map to party-pref ≤1 battlegrounds + a bounded safe-state margin. **★ This is the SAME `:3711`
> jitter surface as QW19 (the `Math.random`→`rng.ts` determinism fix) + the ±3 meter-swing clamp — PAIR them:
> seed the jitter via `rng.ts`, then gate it behind the swing-state set. S-M; pairs QW19/#184/DH-72/#18.**
> **#231** (designer-diagnosed SYSTEMIC, not rolls): the modern-era Lib/Prog/LW-Pop enthusiasm maluses + the
> Moderate primary-penalty/general-bonus make Moderates sweep modern primaries reproducibly (Buttigieg sweeps
> every 2020-primary run) — **a balance REGRESSION CASE to re-tune when #18/#51 land**, not a new system. debt
> #94; game-mechanics §22.4.1.
> **(6) ★ APPOINTMENT-SIDE + FOUNDING deltas — #229 / #199 re-spec / #233 (debt #95; #229/#233 `GM⇒App`).**
> **#229** (wholly unbuilt — the appointee always accepts today; the OPPONENT half): the per-candidate-type CPU
> acceptance/decline matrix (`apptdeepdive#POST 40`) + the human rule (free accept/decline ANY nomination EXCEPT
> VP) + the trait gates (Incompetent/Easily-Overwhelmed/fired-quit) + career-track-pulled-off-track-with-
> milestones. M; **distinct from #73 (the CPU picker) and #91 (the confirmation vote)** → folds into E9 handler
> 9d + the E16 appointment epic. **#199** RE-SPEC: lands INSIDE #91's rejection path as the 33/33/34 blame roll +
> already-Controversial→lifetime-cabinet-ban (SUPERSEDING the Controversial→Incompetent cascade) — **XS once #91
> exists; re-spec BEFORE building #112/#73.** **#233** (founding-era, absent): CC-appointments = committee-chair
> nominates → delegates vote → CC-President breaks ties (incumbent-stays-on-fail), distinct from the
> post-Constitution Presidential phase; wire to `continentalCongress.ts`. S; pairs #133/#101/#92 → folds into E1.
> debt #95; game-mechanics §9.3.11.
> **(7) ★ DRAFT BASELINE + SMALL FOLD-INS — #232 / #211 / #222 / #234+#235 (debt #96).** **#211** re-confirmed:
> the shipped/baseline draft is a **FIXED within-party round-robin** (`runPhase_2_1_1_Draft` `phaseRunners.ts:
> 107` + `pickBestForFaction` `:33`) — NOT a snake, NOT PV-ordered; CPU autodraft = the same `pickBestForFaction`
> (annotate the #211 LOW-priority MP-mode row; no re-size). **#232** (no draft-ideology restriction ships): a
> COLD-START draft needs a tighter restriction (2 ideologies/faction or manual seeding) than a mid-game rookie
> draft (3) — S, folds into the scenario-boot pipeline (#115/K4); pairs #4/#171/#115/#86. **#222** committee-size
> 3→11 at 50 states CORROBORATED by the canonical rulebook (no re-size; already a batch-30 fold-in). **#234
> [PARKING — AMPU-2]** Fog-of-War hidden/scoutable stats (both designers liked it, punted to post-release) +
> **#235 [PARKING]** D6 pre-nomination vetting roll (repeatedly DEFERRED as playtest-impractical) — **do NOT
> schedule.** debt #96; game-mechanics §30.21.
> **(8) ★★ GM-REPLACEMENT LENS — the confirmation/spoiler (① referee) + CPU-acceptance/CC-appointment (②
> opponent) additions (a VIEW, no re-sequence).** New ① referee/calculator pointers: the **confirmation subsystem**
> (#91/#172/DH-76 — the confirmation vote/block/filibuster/cloture/fallout the GM hand-adjudicated each turn) and
> the **spoiler/swing-state** election items (#228 vote-drain + #230 the safe-state cap). New ② opponent pointers:
> the **CPU acceptance-odds matrix (#229)** + the **CC-appointment opponent half (#233)** (how the CPU decides to
> accept/decline a nomination, and how it nominates in the founding CC). The lens stays a VIEW — adding pointers
> re-sequenced nothing.
> **(9) ★ HYGIENE / RECONCILIATIONS + CORPUS-COVERAGE.** **#183 RESOLVED-DIRECTION:** `primary2020` is the
> EARLIEST detailed source for endorsement/withdraw MOMENTUM — it confirms momentum IS specced (serial-group
> withdraw %s + endorsement-splits that VISIBLY redistribute delegates) and the app implements NONE of it →
> **specced-but-unbuilt** (reconcile magnitude across primary2020's 60/75/50% vs completions' 25%/10% vs
> redbutton's none; primary2020 is most detailed) — annotate, no re-size. **#214/#215 got 2020-primary
> PROVENANCE** (the James-Dean #214 exhibit + the #215 office→PV table) — annotate, no re-size. **POST-143
> CLOSURE** (`bestmoments`): the designed→built check confirms Cosmopolitan/Provincial/Magician/Numberfudger/
> Crisis-{Manager,Admin,Gov} SHIPPED (`Lackey` NOT adopted) — no net-new architecture. **★ CORPUS-COVERAGE
> (reconfirmed):** the **1840-start** persistent-Whig ~100-yr run is reconfirmed highest-value un-ingested; the
> **"Groupthinking the Political Value"** trait-EFFECTS thread (gates the #214/#215 PV reweight, debt #85) +
> **matthewyoung123-1772** are also un-ingested — ask the human for exports before any 1840 / PV-effects /
> founding-variant work. **★ NO gap closed (3 RECONCILED: #199 superseded, #183 resolved, #182 confirmed), NO
> new keystone, NO re-sequence; top of queue UNCHANGED (QW0 → K0/K2 → K3/K4 + scenarioBoot → E1) — the headline
> confirmation subsystem GROWS the existing cabinet epic (debt #50, E16+E14c, S-M → M-L), it is NOT a new
> top-of-queue item.** debt #91-#96; technical-guide §9 batch-31 lead + §9.6 + §8 debt #91-#96; game-mechanics
> §9.3.11, §27.1.2, §11.2.1, §22.4.1, §30.21.
>
> **★★★★ Batch-30 version — FIVE DESIGN/DATA/changelog threads, NO playtest (a DESIGN/DATA batch,
> NO playtest exercise): `revampPV` (`4266d56e`, the Feb-2023 PV-SYSTEM OVERHAUL — re-valuing every
...
> tier-1 ALL-DECIDED rule-changes out of the batch-26 `summer2021` run — the ORIGIN of DH-76) + `ampuData`
> (`853ce63f`, the per-administration 1774→2022 content INDEX — an authoring source) + `presidents`
> (`5e00ec8e`, the cross-playtest president RECORD + corpus-coverage map) + `ampuUpdates` (`970616d5`,
> vcczar's Apr-Jul 2021 dev changelog — the earliest `GM⇒App` charter). **NO new keystone, NO re-sequence,
> top-of-queue UNCHANGED (QW0 → K0/K2 → K3/K4 + scenarioBoot → E1).** 14 NEW gap rows #214–#227 + 1 NEW
> design-hole bug DH-77 (max gap #227, max bug DH-77). **The value:**
> **(1) ★★★ HEADLINE → THE PV-SYSTEM OVERHAUL SPLITS into PV QUICK-WINS + a GATED BALANCE EPIC (debt #85;
> `GM⇒App`).** `pv.ts`'s `computePV` ships a PRE-REVAMP formula — flat per-trait `+4`/`−5` (`pv.ts:76-77`),
> flat `command × 10` (`:74`), flat `skill × officeWeight × 4` (`:70-73`), magic `Kingmaker +6` (`:79`), a
> `Math.max(0,…)` clamp (`:88`); office/age/FF terms at `:80-87` (DH-77 is the defect anchor for these
> sites). The `revampPV` thread re-valued every trait on a fixed TIER scale (25/10/7/3/−3/−7/−10, ±25
> extremes) but DIED UNRESOLVED (vcczar disowned the exact numbers — *"the RANKING is the point"* — and the
> fitted formula was deferred to OrangeP47, never delivered). **★ The tech-lead SPLITS this (binding):**
> **(a) PV QUICK-WINS (cheap-fixes lane NOW):** **#216** the trait-name remap (**S — the BLOCKING
> PREREQUISITE** that unblocks #214: ~17 of ~57 P33 trait line-items mis-key/drop vs the shipped `Trait`
> union [`types.ts:62-117` — ~15 absent, 2 renames `Flipflopper`→`Flip-Flopper`/`Two-faced`→`Two-Faced`,
> +12 shipped traits never valued]; an explicit old→new map, independently useful, DO FIRST) + **DH-77** the
> floor-offset clamp fix (**XS** — swap `Math.max(0,…)` at `pv.ts:88` for a constant floor-OFFSET so the CPU
> keeps ordering among sub-zero pols; the clamp is the most clearly *wrong* line today). **(b) BALANCE EPIC
> (GATED):** **#214** per-trait tiers (**M-L**) + **#215** per-ability curve / Command re-weight (**M** —
> first-point-worth-more, Command ≈25/level decreasing, Admin/Military low-floor; restrict-born-Command left
> OPEN as a toggle) — **GATED on the un-ingested "Groupthinking the Political Value" trait-EFFECTS thread**
> (P33 sets only POINT VALUES), and the **SAME PR must re-tune `calcStateVote`'s `pv*0.1` (`phaseRunners.ts:
> 3709`) — `pv = c.pvCache` (`:3699`) feeds every state-vote score, so a PV reweight DIRECTLY shifts election
> margins** (same election-balance surface as #18/#184/DH-72/QW19-21). **#220** PV display-scale = a **S
> DECISION** (raw "pink" [revampPV lean] vs 0-100/100-normalized [summer2021 POST 200-205], UNRESOLVED) →
...
> **(2) ★★ #217 the president/cabinet IMPLEMENTATION sub-engine + ~30 governing traits (debt #86; deepens
> #179; `GM⇒App`) → an L SYSTEM LAYER, gated on federalism.** A DISTINCT layer beyond the flat `traits[]`:
> after legislation/actions pass, a president+cabinet success→Blunder roll, modified by ~30 expertise-gated
...
> (`phaseRunners.ts:3260`) + the flat `chance(0.5)` exec action (`:3632`). **★ DE-DUP RISK** (several overlap
> Efficient/Egghead/Easily-Overwhelmed — reconcile BEFORE adding) + these traits need PV values (#214) + the
> remap (#216). **L (≥2 systems — rides #179's roll engine + #182's action budget + §14.1.y); folds into
> E16/E6.** debt #86; game-mechanics §20.10.1.
> **(3) ★ #221 the era-content registry needs TWO SCHEMA FIELDS (debt #88; sharpens K4 / #92 / #206) → folds
> into K4.** The `EraEvent` shape (`types.ts:1466-1482`) lacks both: (1) a **scripted-vs-flavor tier flag**
> (flavor events are narration-only, post-launch scope — vcczar declines flavor suggestions, `ampuData` POST
...
> TERMINATES at Biden/2022 with zero future-band rows — the authoring-side confirmation of #206/#92's
> two-level content-band model. debt #88; game-mechanics §27.1.1.
> **(4) ★ #218 Rule-of-Four / CJ case-refusal (S) + #219 US-Rep PROPORTIONAL representation (S) (debt #87) →
...
> ≥4-Judicial CJ may refuse a Gov-Action state case; Rule-of-Four overrides; Iron-Fist CJ bypasses;
> Jurisprudence = unlimited; Passive = cannot refuse) **must SHARE #217's Passive/Jurisprudence/Iron-Fist
> trait reads** → E25; **#219** (each influential US Rep carries `floor(stateReps/influentialReps)` of the
> apportioned seats, remainder to the highest-Legislative rep) → E14/E7. debt #87; game-mechanics §27.1.1.
> **(5) ★ SMALL FOLD-INS #222–#227 (debt #89; several `GM⇒App`) → S/XS into existing clusters.** **#222**
> committee-size scales with state count (3→11 at 50; S; → E7/E14, `phaseRunners.ts:1894-1928` is a fixed
> 4-committee sort) · **#223** chair-selection 25%→enthusiasm (XS; → E7/E16, hooks #18/#51/#124) · **#224**
> lowest-satisfaction 25% auto-flip (S; **DISTINCT from conversion #76** — no auto-flip in `src/`; → the
> realignment/conversion cluster E9/#108) · **#225** Celebrity expiry if a pol runs before 40 (XS; → E18/draft,
> reconcile with the `summer2021` Celebrity/Military-Leader merge) · **#226** Frail/Hale lifespan auto-traits
> (died<60⇒Frail, lived86+⇒Hale; XS; **a NEW `scripts/` DATASET rule, NOT engine** — Frail/Hale are runtime-only
> via battle wounds → folds into the #120/E18d dataset umbrella + `scripts/seedDataset.mjs`) · **#227**
> Trim-Roster phase REMOVED (XS verify-only; **already aligned** — no Trim-Roster in `phases.ts`/`src/`; ensure
> the `summer2021` random-retirement tuning prunes — → the death/retirement cluster). debt #89; game-mechanics
> §30.20.
> **(6) ★ DH-76 / #115 SHARPENING (debt #90).** **DH-76** (filibuster-without-cloture) now has its ORIGIN — it
> was FIRST authored in `summer2021` (the batch-26 run's changelog); the batch-29 `wilsons1916#P349` finding
> was the downstream symptom. No re-size — the cloture build stays debt #80/#208 (E9 9b once K5 lands).
...
> 1/9/20/20 tiering + generate-on-vacancy→lowest-score backfill** → folds into the `scenarioBoot`/retro-gen
> cluster (K4, §9.1.9; overlaps #74's player-facing generate). S-M. debt #90.
> **(7) ★★ GM-REPLACEMENT LENS — the PV-electability + implementation/governing additions (a VIEW, no
> re-sequence).** New ① referee/calculator pointers: the **PV-electability** row (#214/#215/#216/DH-77 — the
...
> lens stays a VIEW — adding pointers re-sequenced nothing.
> **(8) ★ HYGIENE — do NOT schedule (logged so a roadmap pass doesn't re-add):** the `summer2021`
> **winning-party lowest-faction score penalty** (ADOPTED-then-REVERSED, Ted's "nosedive"; do NOT reintroduce
> — #68/#102), the **randomized/Legis-weighted proposer** (REJECTED, committee system kept), and the
> `revampPV` per-trait **"absence bonus"** idea (points for LACKING a trait — the additive model can't express
> it; P37-39). **★ CORPUS-COVERAGE:** ≥7 forum playtests named in `presidents` are NOT ingested — highest-value
...
> trait-EFFECTS work. **★ NO gap closed, NO new keystone, NO re-sequence; top of queue UNCHANGED (QW0 → K0/K2
> → K3/K4 + scenarioBoot → E1) — the PV overhaul is a TUNING/DATA change, not a keystone: ship #216 + DH-77 as
> quick-wins now, treat #214/#215 as a balance epic gated on the un-ingested trait-EFFECTS thread + a same-PR
> `calcStateVote` re-tune.** debt #85-#90; technical-guide §9 batch-30 lead + §9.6 + §8 debt #85-#90;
> game-mechanics §3.4.1, §20.10.1, §22.7.z, §27.1.1, §30.20.
>
> **★★★★ Batch-29 version — TWO TIMELINE-EDGE PLAYTESTS THAT BRACKET THE BAND MODEL END-TO-END:
> `wilsons1916` (`a0b7ef49`, "Wilson's Vision: 1916, Era of Hollywood" — the FIRST multiplayer 1916,
> a deliberate START-AT-ERA-OPENING experiment fixing the 1960 mid-cycle failure #186; ran 1916→1920
> then trailed off) + `welcome2future` (`c54b7a9d`, "Welcome to the Future: A Hands-Off, All-CPU
> Playthrough 2022→2072" — the DOWNSTREAM-MOST ingest in the corpus, 8-of-10 factions on CPU rules,
...
> top-of-queue UNCHANGED (QW0 → K0/K2 → K3/K4 + scenarioBoot → E1).** 8 NEW gap rows #206–#213 + 3 NEW
> design-hole bugs DH-74–DH-76 + reframes (#92 terminal edge, DH-14, #115, #186, #106, #114/#110).
> **The value:**
> **(1) ★★★ HEADLINE → #206 the `Era` enum is DOUBLY too coarse, confirmed from BOTH timeline edges
> (debt #78; ties debt #5 + #92/§9.1.5) — and it RESOLVES the long-open "grow the enum vs. adopt the
> #92 data-model" question IN FAVOR of the DATA-MODEL.** Re-verified: `Era = independence|federalism|
> nationalism|modern` (`types.ts:1337`) — NO `future`/`populism`/`progressive`/`gilded`; the forum's
...
> Progressive ALL needed, a flat enum would balloon → **adopt the #92 two-level content-band model in
> K4's era-content registry; band labels become era-content DATA, not enum values; keep `Era` (if kept
> at all) as a COARSE rules-tuning key.** **★ This is NOT a new keystone — it is the most important
> sentence to add to K3/K4 + #92 this batch; annotate those rows, no re-sequence.** PLUS a separate
> **Era-of-the-Future proposal-library authoring epic** (events reportedly done — the missing piece is
> legislation, the same "events-before-legislation" shape as the Federalism gap). M for the enum/
> registry refactor (folds into K4) + a separate content epic. `GM⇒App`. debt #78; game-mechanics §22.11.
> **(2) ★★ #207 NO terminal game-over / completion state (debt #79; pairs debt #70) → a COMPLETION-vs-
...
> event's `triggersGameEnd`); `phases.ts` predicates are pure `year % N` with NO `endYear`/`maxYear` —
...
> Small but architecturally load-bearing — it defines what "done" means for the whole campaign. S-M,
> decision-gated on which completion model. debt #79; game-mechanics §26.4.
> **(3) ★★ #208 + DH-76 the CPU LEGISLATIVE/CONFIRMATION engine + cloture (debt #80; `GM⇒App`, the
> LEGISLATIVE HALF of the GM referee job) → folds into E9 handlers 9b/9d, K5-DEPENDENT.** `grep
> filibuster|cloture|nuclear.?option|confirmThreshold src/` = ZERO; the legislative loop is committee
> (`phaseRunners.ts:3463`) → floor (`:3498`) with NO filibuster step between, and the CPU faction loop
...
> clicks, iron-fist within-1); (c) cross-party cabinet caps (≤1 in top-4, ≤3 total, ≤1 ideology step,
...
> deterministic (NOT a roll). M; folds into E9 9b + 9d once K5 lands. debt #80; game-mechanics §28.10.
> **(4) ★ DH-74 validate the bill-proposal list BEFORE the vote (debt #81; `GM⇒App`, the Progressive
> face of DH-60 for LEGISLATION) → a `billProposable` predicate that SHARES the territory/era content-
...
> legis-active; Teller Amendment blocks Cuba; war "not recent" blocks pensions, `P303`) → a 2.6 HALT +
...
> #5/#78. debt #81; game-mechanics §12.1.1.
> **(5) ★ #209 boot-time retro-generation of prior-cohort career-track bench (debt #83; `GM⇒App`) →
> folds into the existing `scenarioBoot`/`BootSheet` pipeline (K4, §9.1.9).** No boot abstraction exists;
> the only generator is the FORWARD draft-rookie fallback (`phaseRunners.ts:177-209`, raw `Math.random`
> `:188-198`), so a non-1772 boot has no synthetic bench — `welcome2future` Alabama had no eligible
...
> thins).** debt #83; game-mechanics §9.2.1.
> **(6) ★ DH-75 military-appointment eligibility: citizenship + exp-type→branch (debt #82; `GM⇒App`).**
> The GeneralInChief appointment filters ONLY on `!p.currentOffice && p.skills.military >= 3`
> (`phaseRunners.ts:2256-2261`) — no citizenship gate, no Army↔Navy matching, so `wilsons1916#P162-169`
...
> #25/#31 + the 1916-bare-boot officer corps (debt #68). debt #82; game-mechanics §8.5.4.
> **(7) ★ SMALLER MECHANIC ASKS (debt #84):** **#210** Ex-President/Ex-Nominee super-Kingmaker career
> state (office-ineligible, proteges from ANY state; S, pairs #30); **#211** cross-party PV-ordered
> snake-draft MODE vs the within-party `runPhase_2_1_1_Draft` (`phaseRunners.ts:107`; S, LOW priority —
> MP setup flavor); **#212** the unbuilt 2.1.5 ideology-shift pass (`GM⇒App` — 40-60+ shifts/faction/term
...
> DISABLE-able at a boundary (2026), consolidating with #171 (XS-S). debt #84; game-mechanics §4.4, §30.19.
> **(8) ★ REFRAMES (annotate, no re-sequence):** **★ DH-14 is a CPU-AI ARTIFACT, NOT a hard block** — the
...
> ideology-weighting artifact → folds into the era-keyed bill-ideology table (debt #5-adjacent — move Mods
...
> the load-bearing CPU-AI requirement (`welcome2future` = the most complete modern CPU-turn sample) —
> reinforces E9/K5. **★ #173/#186 METHODOLOGY CONVERGENCE — era-band OPENINGS, not mid/tail:** both
...
> `welcome2future` confirms the INVERSE — a 2022 TAIL-of-band start has thin scripted-event density,
> *"only one scripted event capable of occurring right now"*, `post 378`), so BOTH era edges are thin-
> content friction zones → the #173 14-band era-start presets should snap New-Game starts to band
...
> per-year BootSheet** — the 1916 BootSheet is "bare" [NO starting generals/admirals, "filler" credit/debt
> numbers, `wilsons1916#P149-157,340-345`] → DH-75 + #209.)
> **★ This is the FIRST batch read through the GM-replacement lens to add LEGISLATIVE-REFEREE items:** the
> new ① pointers are the #208/DH-76 cloture/filibuster + CPU vote/cabinet-cap engine and the DH-74 bill-
> validator, plus DH-75 (officer-eligibility); DH-14 is logged as a ② opponent-half CPU-AI artifact. **NO
> gap closed, NO new keystone, NO re-sequence; top of queue UNCHANGED — but #206 is the strongest single
...
> engine work.** debt #78-#84; technical-guide §9 batch-29 lead + §9.6 + §8 debt #78-#84; game-mechanics
> §22.11, §26.4, §28.10, §12.1.1, §9.2.1, §8.5.4, §4.4, §30.19.
>
...
> 1960→1962-64, then **"Temporarily Paused" and never resumed solo** [the modern-angle
...
> built modern gaps [#185/#183/#186/#187] + ONE premise-only loss [#188→#88]; NO new
> keystone, NO re-sequence, top-of-queue UNCHANGED — but the #184/DH-72 fix yields a
> genuine QUICK-WIN and the modern primary→convention cluster is now the KB's richest
> forward-played election spec. `redbutton` (`8bc0231c`; the whole platform→meter→
...
> in-thread). **The value:**
> **(1) ★★ DH-72 ELECTION-TUNING SUB-FIXES → SPLIT into a SHIPPABLE QUICK-WIN; the
> highest-leverage work in the batch. Bind at `calcStateVote` (`phaseRunners.ts:3709-
> 3711`); XS–S.** The 492-40 came from a `+1`-rule read as `+3` board-wide (DH-72) on
> top of a bogus `+3` platform bonus; recompute → 284-251 → ~281-129 near-tie (`#POST
...
> traitBonus + (Math.random()-0.5)*8` — NO ±3 cap, NO per-state ideology gate, **raw
...
> sub-fixes bind HERE with no new system: **(a) narrow + SEED the jitter** (the d6→d3
> intent — compress the luck band — routed through `src/rng.ts`, ALSO closing a
...
> vs "go-crazy"); **(c) HIDE the raw formulas in the UI** (anti-collusion — show only
> which ideologies like a position + polls/maps). **★ JOIN these to the QUICK-WIN bucket
> alongside the queued QW3 ±3 cap [debt #1] + the #18 2-layer scorer** — the cheapest
> high-value election work in the corpus. debt #64 (DH-72); technical-guide §9 batch-27
> lead (a) + §9.6; game-mechanics §15.3.5, §28.6.
> **(2) ★★ #184 PLATFORM METER-CAP (+2/+1/−1) → a BUILD-INTO CONSTRAINT on the
...
> 540`). But the cap clamps a term that DOESN'T EXIST yet — `grep platform|plank in
...
> day-one constraint on the platform scorer: build the scorer with the +2/+1/−1 cap +
> hide-the-math; never ship the uncapped forum scorer. debt #65; game-mechanics
> §15.3.5, §28.6.
> **(3) ★★ #185 MODERN PRIMARY→BROKERED-CONVENTION MACHINE → FOLDS INTO Phase-1 #10
...
> primaries-in-GROUPS (debate ±1 carry, Orator +2/group), the 5-tier Cat-1–5
...
> debt #66; game-mechanics §28.6.
> **(4) ★ #183 ENDORSEMENT-MOMENTUM HOLE → FOLDS INTO #185/Phase-1 #10; S; DO NOT build
...
> ideology-penalty waiver) INDEPENDENT of delegate count — but the d6 base leaves little
> headroom (couples to the DH-72 d6→d3 squeeze) + pairs with the ballot-2 80/20
> delegate-release hole (DH-57). CONFIRMED unbuilt (`grep withdraw|endorse|momentum in
> src/engine` = ZERO). debt #67; game-mechanics §28.6.
> **(5) ★★ #186 DETERMINISTIC LATE/MID-CYCLE BOOT → a FIFTH `BootSheet` shape; RIDES
...
> 1960-62 GOVERNANCE turn forced into an Elections-Phase start a turn early → cascading
...
> weights hand-assigned, DC EVs invalid pre-1961, NO Command runway → roster frozen).
...
> in src/` = ZERO). debt #68; game-mechanics §28.1, §28.6.
> **(6) ★ #187 PARTY-LEADER FATIGUE (6-yr decay → ~8-yr presidential rotation) → S;
...
> written but NEVER read for a penalty). debt #69; game-mechanics §8.5.3, §28.1.
> **(7) ★ #188 "BIG RED BUTTON" NUCLEAR LOSS → FOLDS INTO #88 (meter-floor game-end +
...
> ROW on the #88 model, NOT a MAD/arms-race engine (corroborates the §28.2 negative-scope
...
> game-mechanics §28.2, §30.17.
> **(8) ★★ ESCALATIONS IN PLACE (confidence ↑, no new items, NO re-sequence):** **#88**
> gains its first MODERN datum (corpus still shows exactly ONE proven game-over,
> `terror2000`'s coup → confidence ↑ on the meter-floor model); **#86/#115/#173/E1** boot
> cluster — the mid-cycle modern boot is now CONCRETE (the 5th `BootSheet` shape with
...
> and never resumed solo — the CPU-AI cluster E9/K5 carries this too). **Decision-gated /
...
> DH-72 tuning sub-fixes JOIN the quick-win bucket and the modern primary→convention
> cluster (#185/#183 + the #184 platform cap) is now the KB's richest forward-played
> election spec inside Phase-1 #10.**
>
> **★★★★★ Batch-26 version — THE CORPUS'S LARGEST SOURCE (`fe15db25` "Summer 2021 AMPU
> Playtest") + the FIRST run to PLAY FEDERALISM FORWARD (1789→~1810, the whole president/cabinet/
...
> keystone, NO re-sequence, NO keystone moves, top-of-queue UNCHANGED. ★ The marquee is the
> HIGHEST-CONFIDENCE federalism-is-the-next-era signal in the corpus: everything in the federalism
> spine had to be HAND-ROLLED because the build deliberately refuses to run it — so #177/#178 escalate
...
> over ~14 months — trails off, NOT a clean burnout quit] but the marquee rulings are designer-blessed
> in-thread [@vcczar/@MrPotatoTed adjudicate]; 1772→~1810, the first source to run the federalism turn
> loop forward). **The value:**
> **(1) ★ #179 CABINET LINGERING-ROLL METER ENGINE → A REDESIGN of the shipped deterministic drift +
> ≥5 sub-systems. FOLDS INTO E6 (Lingering) + E16 (cabinet) + the §10.4.1 blunder layer. Size L; ★
> SPLIT IT.** The federalism counterpart to the shipped §11.1 lingering: each lingering phase EVERY
> cabinet member ROLLS to move their portfolio meter ±1, and it can BACKFIRE (Treasury rolled badly →
> EconStab → Great Recession; War+Navy both succeeded → MilPrep maxed). **★ Shipped-state CONFIRMED
...
> drift into a roll engine PLUS ≥6 sub-rules spanning ≥2 systems: (a) per-portfolio ±roll w/ backfire +
> Efficient=auto-+2-if-it-moves; (b) Pliable-Pres → Speaker/PPT/Key-Advisor decider override incl.
...
> rolls. **★ SPLIT (tech-lead): (i)+(f) meter/blunder engine · (b)+(d) appointment/confirmation · (c)
> bill-coupling · (e) election-feedback** — different dependencies. **Gated on #177 (federalism era
> existing) + the diplomacy/EconStab meters it moves; couples to #67 meter ladders + #116/#160 EconStab
> cascade.** debt #59; game-mechanics §20.10; technical-guide §9 batch-26 lead (b) + §9.6.
> **(2) ★ #182 command = the President's PER-TURN EXECUTIVE-ACTION BUDGET → FOLDS INTO the §14
> executive-action build (K2 ActionRegistry) + #179. Size S once the exec system is real.** Verbatim:
> "Osgood can do up to 4 actions each turn because he has 4 command" — a President's `command` = his
> executive-action count/turn (0 = inert, N = N actions), each implemented by the relevant cabinet
> member with a blunder roll (#179(f)). **★ Shipped-state CONFIRMED unbuilt:** `command` today is ONLY
> a PV input (`pv.ts:74`), kingmaker reward, grant, decay stat, and candidacy gate — NEVER an
...
> **GATED on the §14 executive-action system becoming a real K2 ActionRegistry chooser** (today a
> placeholder). **Open Q (designer): does the same budget apply to governors/other offices?** debt #62;
> game-mechanics §20.11(2).
> **(3) ★ #181 PRESIDENTIAL PLATFORM-PROMISE CONVENTION SYSTEM → PART OF the #19/#20 convention cluster;
...
> for endorsements), wired kept/broken → §22.2 enthusiasm/points. **★ Shipped-state CONFIRMED unbuilt:**
> no platform object exists (`grep platform|promiseBill|conventionPlatform in src/` = ZERO; convention
> machinery itself is designed-not-built, §15.3 / #19/#20). **Rides the #19/#20 convention epic; gated
> on conventions existing.** debt #61; game-mechanics §20.14(1).
> **(4) ★ #180 MONUMENT / LEGACY-TRAIT ACHIEVEMENT LAYER → RIDES the federalism/founding era-content
> build. Size S–M, LOWER PRIORITY than #177-#179.** The founding era awards named historical-legacy
> traits to whoever performs each in-game FIRST ("Atlas of the Revolution" = 1st Senior General;
> "Author of the Declaration" + Celebrity; "Father of the Country" = final RevWar victory; etc.). **★
...
> the authors) — needs authoring first. **Rides the era-content build; gated on #177.** debt #60;
> game-mechanics §20.14(2).
> **(5) ★ DH-71 FACTION-HANDOVER DUPLICATE-POLITICIAN → a SINGLE-OWNERSHIP CONSTRAINT on the (unbuilt)
> #1 faction-handover, NOT a shipped regression. Size XS.** A mid-thread faction "steal" moved David
> Brearley to a new faction WITHOUT deleting the original → two identical records → a later
> governor-nomination pulled the WRONG one. A transfer must MOVE, not COPY. **★ Shipped-state CONFIRMED
> — the faction-handover feature is itself UNBUILT:** `grep handover|transfer|steal|reassign in src/` =
...
> (a MOVE), never clone — and the owned-state model removes the manual-ledger drift this run exposed.
> **Folds into #1 faction-handover.** debt #63; game-mechanics §20.15.1.
> **(6) ★★ #177/#178/#114 ESCALATED IN PLACE (justification, NOT new scope, NOT a re-sequence):** **#177
> is now DESIGNED-AND-PLAYED-FORWARD, not just designed-and-walled** — `summer2021` is the FIRST source
> to run the whole federalism turn loop forward (cabinet appointment → era/decision events →
> cabinet-driven lingering → diplomacy → legislation+veto+override → presidential action, §20.9),
> proving the walled content is the *actual gameplay* of the next era; the TWO `eraGraph.ts` walls
> (`:153-155` chartIndex≥49 + `:150-152` pre-1789 president/cabinet decider ban → gate on `year >= 1789`)
> + per-power diplomacy-relation meters are CONFIRMED unchanged; **the highest-confidence "federalism is
> the next-era build" signal in the corpus.** **#178** now has LIVE FORWARD-PLAY evidence — the Treaty
...
> CEDE OHIO (the same bounded-retry budget as #178/DH-61, §20.13(5)). **#114/E9/K5/DH-36 re-confirmed
> from the MULTIPLAYER end:** the CPU is mechanically WEAK at moving meters (Ted's meta-math, POST 2671:
> ~50 all-CPU governors × ~3 actions × ~20% only move a crisis meter ~1-2 spaces), so a crisis the CPU
> can't dent must be soluble by a single engaged human → the quantitative root of "solo human is the
...
> **Decision-gated / parking-lot RECOUNT: 0** (the #179-split, #181-rides-convention, #178 ≤3-fires
> canonicity, #182-applies-to-governors are gated WITHIN their own work, not new top-level decisions).
> **NO new keystone, NO re-sequence; top of queue UNCHANGED (QW0 → K0/K2 → K3/K4 + scenarioBoot → E1) —
...
- **Batch-33 ingestion (knowledge milestone — FIVE DESIGN/DISCUSSION/BUILD threads, NO playtest; the RECONCILE/
  ...
  `revampPV`, read as ONE PV spec — **UNBLOCKS #214/#215/#220** + re-scopes DH-77), **`legisexecgov`** (518fb253,
  the **#221 3-PRIMITIVE content-authoring source** — Legis-Prop / Pres-Action / Gov-Action, era-keyed,
  meter-effecting), **`ampuelections`** (92f0659b, the elections content doc, ~70 primitives — **ORIGIN of NEW
  #241** variable term-length + **#242** amendable articles), **`housepoll`** (aed35c6e, the focus-reps-vs-every-
  seat poll — **RESOLVES #219**, fully **AUTHORS #191**), **`excelautomate`** (79fbbb46, the spreadsheet GM-
  automation progress log — charter corroboration + **ORIGIN of NEW DH-78**). **3 NEW gap rows #241/#242 + DH-78**
  (max gap #242, max bug DH-78) + a RECONCILE-HEAVY harvest. **★★★ HEADLINE A → REMOVE `pv*0.1` FROM ELECTIONS
  (debt #85; XS quick-win; `GM⇒App`; SUPERSEDES the batch-30/31 "re-tune `pv*0.1`" note).** The designer + room
  RULED **PV must NOT feed elections** (double-count — the traits/abilities are in BOTH PV and the election roll;
  `groupthinkpv` POST 36-38, a highest-PV election-bonus proposal REJECTED) → **DELETE `+ pv * 0.1` at
  `phaseRunners.ts:3709`** (not re-tune). Pairs with QW19's `Math.random`→`rng.ts` fix at the SAME line.
  **★ Tech-lead judgment-call: the 2.9.1 PRIMARY scorer's raw `pvCache` (`:3733`) is KEPT** (candidate
  *selection* = the endorsed AI-utility use; remove ONLY `:3709`). **→ `pv*0.1`-removal JOINS the cheap-fixes
  lane** (new XS quick-win pairing QW19/QW22/QW23). **★★★ HEADLINE B → THE PV EPIC IS NOW BUILDABLE END-TO-END —
  #214 + #215 + #220 UNBLOCKED (debt #85; moves GATED → READY-AFTER-#216).** The "trait-EFFECTS thread is
  un-ingested" gate is LIFTED — `groupthinkpv` IS that thread. Direction RULED: **Command×15 · Legis/Gov/Admin×10
  · Mil/Jud×5** per level (+ expertise +5); display **0–100, 50=average**; the **trait-EFFECTS-in-PV layer is 0%
  built** (verified `pv.ts` has NO trait→office logic). Sequence: **#216 remap FIRST (QW22)** → #214 tiers (M-L)
  + #215 curve (M) + #220 0–100/floor-offset (XS+S) + remove `pv*0.1` (XS) + BUILD the trait-EFFECTS-in-PV layer
  (M-L). **Only open human-call: office-magnitude ↔ 0–100 scale. Move the PV epic gated → ready-after-#216.**
  **★★ #221 THE 3-PRIMITIVE CONTENT-MODEL REGISTRY** (L, downstream of K4; `GM⇒App`) — `legisexecgov` delivers
  the authoring source (Legis-Prop / Pres-Action / Gov-Action + scripted events, era-gated, importance-curated,
  prereq-chained); VERIFIED 0% shipped (`grep -riE 'presAction|legisProp|govAction|scriptedEvent' src/` = ZERO);
  **#236/#237 depend on it**; stays where it is (downstream of K4), now WITH its source. **★ NEW BACKLOG: #242
  AMENDABLE `ConstitutionalArticles`** (M, gated #221 — Amendment-rewrites-an-article on 75%-state ratification +
  suffrage/EC-toggle/houseSizeCap/term-length fields; `types.ts:1389-1397` written ONCE today) + **#241 VARIABLE
  TERM LENGTH** (M, → K3's `year%4` predicate-replacement — a 6-/2-yr Amendment would break every predicate; the
  SECOND reason to replace year-predicates after #92; cost shared with K3, ties #92) + **DH-78 two-trait AND-combo
  field** (XS-S, gated #216 — `TraitElectionRule` `types.ts:727` can't express Celebrity AND Military Leader → +1;
  add `requiresAlsoHas: Trait[]` + a check in `electionEffects.ts:16`). **★ THE HOUSE MODEL — #219 RESOLVED +
  #191 FULLY AUTHORED** (S-M; debt #87/#73; `GM⇒App`): `housepoll` VOTED to KEEP focus-reps (every-435 REJECTED →
  AMPU-2 parking); **#219** = US Reps=EVs−2 → EV-derived 1-10 focus-rep ladder (SUPERSEDES #55's flat `(EV−2)/5`)
  → vote power=`floor(stateUSReps/focusReps)`, remainder→highest-Legislative rep; **#191** = the per-seat
  `deviantPartyBias` + minority-seat guarantee (5+ focus reps→Bias+2) + 25/25/50 new-seat rolls (era-gated growth
  lobbies); folds into #34/#55/#73/#74. **Mark #219 RESOLVED + #191 AUTHORED.** **★ SHARPENING:** #20 Gov Actions
  are FLAT/state-agnostic today (`legisexecgov`) — sharpen the #20 umbrella (debt #74); **DH-77 RE-SCOPED** —
  worst trait = Incompetent −20 (the `lackey −53` was a spreadsheet TYPO, already correct in `pv.ts`); the
  floor-OFFSET fix stands. **★ GM-REPLACEMENT LENS additions** (a VIEW, no re-sequence): ① referee gains
  PV-as-AI-utility (PV is the ② CPU's draft/persuasion metric, NOT an election input — the `pv*0.1` removal) +
  the #221 content-primitive registry + the House model; the **`excelautomate` ">30 min/phase ⇒ automate"
  charter budget** is the quantified ① mandate. **★ CORPUS-COVERAGE: "Groupthinking the Political Value" is now
  INGESTED** — removed from the still-needed list; un-ingested set = **1840-start + matthewyoung123-1772**.
  **NO gap closed (5 RECONCILED), NO new keystone, NO re-sequence, top-of-queue UNCHANGED (QW0 → K0/K2 → K3/K4 +
  scenarioBoot → E1) — the ONE delta is `pv*0.1`-removal in the cheap-fixes lane + the PV epic gated →
  ready-after-#216. Within-batch order: remove `pv*0.1` + #216 remap (cheap-fixes) → PV epic → #221 → #242 + #241
  (inside K3) → House model → DH-78.** debt #85/#87/#97-#98/#101-#103; technical-guide §9 batch-33 lead + §9.6 +
  §8 debt #101-#103 + updated #85/#87/#73; game-mechanics §3.4.1, §15.1, §29.5, §2.3, §14.2, §14.1.3, §18.1,
  §30.23.
- **Batch-32 ingestion (knowledge milestone — FIVE MIXED DATA/DESIGN/CHANGELOG threads, NO playtest [1 founding
  solo + 4 data/design/changelog]).** Absorbed **`tomorrowlist`** (vcczar's to-do + crowdsourcing call, Feb 2022 —
  the ORIGIN of the alternate-government-form axis), **`businesslabor`** (vcczar AUTHORS the Business/Labor policy
  GENRE — the origin of the stateful policy-genre framework), **`histpres`** (vcczar's 47-real-president AMPU
  statlines Washington→Biden + the curated-pol record SCHEMA), **`georgemartha`** (a 1772 SOLO — the gender/
  suffrage model; the ~9th 1772 source), **`calsleaderboard`** (a ranking-record companion to `summer2021`).
  **5 NEW gap rows #236–#240** (max gap #240) + **NO new design-hole bug** (max bug DH-77). Mostly NEW
  designed-but-unbuilt SURFACES + one DATASET-fill action — almost nothing is shipped; the strongest
  CONTENT-AUTHORING batch since batch 30/31. **★★ HEADLINE / MOST ACTIONABLE → #240 THE `CURATED_ROWS` DATASET
  FILL (debt #100; a STANDALONE author/DATA task OFF the engine track; `GM⇒App`).** **★ CORRECTED FACT (a
  prior-role error is FIXED): `CURATED_ROWS` is NOT empty** — `seedDataset.mjs:275` `= []` is only the INITIALIZER;
  the loop at `:276-289` builds it to **143 entries at runtime** from the `ROWS` source array (`:38-233`)
  (runtime-verified; `ERA_FIGURES` = 23). `ROWS` holds the **1772+1856 marquee roster, NOT the 47 presidents**, so
  **#240 = "ADD ~47 president statlines to the `ROWS` source array"** (folded into `CURATED_ROWS` as same-named
  OVERRIDES, then regenerate the dataset) — NOT "fill an empty array." **★ GATED on the #216 trait-name remap
  (= QW22)** — 14/50 `histpres` trait names mismatch the shipped `Trait` union (`Charisma`→`Charismatic` + 13 with
  no analog); run the remap first or ~14 trait keys mis-key. The Misc/Demographics/Dynasty lines wait on the #99
  data-model cluster (schema half). **★ HUMAN-CALL: trust the 2021-vintage raw skills vs re-derive under the
  #214/#215 PV curve** (every president maxes a single ability at 3, no 4s/5s, per #204). **S-M; standalone once
  QW22 lands.** **★★ #236 ALTERNATE-GOVERNMENT-FORM SCENARIO AXIS** (debt #97; `GM⇒App`, referee half) — Fascist/
  Communist/Theocratic USA the country transitions INTO, each with its own Pres-Action/Legis-Prop/Scripted-Event
  menu + pool + a reusable "authoritarian kit"; Theocracy ×5 by denomination; VERIFIED wholly unbuilt (only a
  `Theocrats` lobby STRING `factions1856.ts:28`); **depends on the unbuilt #221 content-primitive registry + the
  #239 religion field (Theocracy) + §24 amendments**; **HUMAN-CALL on the trigger + win/loss/branch end-state
  (ties #88/#188)**; DISTINCT from #6/#354 movement-factions. **L; folds into the era-content/#221 area, NOT a new
  keystone.** **★★ #237 STATEFUL POLICY-GENRE FRAMEWORK** (debt #98; `GM⇒App`, referee half) — discrete toggleable
  policies in named GENRES (Business/Labor is ONE; Currency/Copyright siblings), each with a `*-Default` baseline +
  `L/P/G/S` prefix + an era-band tag + `Preq:` chains; reads/feeds the SHIPPED `economic` meter
  (`NationalMeters.economic`); VERIFIED no policy-flag store; **depends on #221**; couples to the SHIPPED EconStab
  economic engine; DISTINCT from interest-cards/lobbies; **HUMAN-CALL on the "no policy" default**. **M-L; folds
  into the era-content/policy area.** **★ #238/#239/#194 CURATED DATA-MODEL CLUSTER** (debt #99; `GM⇒App`) —
  `Politician.gender` + suffrage gate + spousal Key-Advisor (#238); curated-override flags `southernUnionist`/
  `canBeIndependent`/`partyFlipBudget`/`canMoveStates` + the race×religion×sex demographics triple (#239); the
  dynasty/family graph wired from `histpres`'s turnkey ~37-dynasty seed table (#194); VERIFIED absent on both
  `Politician`/`ImportedDraftee` shapes; **the religion field gates #236-Theocracy**; **the SCHEMA HALF of #240**.
  **M; folds into dataset/roster work; needs a `DB_VERSION` bump.** **★ SHARPENINGS:** #194 turnkey dynasty seed
  table; #216/#214 office-weighted-PV CONFIRMED-right but raw-command OVER-CREDITED early (the
  decreasing-Command-returns target of #215; skill-cap-3); #206/#221 Era-of-Future NON-tech content from 2 threads
  (Future DOUBLY UNBUILT); #115/#24 curated "what-if" draft-pool (women + Toussaint, ties #238). **★ GM-REPLACEMENT
  LENS additions** (a VIEW, no re-sequence): ① referee gains the government-form axis (#236) + the policy-genre
  framework (#237) as referee-content systems; the data-model cluster (#238/#239) + dataset (#240) noted as
  FOUNDATIONAL. **NO gap closed, NO new keystone, NO re-sequence, top-of-queue UNCHANGED (QW0 → K0/K2 → K3/K4 +
  scenarioBoot → E1). Within-batch order: #216 remap (QW22) → #240 core fill → #99 schema cluster → #240 full
  Misc/Demographics; #236/#237 wait on #221.** debt #97-#100; technical-guide §9 batch-32 lead + §9.6 + §8 debt
  #97-#100; game-mechanics §30.22.A, §30.22.B, §30.22.C, §30.18.E, §29.7, §24.x, §3.1, §3.4.1.
- **Batch-31 ingestion (knowledge milestone — FIVE RULES/FEEDBACK/TEST/DATA threads, NO playtest [a RULES/
  FEEDBACK/TEST batch — no playtest exercise; RECONCILE-HEAVY]).** Absorbed **`rulebook`** (`b9e4f534`, "AMPU
  Rules for Playtest" — the CANONICAL RULEBOOK, ~24 systems, designer @vcczar; the single most authoritative
  mechanics spec in the corpus, STOPS before the election phases withheld as "massive"), **`apptdeepdive`**
  (`b9028796`, "Deep Dive Feedback: Presidential Appointments & Confirmation Process" — the Sept-2022
  DESIGN-ORIGIN of the appointment/confirmation ruleset, @MrPotatoTed-led, vcczar-final), **`primary2020`**
  (`74f1da28`, "The 2020 Democratic Primary: Robust Testing Forum" — a May-2022 balance/edge-case STRESS-TEST,
  NOT played-forward), **`playtestdraft`** (`38ecfaeb`, "AMPU Playtest Draft" — a founding-era 1772/1774
  DRAFT-PROCESS record), **`bestmoments`** (`4f90149c`, "AMPU Best Playtest Moments" — a HIGHLIGHTS/design-intent
  digest). **8 NEW gap rows #228–#235** (max gap #235) + **NO new design-hole bug** (max bug DH-77). **★★★
  HEADLINE → THE CABINET/SCOTUS CONFIRMATION SUBSYSTEM is now FULLY SPECCED + WHOLLY UNBUILT (debt #91; `GM⇒App`,
  the REFEREE half) — it GROWS the EXISTING cabinet/appointments epic (debt #50, E16+E14c) from S-M to M-L; NOT a
  new keystone, NOT a duplicate row.** VERIFIED absent end-to-end: `runPhase_2_3_1_Cabinet` (`phaseRunners.ts:
  2158`) auto-picks the top scored+PV candidate (`:2187`) + logs "confirmed" (`:2197-2198`) with NO vote /
  committee / party-block / filibuster / cloture / nuclear-option / auto-confirm-gate / Top-Four / promise;
  SCOTUS the SAME (`:3667`); `grep -iE 'nuclearOption|cloture|filibuster|topFour' src/` = ZERO. The game-master
  pinned the complete **12-rule §9.3.11 procedure** from `rulebook` + `apptdeepdive`: auto-confirm gate (Admin≥3 ∧
  ¬Controversial ∧ ¬IronFist-SML ∧ ¬Top-Four) → hearing at the **#172 era-keyed threshold** → SML party-wide
  block [5 reasons] → **Puritan filibuster + Manipulation-cloture [the DH-76 half supplied]** → president's tools
  [compel / convert N=Legislative senators] → vote [Admin-1 50/50-flip = DH-23 root cause; trait→vote 75%] →
  **#199 fallout = 33/33/34 blame roll + already-Controversial→lifetime-cabinet-ban, SUPERSEDING the
  Controversial→Incompetent cascade** → SML 5-name recovery → office-vacancy cascades → the #181 promise.
  **★ Build flag:** the two "Top-Four" sets DIFFER (always-vote = State/Defense/Treasury/**War**; cross-party-cap
  = State/War-Def/**AG**/Treasury). **★ SIZING (binding): GROW debt #50 S-M → M-L and re-scope E16 to own the
  whole §9.3.11 flow — do NOT create a duplicate epic; GATED on K2 + K5.** **★ THE 15-ERA TRIGGER-YEAR TABLE**
  (`rulebook#§A POST 2`) is the authoritative band-label data **K3/K4's content-band registry consumes** —
  RECONFIRMS the #92 two-level content-band model over a growing `Era` enum (annotate K3/K4/#92; no re-size).
  **★ #182 CONFIRMED** (`rulebook#POST 30` — Command = per-half-term exec-action budget; was a single-thread
  inference) → reinforces K2 (the exec-action library is one of K2's 6 libraries; replace the flat `chance(0.5)`
  `phaseRunners.ts:3635` with a command-sized loop; S budget / M lifecycle); debt #92, `GM⇒App`. **★ #179/#134
  lingering** (flat drift `phaseRunners.ts:3266-3273` vs the canonical multi-roll pit + decay numbers,
  `rulebook#POST 20`; M) **+ #50/#51 card-assignment** (partial `Faction.*Cards` `types.ts:1298-1300` vs the
  canonical plurality + ban-table + 5-pol-floor + cascade, `rulebook#POST 10`; M) — canonical algos pinned, both
  fold into existing clusters (debt #93, `GM⇒App`). **★ NEW election deltas** (debt #94): **#228** third-party
  VOTE-DRAIN (wholly unbuilt; M; `GM⇒App`) · **#230** swing-state CAP — the `(Math.random()−0.5)*8` jitter at
  `calcStateVote` **`:3711`** can flip safe states; **pairs with QW19's `Math.random`→`rng.ts` fix at the SAME
  line** (S-M) · **#231** modern-Moderate over-performance = a balance REGRESSION CASE when #18/#51 land (not a
  system). **★ Appointment side** (debt #95; `GM⇒App` — the OPPONENT half): **#229** CPU acceptance-odds matrix +
  humans-freely-accept-except-VP (wholly unbuilt; M; distinct from #73 picker + #91 vote → E9 9d / E16) · **#199**
  33/33/34 fallout lands INSIDE #91 (XS; re-spec before #112/#73) · **#233** founding CC chair-nominate→vote→
  tiebreak (S → E1). **★ Draft + small fold-ins** (debt #96): **#211** baseline re-confirmed = FIXED within-party
  round-robin (NOT a snake) · **#232** cold-start draft needs a TIGHTER 2-ideology restriction (vs 3 mid-game) →
  scenario-boot, S · **#222** committee 3→11 scaling CORROBORATED (no re-size) · **#234 + #235 PARKING** (Fog-of-
  War hidden stats + D6 vetting). **★ GM-REPLACEMENT LENS additions** (a VIEW, no re-sequence): ① referee — the
  confirmation subsystem (#91/#172/DH-76) + the spoiler/swing-state items (#228/#230); ② opponent — the CPU
  acceptance matrix (#229) + the CC-appointment opponent half (#233). **★ RECONCILIATIONS:** #199 SUPERSEDED,
  #183 RESOLVED (specced-but-unbuilt; reconcile magnitude), #182 CONFIRMED, #172/DH-76 SHARPENED, POST-143
  designed→built CLOSURE (`bestmoments` — Cosmopolitan/Provincial/Magician/Numberfudger/Crisis-{Manager,Admin,
  Gov} SHIPPED, `Lackey` NOT); #214/#215 got 2020-primary provenance (James-Dean exhibit). **★ CORPUS-COVERAGE
  (reconfirmed):** the **1840-start** persistent-Whig ~100-yr run highest-value un-ingested + the un-ingested
  **"Groupthinking the Political Value"** trait-EFFECTS thread (gates #214/#215) + **matthewyoung123-1772** —
  ask the human for exports. **NO gap closed (3 RECONCILED), NO new keystone, NO re-sequence, top-of-queue
  UNCHANGED (QW0 → K0/K2 → K3/K4 + scenarioBoot → E1).** debt #91-#96; technical-guide §9 batch-31 lead + §9.6 +
  §8 debt #91-#96; game-mechanics §9.3.11, §27.1.2, §11.2.1, §22.4.1, §30.21.
- **Batch-30 ingestion (knowledge milestone — FIVE DESIGN/DATA/changelog threads, NO playtest [a DESIGN/DATA
  batch — no playtest exercise]).** Absorbed **`revampPV`** (`4266d56e`, the Feb-2023 PV-SYSTEM OVERHAUL —
  ...
  vcczar's tier-1 ALL-DECIDED rule-changes out of the batch-26 `summer2021` run — the ORIGIN of DH-76),
  **`ampuData`** (`853ce63f`, vcczar's per-administration 1774→2022 content INDEX — an authoring source),
  **`presidents`** (`5e00ec8e`, the cross-playtest president RECORD + corpus-coverage map), **`ampuUpdates`**
  (`970616d5`, vcczar's Apr-Jul 2021 dev changelog — the earliest `GM⇒App` charter). **14 NEW gap rows
  #214–#227 + 1 NEW design-hole bug DH-77** (max gap #227, max bug DH-77). **★★★ HEADLINE → THE PV-SYSTEM
  OVERHAUL splits into PV QUICK-WINS + a GATED BALANCE EPIC (debt #85; `GM⇒App`):** `pv.ts`'s `computePV`
  ships a PRE-REVAMP formula (flat ±4/−5 traits `pv.ts:76-77`, flat `command×10` `:74`, flat `skill×weight×4`
  `:70-73`, magic `Kingmaker +6` `:79`, `Math.max(0,…)` clamp `:88`) → **#216 the trait-name remap is the
  BLOCKING PREREQUISITE** (~17 of ~57 P33 trait line-items mis-key/drop vs the shipped `Trait` union;
  **S quick-win, DO FIRST**) + **DH-77 floor-offset clamp = XS quick-win** (swap `Math.max(0,…)` for a constant
  ...
  **same PR MUST re-tune `calcStateVote`'s `pv*0.1` (`phaseRunners.ts:3709`) — `pv=c.pvCache` feeds every
  state-vote score, so a PV reweight shifts election margins** + **#220 display-scale = a S DECISION** (raw
  "pink" vs 100-normalized, UNRESOLVED → Decision-gated (A) User-gated). **★ #217 the president/cabinet
  IMPLEMENTATION sub-engine + ~30 governing traits** (debt #86, `GM⇒App`; post-legislation success→Blunder pass
  ...
  riding #179/#182/§14.1.y, gated on federalism**; folds into E16/E6). **★ #221 the era-content registry needs
  TWO SCHEMA FIELDS** (debt #88; a scripted-vs-flavor tier flag + per-category first-active-era metadata;
  `EraEvent` `types.ts:1466-1482` lacks both → **folds into K4**; XS-S each; the authoring-side confirmation of
  the #92 two-level content-band model, content terminates at Biden/2022). **★ #218 Rule-of-Four/CJ
  case-refusal (S → E25, shares #217's trait reads) + #219 US-Rep PROPORTIONAL representation (S → E14/E7)**
  (debt #87; both ABSENT). **★ SMALL FOLD-INS #222–#227** (debt #89; several `GM⇒App`): **#222** committee-size
  scales with state count (S) · **#223** chair→25%-enthusiasm (XS) · **#224** lowest-satisfaction 25% auto-flip,
  DISTINCT from conversion #76 (S) · **#225** Celebrity-expiry-before-40 (XS) · **#226** Frail/Hale lifespan
  auto-traits = a NEW `scripts/` DATASET rule, not engine (XS → #120/E18d) · **#227** Trim-Roster already
  REMOVED, verify random-retirement tuning prunes (XS). **★ DH-76 cloture now has its ORIGIN** (first authored
  `summer2021`; batch-29 `wilsons1916` was the symptom; no re-size, debt #80/#208) + **#115 generate-a-candidate
  sharpened** (name-DB tagging + 1/9/20/20 tiering + lowest-score backfill; folds into `scenarioBoot` §9.1.9;
  debt #90, S-M). **★ GM-REPLACEMENT LENS additions** (a VIEW, no re-sequence): the PV-electability row
  (#214/#215/#216/DH-77), the implementation/governing engine (#217), a cloture/legislative annotation (#221 +
  DH-76), and #115. **★ HYGIENE — do NOT schedule:** the winning-party lowest-faction score penalty (ADOPTED-
  then-REVERSED, "nosedive", #68/#102), the REJECTED randomized/Legis-weighted proposer, the per-trait
  "absence bonus" idea. **★ CORPUS-COVERAGE:** ≥7 un-ingested playtests (esp. the **1840-start** persistent-Whig
  ~100-yr run) + the un-ingested **"Groupthinking the Political Value"** trait-EFFECTS thread (gates #214/#215)
  — recorded in the Author-before-build / corpus-coverage area; ask the human for exports. **NO gap closed, NO
  new keystone, NO re-sequence, top-of-queue UNCHANGED (QW0 → K0/K2 → K3/K4 + scenarioBoot → E1).** debt
  #85-#90; technical-guide §9 batch-30 lead + §9.6 + §8 debt #85-#90; game-mechanics §3.4.1, §20.10.1, §22.7.z,
  §27.1.1, §30.20.
- **Batch-29 ingestion (knowledge milestone — TWO TIMELINE-EDGE playtests that bracket the band model
  end-to-end).** Absorbed **`wilsons1916`** (`a0b7ef49`, "Wilson's Vision: 1916, Era of Hollywood" — the
  **FIRST multiplayer 1916**, a deliberate **START-AT-ERA-OPENING experiment** fixing the 1960 mid-cycle
  failure #186; ran 1916→1920 then trailed off; the US NEVER entered WWI [war-entry event never fired]
  and the **19th Amendment RATIFIED** in a human-majority game [reframes DH-14]) + **`welcome2future`**
  (`c54b7a9d`, "Welcome to the Future: A Hands-Off, All-CPU Playthrough 2022→2072" — the **DOWNSTREAM-MOST
  ...
  legislative phase, never entering the Future band proper). **8 NEW gap rows #206–#213 + 3 NEW design-
  hole bugs DH-74–DH-76** + reframes (#92 terminal edge, DH-14, #115, #186, #106, #114/#110). **★★★
  HEADLINE → #206 the `Era` enum is DOUBLY too coarse, confirmed from BOTH timeline edges (debt #78), and
  ...
  → adopt the #92 two-level content-band model in K4's era-content registry; **band labels become era-
  content DATA, not enum values; `Era` (if kept) stays a COARSE rules-tuning key** — annotate K3/K4/#92,
  NOT a new keystone, NO re-sequence. PLUS a separate **Era-of-the-Future proposal-library authoring epic**
  (events done, legislation a stub — the same "events-before-legislation" shape as the Federalism gap).
  **★ #207 NO terminal game-over / completion state** (debt #79; the ONLY terminal state is `gameEnded` at
  ONE event-driven site `phaseRunners.ts:2871`, NO `endYear`/`maxYear` → a **completion-vs-loss DECISION**
  is owed [year-cap / sandbox / score-out], DISTINCT from the #88/#188 meter/scripted LOSS endings; S-M,
  decision-gated). **★ #208 + DH-76 the CPU LEGISLATIVE/CONFIRMATION engine + cloture** (debt #80,
  `GM⇒App`; `grep filibuster|cloture src/` = ZERO; the hands-off 2022 turn forced the GM to invent a CPU
  filibuster-override + maj/min-leader vote rule + cross-party cabinet caps; DH-76 adds the cloture gap
  from `wilsons1916#P349` — filibuster = automatic full-term delay, no cloture path → the Filibuster
  action + an era-keyed Cloture counter + CPU override + cabinet caps, all deterministic; M, folds into
  E9 9b/9d, K5-DEPENDENT). **★ DH-74 bill-proposal validator** (debt #81, `GM⇒App`; `BILL_TEMPLATES`
  carry NO precondition fields → `wilsons1916` House-voted bills then found them INVALID mid-second-vote,
  a 2.6 HALT; a `billProposable(snap, tpl)` predicate + filter at `phaseRunners.ts:3444` that SHARES the
  territory/era content-filter with #206/#92 — build the predicate library ONCE, use it for events AND
  bills; S-M). **★ #209 boot-time retro-cohort bench** (debt #83, `GM⇒App`; a non-1772 boot has no
  synthetic bench → `welcome2future` Alabama had no eligible Republican; a boot-time retro-gen of SUB-
  FLOOR career-track cohorts stat-dated to the prior ~5 cycles, routed through `rng.ts` → folds into the
  existing `scenarioBoot`/`BootSheet` pipeline [K4]; M). **★ DH-75 military-appointment eligibility**
  (debt #82, `GM⇒App`; the officer filter checks ONLY `military >= 3` → Pancho Villa appointable as a US
  general → add citizenship + exp-type→branch gates; XS-S). **★ SMALLER ASKS** (debt #84): **#210** Ex-
  President super-Kingmaker, **#211** inter-faction PV-snake draft mode, **#212** per-term ideology-shift
  pass + cap, **#213** era-conditional faction-ideology toggle (sunsets 2026, folds into #171). **★
  REFRAMES:** DH-14 = a CPU-AI ARTIFACT (the 19th passed with humans, House 374-61) → NOT a hard block,
  folds into the era-keyed bill-ideology table + the #208 CPU vote model; #106/WWI optionality + #114/#110
  corroborated; **★ #173/#186 METHODOLOGY CONVERGENCE — author/test at era-band OPENINGS, treat mid/tail
  starts as a higher-risk class** (BOTH a mid-cycle AND a tail-of-band start are thin-content friction
  zones). **★ FIRST batch to add LEGISLATIVE-REFEREE items to the GM-replacement lens** (#208/DH-76 +
  DH-74 + DH-75 under ①; DH-14 logged as a ② opponent-half CPU-AI artifact). **NO gap closed, NO new
  keystone, NO re-sequence, top-of-queue UNCHANGED (QW0 → K0/K2 → K3/K4 + scenarioBoot → E1).** debt
  #78-#84; technical-guide §9 batch-29 lead + §9.6 + §8 debt #78-#84; game-mechanics §22.11, §26.4,
  §28.10, §12.1.1, §9.2.1, §8.5.4, §4.4, §30.19.
...
- **Batch-27 ingestion (knowledge milestone).** Absorbed **ONE playtest**: **`redbutton`**
  ...
  **started MID-CYCLE in the 1960 Elections Phase** [a turn before its authored 1960-62 governance
  boot], ran 1960→1962-64, then **"Temporarily Paused" and never resumed solo** [the modern-angle
  ...
  **WON 492-40 over JFK** off a platform/election BUG, BLESSED by both designers in-thread). **ONE
  ...
  modern gaps [#185/#183/#186/#187]** + **ONE premise-only loss [#188→#88]**. **★★ DH-72 ELECTION-TUNING
  SUB-FIXES → SPLIT into a SHIPPABLE QUICK-WIN** (the highest-leverage work in the batch): the 492-40 came
  from a `+1`-rule read as `+3` board-wide on a bogus `+3` platform bonus; recompute → ~281-129 near-tie.
  `calcStateVote` (`phaseRunners.ts:3709-3711`) re-verified UNCHANGED = `50 + baseLean*5 + partyPref*5 +
  enthusiasm*2 + pv*0.1 + factionBias + traitBonus + (Math.random()-0.5)*8` — NO ±3 cap, NO per-state
  ideology gate, RAW `Math.random`, NO "+1 most-enthusiastic-base" term in code. **Three sub-fixes bind
  HERE with no new system → QW19/QW20/QW21:** (a) narrow + SEED the jitter (d6→d3 intent via `src/rng.ts`,
  closes a determinism leak + debt #1/#3), (b) re-weight `baseLean*5`→ the +3/+5 historical-bias band
  ...
  §15.3.5, §28.6. **★ #184 PLATFORM METER-CAP (+2/+1/−1)** is NOT a quick-win — it clamps a term that
  DOESN'T EXIST yet (`grep platform|plank in src/engine` = ZERO; same unbuilt object as #181) → a
  **BUILD-INTO CONSTRAINT on the convention/platform epic (Phase-1 #10 / #181)**: build the scorer with
  ...
  §15.3.5, §28.6. **★ #185 MODERN PRIMARY→BROKERED-CONVENTION MACHINE** (12 primary states, primaries-in-
  GROUPS, 5-tier Cat-1–5 delegate-weight HOST-RIGGING lever, kingmaker +1, 67%-first-ballot, two-leaders-
  force-a-kingmaker-weighted-rule-change-vote, least-delegates-first interballot order, 100%-delegate rule
  + WALKOUT, kingmaker-weighted VP vote) → **FOLDS INTO Phase-1 #10 convention machinery**, L; the single
  biggest election subsystem. CONFIRMED unbuilt (`runPhase_2_9_1_Primaries` `:3725-3750` = flat
  PV+command*5 SORT). debt #66; game-mechanics §28.6. **★ #183 ENDORSEMENT-MOMENTUM HOLE** (withdrawal/
  endorsement is inert unless the dropper holds delegates; add a momentum value +2 Minor/+3 Major OR a
  home-state/region transfer OR an ideology-penalty waiver, INDEPENDENT of delegate count) → **FOLDS INTO
  #185/Phase-1 #10**, S, DO NOT build standalone (couples to the DH-72 d6→d3 squeeze + DH-57). CONFIRMED
  unbuilt (`grep withdraw|endorse|momentum in src/engine` = ZERO). debt #67; game-mechanics §28.6. **★
  #186 DETERMINISTIC LATE/MID-CYCLE BOOT** = a FIFTH `BootSheet` shape (the FIRST modern-era boot + the
  cleanest late-start failure — a 1960 governance boot forced into an Elections-Phase start a turn early →
  ...
  invalid pre-1961, NO Command runway → roster frozen) → **RIDES K4's `BootSheet` + `scenarioBoot`
  ...
  game-mechanics §28.1, §28.6. **★ #187 PARTY-LEADER FATIGUE** (`party preference −1` after 6yr of one
  party leader → ~8-yr presidential rotation; its absence at the mid-cycle start COMPOUNDED the #184
  landslide) → **S, pairs with #15; do the boot retro-apply WITH #186**. CONFIRMED unbuilt (`Party`
  `types.ts:1306-1311` has NO tenure field; `leadershipStartYear` `:1302` written, NEVER read for a
  penalty). debt #69; game-mechanics §8.5.3, §28.1. **★ #188 "BIG RED BUTTON" NUCLEAR LOSS** (the
  campaign's whole PREMISE never fired; Cold-War content that DID fire is event/legislation FLAVOR on the
  generic engine) → **FOLDS INTO #88 (meter-floor game-end + APOCALYPSE clock, Phase-1 #6)**, XS-S, NO new
  subsystem (a configured end-condition ROW, NOT a MAD/arms-race engine — corroborates §28.2 negative
  scope #106). CONFIRMED unbuilt (`triggersGameEnd` consumed in EXACTLY ONE place `:2871`, event-only).
  debt #70; game-mechanics §28.2, §30.17. **★★ ESCALATED IN PLACE (confidence ↑, NO new scope, NO
  re-sequence):** **#88** gains its first MODERN datum (still exactly ONE proven game-over, `terror2000`'s
  coup → confidence ↑ on the meter-floor model); **#86/#115/#173/E1** boot cluster — the mid-cycle modern
  ...
  GM-WORKLOAD DEATH from the MODERN angle (the 10-human run paused, never resumed solo — E9/K5 carries
  ...
  all open Qs are designer-gated WITHIN their epics); top-of-queue UNCHANGED — but the modern primary→
...
- **Batch-26 ingestion (knowledge milestone).** Absorbed **ONE playtest**: **`summer2021`**
  ...
  PLAY FEDERALISM FORWARD** [1772→~1810, the whole president/cabinet/diplomacy TURN LOOP run end-to-end,
  NOT just designed-and-walled]; a **3-GM relay over ~14 months** — GA-run, trails off NOT a clean
  ...
  corpus.** **★ #179 cabinet LINGERING-ROLL meter engine** → **REDESIGN** of the shipped deterministic
  drift (`runPhase_2_5_1_Lingering` `phaseRunners.ts:3260` = `drift(skill)`-keyed, NO roll/backfire)
  into a ROLL engine + ≥6 sub-rules across ≥2 systems (per-portfolio ±roll w/ backfire + Efficient
  ...
  §10.4.1 blunder layer**, L, **★ SPLIT into (i)+(f) meter/blunder engine · (b)+(d) appointment/
  ...
  shipped drift has no roll/decider-override/bill-coupling). debt #59; game-mechanics §20.10. **★ #182
  command = the President's PER-TURN EXECUTIVE-ACTION BUDGET** ("Osgood can do up to 4 actions… he has 4
  command") → folds into **the §14 executive-action build (K2 ActionRegistry) + #179**, S once the exec
  ...
  game-mechanics §20.11(2). **★ #181 presidential PLATFORM-PROMISE convention system** (binding platform
  = 1 econ + 1 domestic + 1 judicial + 1 foreign bill + 1 action promise + a crisis-resolution promise,
  PLUS deal-making office-promises, kept/broken → §22.2 enthusiasm) → **PART OF the #19/#20 convention
  cluster — DO NOT build standalone**, M; rides the convention epic; gated on conventions existing.
  CONFIRMED unbuilt (`grep platform|conventionPlatform in src/` = ZERO). debt #61; game-mechanics
  §20.14(1). **★ #180 monument / legacy-trait achievement layer** (named historical-legacy traits to
  whoever performs each in-game FIRST — "Atlas of the Revolution", "Author of the Declaration",
  "Father of the Country") → **rides the federalism/founding era-content build**, S–M, LOWER PRIORITY
  ...
  monument in src/` = ZERO). debt #60; game-mechanics §20.14(2). **★ DH-71 faction-handover
  DUPLICATE-politician** → a **SINGLE-OWNERSHIP CONSTRAINT on the unbuilt #1 faction-handover, NOT a
  shipped regression**, XS: a "steal" cloned David Brearley into two factions → a later gov-nomination
  pulled the WRONG one; the (new) transfer fn must MOVE not COPY (mutate the SINGLE `factionId`).
  CONFIRMED — the handover feature is itself unbuilt (`grep handover|transfer|steal|reassign in src/` =
  ZERO). Folds into #1. debt #63; game-mechanics §20.15.1. **★★ #177/#178/#114 ESCALATED IN PLACE
  (justification, NOT new scope, NOT a re-sequence):** #177 is now DESIGNED-AND-PLAYED-FORWARD (first
  source to run the federalism turn loop forward, §20.9) → highest-confidence next-era signal, the TWO
  ...
  battles OR permanently cede Ohio, §20.13(5)); #114/E9/K5/DH-36 re-confirmed from the MULTIPLAYER end
  (Ted's meta-math POST 2671: ~50 CPU govs × ~3 actions × ~20% only move a crisis meter ~1-2 spaces →
  solo human is the design target; the 3-GM relay re-confirms DH-36 without a clean burnout quit).
  **No new keystone, no re-sequence, batch 26 nets 0; top-of-queue UNCHANGED — but the federalism
...
§4.1.y. **★★ Batch-30 adds TWO new PV quick-wins (QW22, QW23) — the FRONT-HALF of the PV-system overhaul
...
trait-EFFECTS thread + a same-PR `calcStateVote` `pv*0.1` re-tune; #220 display-scale → Decision-gated (A)
User-gated). **QW22 #216 the trait-name remap** is the BLOCKING PREREQUISITE that unblocks #214 (~17 of ~57
forum trait line-items mis-key/drop vs the shipped `Trait` union — an explicit old→new map, **S,
independently useful, DO FIRST of the two**); **QW23 DH-77 the floor-offset clamp fix** swaps the
`Math.max(0,…)` at `pv.ts:88` for a constant floor-OFFSET so the CPU keeps ordering among sub-zero pols (XS,
standalone). Both are the cheap, no-balance-risk half of the PV overhaul; the reweight itself touches `pv.ts`
which DRIVES elections, so it is held back as a balance epic. **★★ Batch-33 adds ONE new XS quick-win (QW24) —
REMOVE `pv*0.1` FROM ELECTIONS (`phaseRunners.ts:3709`), the SINGLE highest-leverage line in the batch
(`GM⇒App`, debt #85).** The designer + room RULED that **PV must NOT feed elections** (it DOUBLE-COUNTS the
traits/abilities already in BOTH PV and the election roll — `groupthinkpv` POST 36-38, a highest-PV
election-bonus proposal REJECTED) — so this **SUPERSEDES the batch-30/31 "re-tune `pv*0.1`" note**: the term
should not be there at all. **It PAIRS WITH QW19 at the SAME `:3709/3711` line** (route the `:3711`
`Math.random` jitter through `rng.ts` in the same touch — debt #1/#71) and DECOUPLES the #214 PV reweight from
elections (so the now-unblocked PV epic can re-fit freely). **★ KEEP the 2.9.1 PRIMARY scorer's raw `pvCache`
at `:3733`** — the primary is candidate *selection* (the AI-utility use the ruling ENDORSES); remove ONLY
`:3709`. **★ With `groupthinkpv` ingested, the BACK-HALF PV epic (#214/#215/#220 + the 0%-built trait-EFFECTS-
...
**★★★ Batch-29 (the two timeline-edge threads `wilsons1916` + `welcome2future`) RESOLVES the long-open
"grow the `Era` enum vs. adopt the #92 two-level data-model" question IN FAVOR of the DATA-MODEL — a
K3/K4/#92 ANNOTATION, NOT a new keystone, NO re-sequence (§9 batch-29 lead ①, debt #78).** #206 re-
...
Future/Populism/Hollywood/Gilded/Progressive labels ALL needed, a flat enum would balloon → **the era-
content-band model belongs in DATA: K4's per-era era-content registry carries the band labels (already
its design — see the K4 row), and the `Era` enum (if kept at all) stays a COARSE rules-tuning key, NOT
the band carrier.** This was already K3/K4's specced direction (the early-republic sub-bands were already
content-band MARKERS, not enum values) — batch-29 makes it the BINDING resolution and extends it to the
...
a stub — folds into the era-content authoring, see E31 / the Author-before-build bucket). **No new
keystone, no scope change to K3/K4 beyond this annotation, top-of-queue UNCHANGED.**
**★ Batch-31 (`rulebook`, the CANONICAL RULEBOOK) — the 15-ERA TRIGGER-YEAR TABLE is the AUTHORITATIVE
band-label DATA K3/K4's content-band registry consumes; it RECONFIRMS the #92 two-level content-band model over
a growing enum — a K3/K4/#92 ANNOTATION, NOT a new keystone, NO re-sequence (§9 batch-31 lead ②, debt
corroboration on #92/#206; game-mechanics §27.1.2).** `rulebook#§A POST 2` prints the canonical **15-era
trigger-year calendar** (Independence 1774 / Federalism 1788 / Republicanism 1800 / Democracy 1820 / Manifest
Destiny 1840 / Nationalism 1856 / Gilded 1868 / Progressivism 1892 / Normalcy 1916 / Ideologies 1928 / Nuclear
Age 1948 / Neocons 1972 / Terror 2000 / Populism 2012 → Era-of-the-Future = after the current real-world cycle)
and states eras "auto-update when their trigger date is reached," driving legislation/event availability + state
populations + preferred ideologies + the statesman death rate. The shipped `Era` enum is the 4-value
`independence/federalism/nationalism/modern` (`types.ts:1337`); the **15 named bands collapse into those 4
mechanical buckets** → the table is the **band-label spec K4's per-era content-band registry keys on** (the
`{era, startYear}` / trigger-cycle map; "trigger date reached" = the EARLIEST a band opens, the transition
predicate stays game-state-gated per #92). This reconciles with the modernday 14-band map (#2) + the nuke
"Neocons = rebrand, not a point-banked band" finding via the two-level model, and re-affirms the batch-29 #206
"data-model over a growing enum" resolution. **No re-size; the `gilded`/`progressive` enum values K4 still adds
remain justified ONLY where their rule tables diverge; everything else is a band label in the registry. See the
updated K3 + K4 rows.**
**★ Batch-31 — #182 CONFIRMED reinforces K2 (the exec-action library), and the confirmation subsystem (debt
#91, E16) GATES ON K2 + K5** — both ANNOTATIONS (see the updated K2 + K5 rows + the E16 cabinet epic); NO
re-sequence, NO new keystone.
**★ Batch-32 — #221 (the content-primitive registry / era-content schema, K4) is RE-CONFIRMED as the LOAD-BEARING
prereq for the batch's two new system surfaces (#236 government-form + #237 policy-genre) — a K4/#221 ANNOTATION,
NOT a new keystone, NO re-sequence (§9 batch-32 lead ②③; debt #97/#98).** BOTH new systems' content (regime
Pres-Action/Legis-Prop/Scripted-Event menus; the policy-genre `L/P/G/S` entries) ARE per-administration content
primitives — and `grep -iE 'presAction|legisProp|govAction|scriptedEvent' src/` = ZERO, so neither #236 nor #237
can land before #221's registry exists. Separately, `tomorrowlist` + `businesslabor` RE-CONFIRM the **Era-of-the-
Future band as an actively-authored, explicitly NON-tech content frontier** (e-democracy / USCoin / drones /
lab-grown meat / AI-employee restrictions) — reinforcing the batch-29 #206 "Future DOUBLY UNBUILT" finding + the
#92 two-level content-band model K3/K4 already adopt. No scope change to any keystone; #236/#237 fold into the
...
> removes. Charter: `game-context.md` → **"★ North-star: the app must subsume the
> human GM"**; sequencing: `technical-guide.md` §9. **Top of queue is unchanged:
...
via `src/rng.ts` — never a human die-roll or a raw `Math.random`). The GM does
**two distinct jobs**, and the lens shows the GM-replacement program is the
**spine of the backlog** — the keystones + the bulk of Phase 1 exist to retire one
...
**① GM as referee / calculator** — the rules math + machinery the GM computes by
...
| **Down-ballot election scoring: per-office trait stack, expertise/lobby↔industry "Manchin" survival, per-seat gerrymander bias, max-margin→popular-vote-% caps + ★ batch-33 THE HOUSE MODEL (EV-ladder + vote-power-with-remainder + per-seat deviant bias)** *(batch-28; ★ batch-33 — `GM⇒App`)* | **#189, #190, #191, #192** (with #103, #20), **★ batch-33 #219** (RESOLVED — the EV-derived 1-10 focus-rep ladder + vote-power remainder) | **E20b / `calcStateVote`** — PARTLY-BUILT (the route already applies *some* presidential traits at `:3685,3707-3708`); debt #73, M, builds ON the existing routing. The per-seat `deviantPartyBias` (#191) is SET-BY the gerrymander gov-action (#20, debt #74). **★ BATCH-33 (`housepoll`): the House model is the ① referee half of the single most-quit-over phase** — **#219 is RESOLVED** (community KEPT focus-reps, every-435 REJECTED → AMPU-2 parking): US Reps=EVs−2 → an **EV-derived 1-10 focus-rep ladder** (SUPERSEDES #55's flat `(EV−2)/5`) → **vote power=`floor(stateUSReps/focusReps)`, remainder→highest-Legislative rep** = exactly the seat-tally math the `excelautomate` GM ranked **automation priority #1**; **#191 is FULLY AUTHORED** (`housepoll` POST 17-19 — the per-seat deviant-bias engine: minority-seat guarantee at 5+ focus reps→Bias+2, 25/25/50 new-seat rolls keyed to era-gated growth lobbies). Both ride a NEW per-seat struct; fold into #34/#55/#73/#74; S-M, not a new keystone. The focus-Rep abstraction is the deliberate ">435-seats is AMPU-2" tractability call that keeps the phase under the ">30 min/phase ⇒ automate" charter budget. |
...
| Scenario boot: year-correct incumbents, seeded delegates, back-dated careers, retro-fatigue (incl. mid-cycle / late-era shape) **+ a retro-cohort bench for thin-dataset starts** | #186, #187, #86, #115, #173, **#209** *(batch-29)* | **K4** (the `BootSheet` schema + `scenarioBoot` pipeline; #186 = a 5th `BootSheet` shape, #173 = start-year presets) + the boot validators **QW8/QW9** + #187 pairs with #15. **★ Batch-29: #209** = a boot-time retro-gen of SUB-FLOOR career-track cohorts (Ambassadors/Admirals/lower-Justices/bench-Reps, routed through `rng.ts`) for non-1772 starts whose real dataset thins — the boot prerequisite for ANY deep-modern / Future-band start; folds into this same `scenarioBoot` pipeline; debt #83. **★ Batch-30: #115 generate-a-candidate** (the candidate-pool generator the GM hand-ran when the bench thinned — `wilsons1916` hand-built "Joe Simpson (auto-generated)") is sharpened by the full `ampuUpdates` spec (name-DB demographic tagging + 1/9/20/20 tiering + generate-on-vacancy→lowest-score backfill); the forward rookie-gen path (`phaseRunners.ts:177-209`, raw `Math.random` `:188-198`) lacks all three → folds into this same `scenarioBoot`/retro-gen cluster (K4, §9.1.9; overlaps #74's player-facing generate); debt #90, S-M. |
...
the manual CPU-faction sim (②) + opaque rules (①) are what *kill* playtests
...
*because* ① and ② sit on them. **But foundational-first is already the queue's
order** — this lens explains the *why*, it does not move anything.

**`GM⇒App` tagging convention (for future ingests).** Per the charter, when a
digest records the GM doing something by hand, tag it **`GM⇒App`** and give it (or
confirm it has) a gap-log row. A `GM⇒App` item is never "designer flavor" to leave
to a human — it is an automation requirement. New `GM⇒App` gaps slot into the lens
...
> **★ Batch-29 is the FIRST batch to add LEGISLATIVE-REFEREE items to this lens.** The new ①
...
> run proved the engine doesn't own — `grep filibuster|cloture src/` = ZERO; E9 9b/9d, K5-
> dependent) + the **DH-74 `billProposable` bill-validator** (which SHARES the territory/era
> content-filter with #206/#92 + DH-60 — one predicate library for events AND bills) + **DH-75
> officer-eligibility** + **#209 retro-cohort bench** (boot row) + **#207 timeline-completion
> terminus** (end-condition row). On the ② opponent half the new entry is a *correction*:
> **DH-14 is a CPU-AI artifact, not a mechanical block** — the 19th Amendment PASSED with humans
> (House 374-61), so it folds into the era-content table (① data) + the #208 CPU vote model (②);
> `welcome2future` is logged as the corpus's most complete modern CPU-turn sample (charter
> evidence). **Adding these pointers re-sequenced nothing — top of queue is unchanged (QW0 →
> K0/K2 → K3/K4 + scenarioBoot → E1).**

> **★ Batch-30 adds the PV-ELECTABILITY + GOVERNING/IMPLEMENTATION rows to ① referee/calculator** (still a
> VIEW, no re-sequence). The new ① pointers: **PV-electability** (#214/#215/#216/DH-77 — the per-trait +
...
> — the post-legislation success→Blunder pass + ~30 governing traits the GM hand-resolved each turn); a
...
> (the candidate-pool generator the GM ran by hand when the bench thinned, on the boot row). On the ② opponent
> half there is no new entry this batch (the PV math the CPU uses sits under ① as the rules-calculator it reads;
> the candidate generator is a ① mechanic). **Adding these pointers re-sequenced nothing — top of queue is
> unchanged (QW0 → K0/K2 → K3/K4 + scenarioBoot → E1).**

> **★ Batch-31 adds BOTH halves — the CONFIRMATION/SPOILER referee (①) AND the CPU-acceptance/CC-appointment
> opponent (②) — still a VIEW, no re-sequence.** The new ① referee/calculator pointers: the **cabinet/SCOTUS
> confirmation subsystem** (#91/#172/DH-76 — the confirmation vote/block/Puritan-filibuster/Manipulation-cloture/
> 33-33-34-fallout the GM hand-adjudicated every turn, the batch headline; it GROWS the existing cabinet epic
> debt #50 from S-M to M-L and is GATED on K2+K5 — NOT a new keystone, NOT a duplicate row) and the
> **spoiler/swing-state** election items (#228 the third-party VOTE-DRAIN + #230 the safe-state cap that pairs
> with QW19's determinism fix at the shared `:3711` jitter; #231 is a balance regression case folded into the
> same election-scoring row). The new ② opponent pointers: the **CPU acceptance-odds matrix (#229)** — the
> per-candidate-type accept/decline roll that is the OPPONENT half of the appointment phase (distinct from #73
> the picker + #91 the vote) — and the **founding-CC appointment opponent half (#233)** (chair-nominates →
> delegates-vote → CC-Pres-tiebreak). **★ This batch ALSO reconciled three lens items: #199** is RE-SPEC'd
> (the 33/33/34 fallout SUPERSEDING the cascade, now landing INSIDE #91's rejection path — same lens cell, sharper
> spec); **DH-76** gains its cloture half inside #91 (the legislative-referee row already pointed to it as
> "missing"); and **#182** (Command = exec-action budget) is CONFIRMED-authoritative (it reinforces K2, a ① rules-
> calculator the exec library reads). **Adding/sharpening these pointers re-sequenced nothing — top of queue is
> unchanged (QW0 → K0/K2 → K3/K4 + scenarioBoot → E1).**

> **★ Batch-32 adds two REFEREE-CONTENT SYSTEMS to ① + a FOUNDATIONAL data/dataset pair — still a VIEW, no
> re-sequence.** The new ① referee/calculator pointers (both ride the unbuilt #221 content-primitive registry):
> the **government-form axis (#236)** — the Fascist/Communist/Theocratic regime transition + the cross-form
> authoritarian kit the GM would hand-adjudicate (L; additionally gated on the #239 religion field + §24; trigger +
> win/loss/branch end-state are a HUMAN ruling, Decision-gated) — and the **stateful policy-genre framework
> (#237)** — the Business/Labor + Currency + Copyright genres with `*-Default` baselines + `Preq:` chains that
> read/feed the SHIPPED `economic` meter, the active-policy state the GM hand-tracked (M-L; couples to the SHIPPED
> EconStab engine; the "no policy" default is a HUMAN ruling, Decision-gated). The **data-model cluster
> (#238/#239)** sits on the OPPONENT/referee boundary (the GM hand-decided who may go Independent / flip parties /
> stay Unionist → data-driven gates the CPU + engine read) and the **dataset fill (#240)** is referee
> content-authoring that removes statline math from the GM — both flagged **FOUNDATIONAL**: the **#239 religion
> field gates #236-Theocracy**, and the #99 schema cluster is the half #240's Misc/Demographics statline lines wait
> on (#240 itself is a STANDALONE author/DATA task OFF the engine track, gated only on the #216 remap = QW22).
> **Adding these pointers re-sequenced nothing — top of queue is unchanged (QW0 → K0/K2 → K3/K4 + scenarioBoot →
> E1).**

> **★ Batch-33 sharpens the ① referee half AND restates the ② opponent BOUNDARY — still a VIEW, no re-sequence.**
> The cleanest cross-batch statement yet of **PV-as-AI-utility** (the ② GM-as-opponent's metric): `groupthinkpv`
> RULED that PV exists to drive the CPU's **draft / steal / persuasion / leadership / primary-selection**
> decisions and **must NOT feed the vote** — so the ① referee/calculator does NOT use PV for elections, and the
> PV-electability row above is RE-FRAMED accordingly: **`pv*0.1` is REMOVED from `calcStateVote` (QW24,
> `phaseRunners.ts:3709`)** to enforce that boundary, while the 2.9.1 primary scorer's raw `pvCache` (`:3733`) is
> the endorsed AI-utility use and is KEPT. New ① referee/calculator pointers: the **#221 content-primitive
> registry** (`legisexecgov` — the typed Legis-Prop / Pres-Action / Gov-Action + scripted-event pool the GM
> hand-curated, with the CPU-selection draw #70-#78 reading from it; L, downstream of K4; #236/#237 ride it) +
> the **House model** (`housepoll` — #219 RESOLVED + #191 FULLY AUTHORED; the EV-ladder + vote-power-with-
> remainder + per-seat deviant bias = the seat-tally math the `excelautomate` GM ranked automation priority #1,
> with the focus-Rep abstraction the deliberate ">435-seats is AMPU-2" tractability call) + **#241/#242** (the
> referee half of constitutional adjudication — amendable articles + variable term-length the GM hand-tracked).
> **★ The `excelautomate` ">30 min/phase ⇒ automate" GM-burden budget** (game-mechanics §18.1) is the concrete,
> quantified statement of the ① mandate (the GM's own dated automation-priority order: tallies → trait-lookup →
> election-scoring → legislative-scoring), corroborating #114/DH-36 from the BUILD side — recorded as the
> charter metric, not a build row. On the ② opponent half there is no new entry (the PV math the CPU uses sits
...
**★★ Batch-29 fold-ins (TWO timeline-edge playtests — `wilsons1916` [first multiplayer 1916, a START-
AT-ERA-OPENING experiment] + `welcome2future` [hands-off all-CPU 2022→Future, the downstream-most
ingest; 8-of-10 factions on CPU rules]; 8 NEW gap rows #206–#213 + 3 NEW design-hole bugs DH-74–DH-76;
NO gap closed, NO new keystone, NO re-sequence, top-of-queue UNCHANGED. Items below are dependency-
ordered per the tech-lead's binding §9 batch-29 lead + §8 debt #78-#84):**
**★★★ THE HEADLINE → #206 the `Era` enum is DOUBLY too coarse, confirmed from BOTH timeline edges
(debt #78; ties debt #5 + #92/§9.1.5) — and it RESOLVES the long-open "grow the enum vs. adopt the #92
data-model" question IN FAVOR of the DATA-MODEL → a K3/K4/#92 ANNOTATION (NO new keystone, NO re-
sequence; see the annotated keystone-overview + K3/K4 rows above).** Re-verified at code level: `Era =
independence|federalism|nationalism|modern` (`types.ts:1337`) — NO `future`/`populism`/`progressive`/
`gilded`; `currentEra` is boot-set + advanced at a SINGLE runtime site (`constitutionalConvention.ts:198`
→ `'federalism'`); the era graph is hard-scoped to one era (`ERA_GRAPH_1772` + the `chartIndex >= 49`
THROW at `eraGraph.ts:153-155`); the only year→era map is a UI-only `eraForYear`
...
6`]). The Future band is **doubly unbuilt** (absent from the enum AND under-content'd — events done,
legislation a stub). With Future/Populism/Hollywood/Gilded/Progressive labels ALL needed, a flat enum
would balloon → **adopt the #92 two-level content-band model in K4's era-content registry; band labels
...
markers) — batch-29 makes it the BINDING resolution and extends it to the terminal edge. **PLUS a
separate Era-of-the-Future PROPOSAL-LIBRARY authoring epic** (events reportedly done — the missing piece
...
into K4); separate content epic for Future proposals.** `GM⇒App`. debt #78; game-mechanics §22.11.
**★ #207 NO terminal game-over / completion state → a COMPLETION-vs-LOSS DECISION (debt #79; pairs debt
...
`endYear`/`maxYear` — so the build has **no year-cap, no completion, no victory, no sandbox terminus**,
...
branch + a win/score screen). Small but architecturally load-bearing — it defines what "done" means for
the whole campaign. **S-M; decision-gated on which completion model** (recorded under Decision-gated).
debt #79; game-mechanics §26.4.
**★ #208 + DH-76 the CPU LEGISLATIVE/CONFIRMATION engine + cloture → FOLDS INTO E9 handlers 9b
(legislation) + 9d (cabinet), K5-DEPENDENT; the LEGISLATIVE HALF of the GM referee job, `GM⇒App`.** **★
Shipped-state CONFIRMED unbuilt:** `grep filibuster|cloture|nuclear.?option|confirmThreshold src/` =
ZERO; the legislative loop is committee (`phaseRunners.ts:3463`) → floor (`:3498`) with NO filibuster
...
ideology-clicks, iron-fist within-1); (c) cross-party cabinet caps (≤1 in top-4, ≤3 total, ≤1 ideology
...
(`phaseRunners.ts:3463-3498`) + the cabinet handler. debt #80; game-mechanics §28.10.
**★ DH-74 validate the bill-proposal list BEFORE the vote → a `billProposable` predicate that SHARES the
territory/era content-filter with #206/#92 + DH-60 (debt #81; the Progressive face of DH-60 for
LEGISLATION, `GM⇒App`).** **★ Shipped-state CONFIRMED:** `runPhase_2_6_1_Proposals`
...
`currentEra`) but never filters proposals → `wilsons1916` proposed/committee-passed/packaged/House-voted
...
only LEGAL proposals surface. **★ Build the predicate library ONCE, use it for events AND bills** (it is
the same content-filter as #206/#92 + DH-60). S-M; pairs DH-60 + debt #5/#78. debt #81; game-mechanics
§12.1.1.
**★ #209 boot-time retro-generation of prior-cohort career-track bench → FOLDS INTO the existing
`scenarioBoot`/`BootSheet` pipeline (K4, §9.1.9; debt #83, `GM⇒App`).** **★ Shipped-state CONFIRMED:** no
...
(`:188-198`, debt #2). So a non-1772 boot has no synthetic bench → `welcome2future` Alabama had no
...
Ambassadors/Admirals/lower-Justices/bench-Reps — routed through `rng.ts`, NOT the `:188` raw
...
the real dataset thins).** debt #83; game-mechanics §9.2.1.
**★ DH-75 military-appointment eligibility: citizenship + exp-type→branch → XS-S, standalone (debt #82,
`GM⇒App`).** **★ Shipped-state CONFIRMED:** the GeneralInChief appointment filters ONLY on
`!p.currentOffice && p.skills.military >= 3` (`phaseRunners.ts:2256-2261`) — no citizenship gate, no
Army↔Navy matching → `wilsons1916#P162-169` made Pancho Villa + Aguinaldo appointable as US Army
...
`phaseRunners.ts:2256-2261`. debt #82; game-mechanics §8.5.4.
**★ SMALLER MECHANIC ASKS (debt #84) — ride existing epics, LOWER priority.** **#210** Ex-President/Ex-
...
MODE vs the within-party `runPhase_2_1_1_Draft` (`phaseRunners.ts:107`; S, LOW priority — MP setup
flavor, folds into the draft work #4/#108) + **#212** the unbuilt 2.1.5 ideology-shift pass (`GM⇒App` —
...
§4.4, §30.19.
**★ REFRAMES (annotate in place, no new scope, no re-sequence):** **★ DH-14 is a CPU-AI ARTIFACT, NOT a
hard block** — the 19th Amendment ("Equal Voting Rights for Women") PASSES in the 10-human 1916 game
(House 374-61, Senate 79-17, ratified), so the `drums` all-CPU "this amendment will never pass with CPUs"
is a **CPU-AI / era-ideology-weighting artifact** → folds into the era-keyed bill-ideology table (debt
#5-adjacent — move Mods off suffrage's negative side in 1916) AND a CPU vote model (#208 / E9 9b) that
can reach 2/3 like a human field; NO standalone work (logged as a ② opponent-half artifact in the GM-
...
CPU-AI requirement (`welcome2future` = the most complete modern CPU-turn sample — direct charter evidence
that hands-off modern resolution WORKS). **#115/#186** boot cluster — both threads converge on the boot
lesson (even a clean OPENING start needs a fully-populated per-year BootSheet: the 1916 BootSheet is
"bare" — NO starting generals/admirals, "filler" credit/debt, `wilsons1916#P149-157,340-345` → DH-75 +
#209). **★ #173/#186 METHODOLOGY CONVERGENCE — era-band OPENINGS, not mid/tail:** both threads
...
the INVERSE — a 2022 TAIL-of-band start has thin scripted-event density, *"only one scripted event
capable of occurring right now"*, `post 378`), so BOTH era edges are thin-content friction zones → the
...
**Decision-gated RECOUNT: +1 (#207's completion-model pick — year-cap vs sandbox vs score-out — is a
genuine planner/designer decision, recorded under Decision-gated below).** **NO gap closed; NO new
keystone, NO re-sequence; top-of-queue UNCHANGED (QW0 → K0/K2 → K3/K4 + scenarioBoot → E1) — but #206 is
...
concrete, K5-dependent engine work.** debt #78-#84; technical-guide §9 batch-29 lead + §9.6 + §8 #78-#84;
game-mechanics §22.11, §26.4, §28.10, §12.1.1, §9.2.1, §8.5.4, §4.4, §30.19.
...
premise-only loss [#188→#88]; NO new keystone, NO re-order, NO keystone moves — but the DH-72 sub-fixes
JOIN the quick-win bucket [QW19-QW21 above] + the modern primary→convention cluster is now the KB's
richest forward-played election spec):**
**★ DH-72 / #184 PLATFORM-OVERPOWER — SPLIT into a SHIPPABLE QUICK-WIN + a BUILD-INTO CONSTRAINT.** The
hand-driven 1960 run produced a **Bricker (R) 492-40 landslide over JFK** off a `+1`-rule read as `+3`
board-wide (DH-72) ON TOP of a bogus `+3` platform bonus; recompute → 284-251 → ~281-129 near-tie
(`redbutton#POST 532, 587, 603-607`). Both designers played + agreed *"platform is way too powerful"*
(`#POST 538-540`). **★ Shipped-state CONFIRMED:** `calcStateVote` (`phaseRunners.ts:3709-3711`) is the
single election scorer = `50 + baseLean*5 + partyPref*5 + enthusiasm*2 + pv*0.1 + factionBias +
traitBonus + (Math.random()-0.5)*8` — **NO ±3 cap, NO per-state ideology gate, RAW `Math.random` (a
...
leverage work in the batch → QW19/QW20/QW21 in the quick-win bucket above:** (a) narrow + SEED the
jitter (d6→d3 intent via `src/rng.ts`, ALSO closing a determinism leak vs the rng rule + debt #1/#3),
(b) re-weight `baseLean*5` into the +3/+5 historical-bias band behind a `game.biasMode?` toggle, (c)
HIDE the raw formulas in the UI (anti-collusion — show only which ideologies like a position +
polls/maps). They pair with the queued **QW3 ±3 cap (debt #1) + the #18 2-layer scorer** — the cheapest
high-value election work in the corpus. **Binds at** `phaseRunners.ts:3697/3709-3711` + a `game.biasMode?`
toggle + a UI hide-the-math surface. debt #64 (DH-72). **(B) #184 = the +2/+1/−1 PLATFORM meter-cap is
NOT a quick-win** — it clamps a term that DOESN'T EXIST yet (`grep platform|plank in src/engine` = ZERO;
the platform engine [5 planks × per-card faction scoring × 4-step comparison] is the SAME unbuilt object
as #181, `summer2021`). So the cap is a **BUILD-INTO CONSTRAINT on the convention/platform epic (Phase-1
#10 / #181)**: build the platform scorer with the +2/+1/−1 cap + hide-the-math from day one; never ship
the uncapped forum scorer. **Binds at** the (unbuilt) #181 convention/platform object. debt #65;
game-mechanics §15.3.5, §28.6.
**★ #185 MODERN PRIMARY→BROKERED-CONVENTION MACHINE → FOLDS INTO Phase-1 #10 convention machinery
...
+2/group), the **5-tier Cat-1–5 delegate-weight HOST-RIGGING lever** (a random gov per party assigns
each state 1×–4×/bias-conditional EV; a player gerrymandered GOP weights to bury MacArthur — Ted holds
rigging is INTENDED), kingmakers-affect-conventions-not-primaries (+1), 67%-first-ballot (mutable),
**two-leaders force a KINGMAKER-WEIGHTED rule-change vote** (→50%+1/WTA/proportional), **least-delegates-
...
tie→PL picks lowest faction). **★ Shipped-state CONFIRMED unbuilt:** `runPhase_2_9_1_Primaries`
...
end) + the delegate-class fork (§29.11b). **Binds at** the (unbuilt) convention subsystem (#19/#20).
debt #66; game-mechanics §28.6.
**★ #183 ENDORSEMENT-MOMENTUM HOLE → FOLDS INTO #185/Phase-1 #10; S; DO NOT build standalone.**
...
action, INDEPENDENT of delegate count** — but the **d6 base leaves little headroom** (couples to the
DH-72 d6→d3 squeeze, QW19) + pairs with the ballot-2 80/20 delegate-release hole (DH-57). **★ Shipped-
state CONFIRMED unbuilt** (`grep withdraw|endorse|momentum in src/engine` = ZERO). **Binds at** the
(unbuilt) primary action loop inside #185. debt #67; game-mechanics §28.6.
...
1961) forced into an Elections-Phase start a turn early → cascading manual patches (Senate 50-50 vs real
66-D, no Leadership-gated Party Leader, on-election rolls skipped, party-fatigue dropped, gov terms-
served unknowable, delegate weights hand-assigned, DC EVs invalid pre-1961, NO Command runway → roster
frozen). Ted: eras are *"the stoniest"* thing → a **per-year boot TABLE is the only path** (`#POST 528`).
**★ Shipped-state CONFIRMED unbuilt:** no generic boot (`GameContext.tsx:5-6,24` → hand-authored
...
most-demanding instantiation of the SAME K4 `BootSheet` schema — it SHARPENS the bar, it does NOT add a
keystone. **Binds at** the K4 `BootSheet` schema + `scenarioBoot` pipeline (#115). debt #68; game-
mechanics §28.1, §28.6.
**★ #187 PARTY-LEADER FATIGUE (6-yr decay → ~8-yr presidential rotation) → S; pairs with #15; do the
boot retro-apply WITH #186.** A **`party preference −1` after 6yr of one party leader**, ENGINEERED to
force a ~8-yr presidential rotation (Ted, `#POST 645`); dropped here because the mid-cycle start skipped
1958 → its absence COMPOUNDED the #184 landslide (let the House flip 58R-42D unchecked). **★ Shipped-
...
substrate. debt #69; game-mechanics §8.5.3, §28.1.
**★ #188 "BIG RED BUTTON" NUCLEAR LOSS → FOLDS INTO #88 (meter-floor game-end + APOCALYPSE clock,
...
state bill, Bay of Pigs as a minority-flip lever, Test Ban Treaty as an A/B event). **★ Shipped-state
...
end-condition ROW on the #88 model, NOT a MAD/arms-race engine** — corroborates the §28.2 negative-scope
directive (#106). **Binds at** the #88 meter-floor end-condition table (`:2871`-area). debt #70; game-
mechanics §28.2, §30.17.
**★★ #88/#86/#115/#173/E1/#14/#114/DH-36 ESCALATED IN PLACE (justification, NOT new scope, NOT a
re-sequence):** **#88** gains its FIRST MODERN datum (the corpus still shows exactly ONE proven game-over
— `terror2000`'s coup; the meter-floor model now has the modern nuclear premise pointing at it →
confidence ↑); **#86/#115/#173/E1** boot cluster — the mid-cycle modern boot is now CONCRETE (a 5th
...
Paused" and never resumed solo — the CPU-AI cluster E9/K5 carries this too). **★ Raise priority/
...
top-of-queue UNCHANGED (QW0 → K0/K2 → K3/K4 + scenarioBoot → E1) — but the DH-72 tuning sub-fixes JOIN
the QUICK-WIN bucket (QW19-QW21) and the modern primary→convention cluster (#185/#183 + the #184 platform
cap) is now the KB's richest forward-played election spec inside Phase-1 #10.**
**★★ Batch-26 fold-ins (`summer2021` — THE CORPUS'S LARGEST SOURCE + the FIRST run to PLAY FEDERALISM
FORWARD [1772→~1810, the whole president/cabinet/diplomacy turn loop end-to-end], a 3-GM relay over ~14
...
**★ #179 CABINET LINGERING-ROLL METER ENGINE → A REDESIGN of the shipped deterministic drift + ≥5
sub-systems; FOLDS INTO E6 (Lingering) + E16 (cabinet) + the §10.4.1 blunder layer; L; ★ SPLIT IT.**
The federalism counterpart to the shipped §11.1 lingering: each lingering phase EVERY cabinet member
ROLLS to move their portfolio meter ±1, and it can BACKFIRE (Treasury rolled badly → EconStab → Great
Recession; War+Navy both succeeded → MilPrep maxed). **★ Shipped-state CONFIRMED unbuilt-as-designed:**
`runPhase_2_5_1_Lingering` (`phaseRunners.ts:3260`) is a DETERMINISTIC skill-keyed DRIFT (`drift(skill)`
`:3266-3273` — fixed ±delta, NO roll, NO backfire, NO Efficient/Pliable/regional hook); the cabinet
pick `runPhase_2_3_1_Cabinet` (`:2158-2224`) is a flat score-sort with NO ideology gate, NO
Speaker/PPT/advisor decider override, NO bill-coupling. So #179 is a **REDESIGN of the deterministic
drift into a roll-based engine** PLUS ≥6 sub-rules across ≥2 systems: (a) per-portfolio ±roll w/
backfire + Efficient=auto-+2-if-it-moves; (b) Pliable-Pres → Speaker/PPT/Key-Advisor decider override
incl. SCOTUS nom; (c) cabinet-bill double-points coupling (propose-in-portfolio → carried to law = 2×,
dropped = SecXX −50); (d) ideology-gated confirmation by office (RW-Populist SecWar rejected); (e)
regional-representation meter penalty (DomStab fell — no Deep-South Sec); (f) executive-actions-
implemented-by-cabinet-with-BLUNDER rolls (the §10.4.1/#22/#126 layer in a cabinet context). **★ SPLIT
(tech-lead): (i)+(f) = the meter/blunder roll engine · (b)+(d) = the appointment/confirmation layer ·
(c) = bill-coupling · (e) = election-feedback input** — they have different dependencies. **Binds at**
`runPhase_2_5_1_Lingering` (`:3260`, the roll engine) + `runPhase_2_3_1_Cabinet` (`:2158`, the
gate+decider-override) + the §10.4.1 blunder layer + the bill-scoring path (`:3508`-area double-points)
+ the §22.2 enthusiasm input. **Gated on #177 (federalism era existing) + the diplomacy/EconStab meters
it moves; couples to #67 meter ladders + #116/#160 EconStab cascade.** debt #59; game-mechanics §20.10.
**★ #182 command = the President's PER-TURN EXECUTIVE-ACTION BUDGET → FOLDS INTO the §14
executive-action build (K2 ActionRegistry) + #179; S once the exec system is real.** Verbatim: "Osgood
can do up to 4 actions each turn because he has 4 command" — a President's `command` = his
executive-action count per Presidential-Action phase (0 = inert, N = N actions/turn), each implemented
by the relevant cabinet member with a blunder roll (#179(f)); generalizes the §24.1/#61 "0-command
acting president is inert" read into a positive budget. **★ Shipped-state CONFIRMED unbuilt:** `command`
...
(`:2398/2409`), and a candidacy gate (`:1386`/`:3729`) — NEVER an action-count budget; the executive
...
`command`-many times) but **GATED on the §14 executive-action system becoming a real K2 ActionRegistry
chooser** (today a placeholder) + couples to #179(f) blunder. **Binds at** `runPhase_2_8_1_Executive`
(`:3632`) — turn the single placeholder into a `command`-budgeted loop over a real ActionRegistry (K2).
**★ Open Q (designer-gated): does the same budget apply to governors/other offices?** debt #62;
game-mechanics §20.11(2).
**★ #181 PRESIDENTIAL PLATFORM-PROMISE CONVENTION SYSTEM → PART OF the #19/#20 convention cluster; DO
NOT build standalone; M.** On winning the nomination (primary → convention → multiple ballots →
...
CRISIS, PLUS deal-making office-promises (cabinet/VP posts to allies for endorsements — "promise John
Marshall SecState/VP"); a binding commitment object feeding kept/broken → enthusiasm/points. **★
Shipped-state CONFIRMED unbuilt:** no convention/platform object exists (`grep platform|promiseBill|
conventionPlatform in src/` = ZERO; the convention machinery itself is designed-not-built, §15.3 /
#19/#20). **Binds at** the (unbuilt) convention subsystem — a `PresidentialPlatform` object generated
at the convention + a kept/broken evaluator wired to the §22.2 enthusiasm engine + the office-promise
deal layer. **Rides the #19/#20 convention epic; gated on conventions existing — DO NOT build
standalone.** debt #61; game-mechanics §20.14(1).
**★ #180 MONUMENT / LEGACY-TRAIT ACHIEVEMENT LAYER → RIDES the federalism/founding era-content build;
S–M, LOWER PRIORITY than #177-#179.** The founding era awards **named historical-legacy traits to
...
"Architect of the French Alliance"; etc.). **★ Shipped-state CONFIRMED unbuilt:** `traits[]` is a
freeform `string[]` granted only by `addTrait`/era-event/draft — NO first-achievement-detection /
...
rule needs authoring first.** debt #60; game-mechanics §20.14(2).
**★ DH-71 FACTION-HANDOVER DUPLICATE-POLITICIAN → a SINGLE-OWNERSHIP CONSTRAINT on the (unbuilt) #1
faction-handover, NOT a shipped regression; XS.** A mid-thread faction "steal" moved David Brearley to
a new faction WITHOUT deleting the original → two identical records in two factions → a later
governor-nomination pulled the WRONG one (career-track flag mismatch). A transfer must MOVE, not COPY.
**★ Shipped-state CONFIRMED — the faction-handover feature is itself UNBUILT:** `grep handover|transfer|
...
clone the record — and the build's owned-state model removes the manual-spreadsheet ledger drift this
run also exposed (an ideology-tally miscount; a crisis flag stuck "on"). **Binds at** the (new)
faction-transfer function (#1) — enforce single-ownership. **Folds into #1 faction-handover.** debt
#63; game-mechanics §20.15.1.
**★★ #177/#178/#114 ESCALATED IN PLACE (justification, NOT new scope, NOT a re-sequence):** **#177 is
now DESIGNED-AND-PLAYED-FORWARD, not just designed-and-walled** — `summer2021` is the FIRST source to
run the whole federalism TURN LOOP forward (cabinet appointment → era/decision events → cabinet-driven
lingering → diplomacy → legislation+veto+override → presidential action; §20.9), proving the walled
content is the *actual gameplay* of the next era; `eraGraph.ts::validate()` still throws via the same
TWO guards (`:153-155` `chartIndex >= 49` + `:150-152` the pre-1789 president/cabinet decider ban →
gate on `year >= 1789`), and the per-power diplomacy-relation meter is confirmed missing (the shipped
`snap.game.diplomacy` is a flat 8-power score map mutated only by §11.1 lingering drift, not a
relation-event meter) — **the highest-confidence "federalism is the next-era build" signal in the
corpus.** **#178** now has LIVE FORWARD-PLAY evidence — the war-version **Treaty of Paris is
...
Indian War** = win 5 land battles in 5 attempts (10 yrs) OR permanently CEDE OHIO — the same
bounded-retry budget as #178/DH-61 (§20.13(5)); `admitState` (`territories.ts:8-23`) still admits ONLY
from the static registry. **#114/E9/K5/DH-36 re-confirmed from the MULTIPLAYER end:** the CPU is
mechanically WEAK at moving meters (Ted's meta-math, POST 2671: ~50 all-CPU governors × ~3 actions ×
~20% only move a crisis meter ~1-2 spaces), so a crisis the CPU can't dent must be soluble by a single
engaged human → the quantitative root of "solo human is the design target"; the 3-GM relay over 14
months re-confirms DH-36 (GM-workload is the constraint) WITHOUT a clean burnout quit (it trails off).
**★ Raise priority/confidence; do NOT re-sequence, do NOT add scope.** **★ Corroborations (tags, not
...
impeaches → Senate convicts a SCOTUS Justice; contrast the VOIDED `ideo1928`, DH-66); **veto-override
scoring isolates Speaker+SenMajLdr**; **SCOTUS replacement nominated at the Presidential-Action phase ON
PURPOSE** (#52); the **Bank-of-the-US charter→repeal-attempt→repurpose-by-bill** trajectory (#177
institutions created↔killed↔recreated by law). **Decision-gated RECOUNT: 0** (the #179-split,
#181-rides-convention, #178 ≤3-fires canonicity, #182-applies-to-governors are gated WITHIN their own
work). **No re-sequence; top-of-queue UNCHANGED (QW0 → K0/K2 → K3/K4 + scenarioBoot → E1) — but the
...
| E20 | **Bill-scoring leaderboard (divergence #1, Phase A) + ★ batch-12 #128 Kingmaker / Master Kingmaker per-state bonus binding** | Add `legislativeScoring(bill, faction)` (±50/100/150 per matching ideology+lobby+interest card; **failed bills also score** + ±1 per-pol reelection deltas) onto a new `Faction.score?` ledger — **the same per-era running score K3 banks-and-zeros at the boundary (#68)** and the Phase-2 enthusiasm engine (E21) consumes. Keep `cardVoteBias` (`phaseRunners.ts:1516`) as the **vote-probability** input for floor votes. (Phase B parked.) **Batch-8 placements:** **(a) #103 pres-vote modifier stack + era-stamped issue list (`tea1772`);** **(b) DH-42 BALANCE — national meters swamp per-state lean (`tea1772`).** **Batch-9 placements:** **(c) DH-52 landslide-margin-cap;** **(d) #108 gradual 4-lever realignment;** **(e) #110 modern election machine** (deep form rides E23). **★ Batch-10 placement — DH-53 bill→EV/meter STRUCTURED-effect tables (S, NOT a sign-flip):** `dem1820` corroborates `nuke`'s recurring bill-effect sign bugs from a 3rd thread (POST 462-466). **The fix is NOT a sign-flip — AUTHOR the structured per-bill effect tables**: `Legislation.effects` has **no per-state EV field**, and `applyEffect` **cannot mutate `electoralVotes`** today. Pairs with K4's DH-48 structured-`evDelta` shape. **★★ Batch-12 #128 (`tedchange#POST 316`, game-mechanics §6.5.y) — KINGMAKER / MASTER KINGMAKER per-state BONUS SCOPE (S — pin the +1 binding sites):** verified shipped: the Kingmaker bonus is **NOT YET a state-vote bonus** — `calcStateVote` (`phaseRunners.ts:3685-3722`) does NOT consult Kingmaker state to add a per-state +1. Ted's `tedchange#POST 316` rules: **basic Kingmaker = +1 in own state only** (incl. state's Pres primary + state's general); **Master Kingmaker = +1 in EVERY state** (all Pres primaries + all generals, incl. own state). Pin the binding at `calcStateVote` `phaseRunners.ts:3711` (adjust the score sum) + a new helper `kingmakerBonus(snap, candidate, stateId): number` that returns +1 if any Kingmaker in the candidate's faction is from the same state (basic) or anywhere (Master). **SUPERSEDES Matt's "state OR national, pick one" reading.** **RULED by Ted (`tedchange#POST 302-320`).** **★★ Batch-31 — THREE NEW ELECTION DELTAS land at this scorer (`calcStateVote` `phaseRunners.ts:3685-3723`; debt #94; `primary2020`): (f) #228 the THIRD-PARTY SPOILER VOTE-DRAIN (M; `GM⇒App`)** — wholly unbuilt (`grep spoiler|thirdParty` in `src/` = ZERO): a vote-drain keyed on **party-of-origin** (3rd party sprung from a nominee's party → 50% −1 party-pref in the nominee's HOME REGION, nationwide at 50% if Charisma), **shared personal-ideology** (50% −1 nationwide → 75% if Charisma), and **incumbency** (50% −1 if party-pref leans away from the incumbent party), with **Charisma the region→nationwide / 50%→75% force-multiplier**; FIX the current incumbent-only mis-attribution so a 3rd party springing from the CHALLENGER drains the challenger (`primary2020` §4 POST 68-77, both designers liked it; pairs #48 the trigger/PV half + #214 Charisma valuation). **(g) #230 the SWING-STATE / SAFE-STATE CAP (S-M)** — `calcStateVote` scores EVERY state identically and the `(Math.random()−0.5)*8` jitter at **`:3711`** can flip safe states (Dem wins OK/KS, "FL right of MS"): only **party-pref ≤1 states are flip-eligible battlegrounds**; safe states draw a **bounded margin** so the jitter can't flip them; reconcile the sub-point margin-cutoff so safe states LOCK. **★ This is the SAME `:3711` jitter surface as QW19 — PAIR them: seed the jitter via `rng.ts` (QW19), THEN gate it behind the swing-state set; also the ±3 meter-swing clamp (QW3) lives at the same line** (`primary2020` §3 POST 28, 40-50; pairs QW19/QW3/#184/DH-72/#18). **Open: wanted feature or shelved "wait for CPU elections" — designer confirm before scheduling** → put the swing-state cap behind that confirm, but the QW19 seed/narrow ships regardless. **(h) #231 the MODERN-MODERATE OVER-PERFORMANCE balance REGRESSION CASE (not a new system)** — designer-diagnosed SYSTEMIC (not rolls): the modern-era Lib/Prog/LW-Pop enthusiasm maluses + the Moderate primary-penalty/general-bonus knob make Moderates sweep modern primaries reproducibly (Buttigieg 600→989→700 delegates every run; `primary2020` §2-§3 POST 4-12, 66-67) → **re-tune so candidate quality, not meter state, decides — fold into the #18/#51 enthusiasm-engine build, NOT a standalone row.** | K0; K3 (banking consumes `Faction.score`); **K4** (shares the DH-48 structured-`evDelta` schema for DH-53); **#230 pairs with QW19 at the shared `:3711` jitter** | M (+ DH-53 S + #128 S + **★ batch-31: #228 spoiler M; #230 swing-state cap S-M [pairs QW19]; #231 a re-tune folded into #18/#51, not a row**) | gap #12, divergence #1 (`gilded`/`1772s`/`modern`) + **#103 (`tea1772`)** + **DH-42 (`tea1772`)** + **#108/#110/DH-52 (`nuke`)** + **DH-53 structured bill-effect tables (`dem1820` POST 462-466)** + **★★ #128 Kingmaker / Master Kingmaker per-state bonus (`tedchange#POST 302-320`)** + **★★ batch-31 #228 third-party SPOILER vote-drain (`primary2020` §4 POST 68-77; `GM⇒App`; M) + #230 swing-state/safe-state CAP (`primary2020` §3 POST 28, 40-50, 61; pairs QW19 at the shared `:3711` jitter; S-M; designer-confirm-before-scheduling) + #231 modern-Moderate over-performance balance regression case (`primary2020` §2-§3 POST 4-12, 66-67; re-tune folded into #18/#51)** — CARRIED + EXTENDED + designer-RULED, HI-CONF (4 era + designer + batch-31 stress-test) | ready (#228/#230 NEW; #230 paired with QW19; #231 a regression-case re-tune, not a standalone row) |
...
> **★★ Batch-30 Phase-1 FOLD-INS (`revampPV`/`summer2021`/`ampuData`/`ampuUpdates`; debt #85-#90; dependency-
> ordered into the epics they belong to — NO new keystone, NO re-sequence, top-of-queue UNCHANGED).** The
> batch is DESIGN/DATA (no playtest); its deltas FOLD into existing epics rather than spawning new ones. **★
> THE PV-REWEIGHT BALANCE EPIC (E20b/E20 + `pv.ts`; debt #85; the back-half of the PV overhaul — the front-half
> #216/DH-77 are QW22/QW23 above):** **#214** (per-trait TIER table replacing the flat ±4/−5 + magic
> `Kingmaker +6` at `pv.ts:76-79`; **M-L**) + **#215** (per-ability decreasing-returns curve replacing flat
> `skill×weight×4`/`command×10` at `pv.ts:70-74` — first-point-worth-more, Command ≈25/level, Admin/Military
> low-floor; restrict-born-Command left OPEN as a toggle; **M**). **Build them as ONE PV-reweight PR**, AFTER
> QW22's #216 trait-name remap (un-applyable without it). **★★ BATCH-33 SUPERSEDES TWO calls in this row (see
> the batch-33 Phase-1 fold-ins + the QW24 row):** (i) the **"GATED on the un-ingested 'Groupthinking the
> Political Value' trait-EFFECTS thread"** call is LIFTED — `groupthinkpv` IS that thread (now INGESTED), so the
> epic moves **gated → READY-AFTER-#216**; the curve is RULED (Command×15 · Legis/Gov/Admin×10 · Mil/Jud×5 per
> level + expertise +5) and the build now ALSO includes the **0%-built trait-EFFECTS-in-PV layer** (M-L); (ii)
> the **"MANDATORY same-PR `pv*0.1` re-tune"** call is REPLACED by **REMOVE `pv*0.1` outright (QW24)** — the
> designer + room RULED PV must NOT feed elections (`groupthinkpv` POST 36-38), so the term is DELETED at
> `phaseRunners.ts:3709` (the 2.9.1 primary's raw `pvCache` at `:3733` is KEPT), DECOUPLING the reweight from
> elections so it can re-fit freely. ~~**★ MANDATORY same-PR coupling: re-tune `calcStateVote`'s `pv*0.1`
> (`phaseRunners.ts:3709`; `pv= c.pvCache` at `:3699`) — a PV reweight directly shifts election margins, the
> SAME election-balance surface as #18/#184/DH-72/QW19-21**, so this composes with E20b and must not ship the
> new PV ceiling without re-banding the election input.~~ Keep office/age/FF terms ADDITIVE; do NOT double-count
> `flipFlopperPenalty` (`:86`); fold Kingmaker + Master-Kingmaker into the table (drop the `:79` `+6`). **(#220
> display-scale was a Decision-gated (A) User-gated pick — ★ batch-33 RESOLVED it to 0–100 / 50=average; the only
> remaining human-call is the office-magnitude ↔ 0–100 scale reconciliation.)** **★ #217 the president/cabinet
> IMPLEMENTATION sub-engine + ~30 governing traits → E16 (cabinet) + E6 (lingering/blunder); L, gated on
> federalism (E1) existing.** A post-legislation success→Blunder pass modified by expertise-gated genre traits
> + Delegator/Micromanager + Crisis-{Manager,Admin,Gov} (the Crisis-Admin 10/20% Lingering crisis-pull ON TOP
> of #179's cabinet rolls) + Passive/Predictable + Passive+Pliable PUPPET + Magician — **rides #179's roll
> engine (`phaseRunners.ts:3260`) + #182's action budget (`:3632`) + the §14.1.y/#126 2-step Admin→Command
> blunder rule.** De-dup vs Efficient/Egghead/Easily-Overwhelmed FIRST (vcczar kept overlapping traits,
> "slightly different"); these traits also need PV values (#214) + the remap (#216). **★ #218 Rule-of-Four/CJ
> case-refusal → E25** (a ≥4-Judicial CJ may refuse a Gov-Action state case; Rule-of-Four overrides; Iron-Fist
> CJ bypasses; Jurisprudence = unlimited; Passive = cannot refuse — **shares #217's Passive/Jurisprudence/
> Iron-Fist trait reads**; S, folds into the SCOTUS docket). **★ #219 US-Rep PROPORTIONAL representation →
> E14/E7** (each influential US Rep carries `floor(stateReps/influentialReps)` of the apportioned seats,
> remainder to the highest-Legislative rep; refines the #55 focus-Rep abstraction; S). **★ SMALL FOLD-INS
> #222–#227** (several `GM⇒App`): **#222** committee-size scales with state count (3→11 at 50; S → E7/E14,
> over the fixed 4-committee sort `phaseRunners.ts:1894-1928`) · **#223** chair-selection 25%→enthusiasm (XS →
> E7/E16, hooks #18/#51/#124) · **#224** lowest-satisfaction 25% AUTO-FLIP (S → the realignment/conversion
> cluster E9/#108; **DISTINCT from the explicit conversion AI #76** — an automatic defection triggered by a
> bottomed satisfaction meter, no conversion attempt) · **#225** Celebrity expiry if a pol runs before 40 (XS →
> E18/draft + the obscure cluster #136; reconcile with the `summer2021` Celebrity/Military-Leader merge + nerf)
> · **#226** Frail/Hale lifespan auto-traits (died<60⇒Frail, lived86+⇒Hale; XS — **a NEW `scripts/` DATASET
> rule, NOT engine**; folds into the #120 dataset umbrella E18d + `scripts/seedDataset.mjs`, alongside the
> `draftYear`/sub-floor rules) · **#227** Trim-Roster phase REMOVED (XS verify-only → the death/retirement
> cluster; **already aligned** — no Trim-Roster in `phases.ts`/`src/`; just confirm the `summer2021`
> random-retirement tuning [no random retire <60, one-term-loser 50%, losing-nominee-<60 25%] does the
> pruning). **★ DH-76 cloture** rides E9 9b (K5-dependent, debt #80/#208) — no re-size, it now just has its
> `summer2021` ORIGIN; **#115 generate-a-candidate** (sharpened: name-DB tagging + 1/9/20/20 tiering +
> lowest-score backfill) folds into the `scenarioBoot`/retro-gen cluster (K4, §9.1.9; overlaps #74). **NO
> re-sequence; the top-of-queue is UNCHANGED (QW0 → K0/K2 → K3/K4 + scenarioBoot → E1).** debt #85-#90;
> technical-guide §9 batch-30 lead + §8 debt #85-#90.
>
> **★★ Batch-32 Phase-1 FOLD-INS (`tomorrowlist`/`businesslabor`/`histpres`/`georgemartha`; debt #97-#100;
> dependency-ordered into the area each belongs to — NO new keystone, NO re-sequence, top-of-queue UNCHANGED).**
> A MIXED data/design batch (1 founding solo + 4 data/design); its deltas are NEW designed-but-unbuilt SURFACES +
> one DATASET-fill action that FOLD into existing areas rather than spawning new epics. **The within-batch order is
> binding: the #216 remap (= QW22, already queued above) → #240 core skills/traits fill (standalone) → the #99
> data-model cluster (schema) → #240 full Misc/Demographics fill; #236/#237 wait on #221.** **★ #240 THE
> `CURATED_ROWS` DATASET FILL → a STANDALONE author/DATA task OFF the engine track, folds alongside the #120
> dataset umbrella (E18d / `scripts/seedDataset.mjs`); S-M (debt #100; `GM⇒App`).** **★ CORRECTED FRAMING:**
> `CURATED_ROWS` is NOT empty — `seedDataset.mjs:275` `= []` is only the INITIALIZER; the loop `:276-289` builds it
> to **143 entries at runtime** from the `ROWS` source array (`:38-233`) (runtime-verified; `ERA_FIGURES` = 23).
> `ROWS` holds the 1772+1856 marquee roster, NOT the 47 presidents, so **#240 = "ADD ~47 president statlines to the
> `ROWS` source array"** (the loop folds them into `CURATED_ROWS` as same-named OVERRIDES that beat the
> `congress-legislators` real-served rows per the `CLAUDE.md` merge precedence), then regenerate (`bash
> scripts/fetchLegislators.sh && node scripts/legislatorsToDataset.mjs && npm run build`) — NOT "fill an empty
> array." **★ HARD PREREQ = the #216 trait-name remap (QW22):** 14/50 `histpres` trait names mismatch the shipped
> `Trait` union (`Charisma`→`Charismatic` + 13 with no analog) → an importer CANNOT author the tuples until the
> rename/cut map exists. The skills/traits/ideology CORE lands once QW22 does; the **Misc/Demographics/Dynasty
> lines wait on the #99 data-model cluster (the schema half)**. **★ HUMAN-CALL (recorded in Author-before-build):
> trust the 2021-vintage raw skills vs re-derive under the #214/#215 PV curve** (every president maxes a single
> ability at 3, no 4s/5s, per #204). It does NOT block or get blocked by the engine track. **★ #236 THE
> ALTERNATE-GOVERNMENT-FORM SCENARIO AXIS → folds into the era-content / #221 content-primitive area; L (debt #97;
> `GM⇒App`, the REFEREE half).** Fascist/Communist/Theocratic USA the country transitions INTO, each with its own
> Pres-Action/Legis-Prop/Scripted-Event menu + candidate pool + a reusable cross-form "authoritarian kit"
> (ban-opposition-parties / empower-executive-via-amendment / ban-politicians-by-class / state-media /
> nationalize-sector / dissolve-Congress); Theocracy ×5 by denomination, via a `game.governmentForm` flag.
> **VERIFIED wholly unbuilt** (only a `Theocrats` lobby STRING `factions1856.ts:28`). **★ TWO HARD DEPENDENCIES
> keep it OFF the near-term queue: (a) it RIDES the unbuilt #221 per-administration content-primitive registry**
> (`grep -iE 'presAction|legisProp|govAction|scriptedEvent' src/` = ZERO — nothing to attach a regime menu to);
> **(b) Theocracy is BLOCKED on the #239 religion field** (assign a denomination per pol + denominational
> party-splits) + §24 amendments (for the empower-executive primitive). **★ DISTINCT from #6/#354** (mid-era
> social-MOVEMENT factions / the `Theocrats` lobby — a movement-faction is a lobby; a government-form is a scenario
> branch). **★ HUMAN-CALL (recorded in Decision-gated): the transition TRIGGER + the win/loss/persistent-branch
> END-STATE (ties #88/#188).** **★ #237 THE STATEFUL POLICY-GENRE FRAMEWORK → folds into the era-content/policy
> area alongside #236; M-L (debt #98; `GM⇒App`, the REFEREE half).** Discrete toggleable prerequisite-gated
> policies in named GENRES (Business/Labor is ONE; **Currency + Copyright are siblings**), each with a `*-Default`
> baseline (absence-of-policy IS a state) + an `L/P/G/S` mechanism prefix + an era-band tag from the SAME #92/#206
> vocabulary + `Preq:` chains gated on other policy flags / scripted-event state / **the SHIPPED `economic` meter**
> (`NationalMeters.economic` `:1401` — Recession→Create-Government-Work, Depression→WPA/PWA) / party-type flags
> (Communist-major→Nationalize-Unions — **ties the #236 kit**). Fix = a `game.policies` flag store + the schema +
> the economic-meter wiring + content for the 3 genres + the symmetric pro-business institutions (National Business
> Council / Sec. of Business). **★ DEPENDENCY: like #236, the `L/P/G/S` entries ARE content primitives → RIDES the
> unbuilt #221 registry**; couples to the SHIPPED EconStab economic engine (§29.7/§29.7.1, #116/#160). **★ DISTINCT
> from interest-cards/lobbies** (those nudge state industries; this is a national + per-state policy-FLAG axis).
> **★ HUMAN-CALL (recorded in Decision-gated): the "no policy" default = flat "left to the states" vs
> computed-from-state-makeup.** **★ #238 / #239 / #194 THE CURATED DATA-MODEL CLUSTER → folds into the dataset/
> roster work (E18d-adjacent) as the SCHEMA HALF of #240; M (debt #99; `GM⇒App`).** **VERIFIED ABSENT on both
> `Politician` (`types.ts:1251-1291`) and `ImportedDraftee` (`:1780-1793`).** Build all three TOGETHER as ONE
> data-model extension: **(#238 gender)** `Politician.gender` + a suffrage-keyed electoral-eligibility GATE
> (women→backroom/lobby-only pre-suffrage) + a spouse/family relationship EDGE enabling spousal Key-Advisor
> (Ted-endorsed); **(#239 curated-override flags)** `southernUnionist` (corroborates #121/#122) / `canBeIndependent`
> / `partyFlipBudget` (Trump ×3, Reagan-starts-Dem) / `canMoveStates` (atop the SHIPPED relocation
> `Politician.altState` `:1256-1258` — a curated AUTHORIZATION, not a new system) + the **race×religion×sex
> demographics triple — the `religion` field is the PREREQUISITE for the #236 Theocracy branch** (NOT low-priority
> flavor); **(#194 dynasty)** wire the `histpres` turnkey **dynasty SEED TABLE** (~37 named dynasties incl.
> multi-president Adams/Harrison/Roosevelt/Bush) into the existing §30.18.E #194 lineage spec. **★ Build the
> relationship-edge + the demographics tuple TOGETHER** (the family edge is shared by #238/#194; the religion field
> gates #236-Theocracy). Needs a save-migration (`DB_VERSION` bump). The #194 lineage ENGINE
> (auto-shed-`Obscure`-on-elder's-Presidency / extinguish-line-if-parent-predeceases-heir) is a separate §30.18.E
> build. **★ SHARPENINGS (annotate in place, no new scope):** **#194** now has its turnkey dynasty seed table;
> **#216/#214/#215** — `calsleaderboard` CONFIRMS the office-weighting is RIGHT (Presidency ≫ doc-authorship/CC-Pres
> > governor/Congress > faction-leader; office LOSS = instant PV collapse) AND that **raw command is OVER-CREDITED
> early** (the decreasing-Command-returns target of the #215 curve at `pv.ts:74`; skill-cap-3 per #204) — folds
> into the debt #85 PV-reweight epic above; **#206/#221 Era-of-Future NON-tech content** from BOTH `tomorrowlist` +
> `businesslabor` reconfirms Future DOUBLY UNBUILT (no new architecture); **#115/#24** `georgemartha`'s curated
> "what-if" draft-pool toggle (women + Toussaint) ties #238 (gender makes drafted women PLAYABLE), folds into #115.
> **NO re-sequence; the top-of-queue is UNCHANGED (QW0 → K0/K2 → K3/K4 + scenarioBoot → E1).** debt #97-#100;
> technical-guide §9 batch-32 lead + §9.6 + §8 debt #97-#100; game-mechanics §30.22.A, §30.22.B, §30.22.C,
> §30.18.E, §29.7, §24.x, §3.1.

> **★★ Batch-33 Phase-1 FOLD-INS (`groupthinkpv`/`legisexecgov`/`ampuelections`/`housepoll`/`excelautomate`;
> debt #85/#87/#97-#98/#101-#103; dependency-ordered into the area each belongs to — NO new keystone, NO
> re-sequence, top-of-queue UNCHANGED).** A RECONCILE/UNBLOCK batch: most deltas FOLD into existing epics or
> UNBLOCK ones already on the board. **The within-batch order is binding: remove `pv*0.1` + the #216 remap
> (cheap-fixes lane, now) → the PV epic (#214/#215/#220 + the trait-EFFECTS layer) → #221 registry → #242
> amendable articles + #241 cadence (inside K3) → the House model (#219/#191) → DH-78.**
> **★★ THE PV-REWEIGHT BALANCE EPIC IS NOW UNBLOCKED → READY-AFTER-#216 (debt #85; `pv.ts:70-88` + the 0%-built
> trait-EFFECTS layer; the batch-30 block above scoped this as GATED — `groupthinkpv` LIFTS the gate).** The
> long-flagged blocker ("the trait EFFECTS live in an un-ingested thread") is GONE: `groupthinkpv` IS that thread
> (the EFFECTS/office-PV sibling of `revampPV`'s VALUES — read as ONE spec). **Direction now RULED:** the ability
> curve = **Command×15 · Legislative/Governing/Admin×10 · Military/Judicial×5** per level (+ expertise +5 each);
> display = **0–100, 50 = average pol** (so **#220 is RESOLVED to 0–100/50-avg, off Decision-gated (A)**); worst
> trait = Incompetent −20 (the spreadsheet `lackey −53` was a TYPO → −5, already correct → re-scopes DH-77/QW23).
> **The build is #214 tiers (M-L re-fit) + #215 curve (M) + #220 (XS+S) + remove `pv*0.1` (now QW24) + BUILD the
> 0%-BUILT trait-EFFECTS-in-PV layer (M-L** — office-conditional ability buffs, Crisis-*, Magician take-higher,
> Cosmopolitan/Provincial regional EV, Lackey behavioral lock-in; OVERLAPS the §14.1.y #217 implementation
> engine). **Order: #216 remap (QW22) FIRST** (un-applyable without it). **★ The MANDATORY same-PR `pv*0.1`
> re-tune from the batch-30 block is SUPERSEDED — `pv*0.1` is REMOVED outright (QW24), which DECOUPLES the
> reweight from elections so it can re-fit freely; office-PV is scoped to the active-draft/persuasion AI, NOT the
> vote.** **★ The ONLY remaining human-call across the whole epic = the office-magnitude ↔ 0–100 display-scale
> reconciliation** (the #220 scale is otherwise resolved). **Net: this epic MOVES from gated → ready-after-#216;
> it stays in Phase-1 (down-ballot/PV cluster, E20b/`pv.ts`), it is NOT a new top-of-queue item.** debt #85.
> **★★ #221 THE 3-PRIMITIVE CONTENT-MODEL REGISTRY → folds into the era-content/K4 area; L, downstream of K4
> (debt #97/#98 area; `GM⇒App`, the REFEREE half) — now WITH its authoring source.** `legisexecgov` is vcczar's
> explicit crowdsourcing call for a registry of THREE content primitives: **Legislative Proposals** (federal+state
> bills incl. constitutional Amendments) / **Presidential Actions** (president-only, inactive pre-Constitution) /
> **Governor Actions** (state-scoped) + **scripted events**, with era/date gating, importance-curation (joke laws
> REJECTED so the CPU doesn't propose them), meter/payoff tradeoffs, prereq-chain ladders, and option-set/
> branching primitives + an era-weighted CPU-selection buff that "kind of exists already." **VERIFIED 0% shipped:**
> `grep -riE 'presAction|legisProp|govAction|scriptedEvent' src/` = ZERO; the only adjacent thing is the runtime
> `Legislation` INSTANCE record (`types.ts:1506`) built from tiny inline `templates` arrays
> (`continentalCongress.ts:237`, `phaseRunners.ts:3437+`) — NO era-keyed authored proposal pool, NO Pres-Action
> type, NO Gov-Action type. **★ #236 (alternate-government-form, debt #97) and #237 (policy-genre framework, debt
> #98) BOTH RIDE this** — neither lands before #221. **L (a new typed primitive registry + era-gating + the
> CPU-selection draw #70-#78 reading from it); folds into the era-content/K4 area, NOT a new keystone; stays
> where it is in the queue (downstream of K4).** debt #97/#98 area; game-mechanics §15.1.
> **★ #242 AMENDABLE `ConstitutionalArticles` → folds into the §24 amendments / #221 content area; M, gated on
> #221 (debt #102; `GM⇒App`).** `ConstitutionalArticles` (`types.ts:1389-1397`) is written ONCE at the CC
> (`constitutionalConvention.ts:145`) and never mutated; the design wants it AMENDABLE mid-game — a Legis-Prop
> Amendment rewrites an article on 75%-state ratification — plus NEW fields (suffrage / EC-toggle / houseSizeCap /
> term-length). **Downstream of #221 (an Amendment IS a Legis-Prop) + #241 (term-length is a shared field); the
> data shape already exists, so the build is the mutation PATH + new fields + a `DB_VERSION` default.** debt #102;
> game-mechanics §14.2, §15.4(c).
> **★ #241 VARIABLE TERM-LENGTH CADENCE → folds into K3's `year%4`-predicate-replacement; M, cost shared with K3
> (debt #101; `GM⇒App`; ties #92).** The election cadence is hardcoded `year%4`/`%2` (`phases.ts:53-58`) with NO
> term-length field anywhere; a passed 6-/2-yr-presidency Amendment would break every predicate. vcczar flagged
> this in his own voice (`ampuelections`). **This is the SECOND independent reason to replace the year-predicates**
> (the FIRST is #92's era-advance, debt #5/§9.1.5) — so build the predicate-replacement ONCE inside K3's
> `advanceEra`/`year%4`-replacement work and a per-office term-length field serves BOTH #92 and #241. **M, but the
> cost is SHARED with K3 — no standalone epic; folds into K3's cadence/predicate work** (and feeds #242's
> term-length article field). debt #101; game-mechanics §2.3.
> **★ THE HOUSE MODEL — #219 RESOLVED + #191 FULLY AUTHORED → folds into the census/apportionment + down-ballot
> clusters (#34/#55/#73/#74); S-M (debt #87/#73; `GM⇒App`, the REFEREE half).** `housepoll`: the community VOTED
> to **KEEP the focus-Rep abstraction; every-real-435-seat was REJECTED → parked "AMPU 2."** Shipped state: flat
> `representativeIds: string[]` (`types.ts:1327`) — NO size enum, NO EV-ladder, NO vote-weight, NO census.
> **#219 (RESOLVED — keep focus-reps):** US Reps = historical EVs−2 (census recompute = first election after a
> decade) → Focus Reps via an **EV-derived 1-10 ladder** (≤5→1 … ≥46→10, **SUPERSEDES #55's flat `(EV−2)/5`**) →
> **vote power = `floor(stateUSReps / focusReps)`, remainder → the highest-Legislative rep**. **#191 (FULLY
> AUTHORED, `housepoll` POST 17-19):** a per-seat STATIC `deviantPartyBias` (overrides ONLY the party term) + the
> minority-seat guarantee (5+ Focus Reps → a Bias+2 minority seat) + the **25/25/50 new-seat / seat-gain /
> seat-loss rolls** keyed to **era-gated growth lobbies** (the per-seat deviant-bias engine). Both ride a NEW
> **per-seat struct** (the vote-power remainder + the deviant bias live on the same shape) + the #34 census + the
> row-74 gerrymander gov-action that SETS the bias. **S-M; folds into the #34/#55/#73/#74 clusters; not a new
> keystone. ★ Mark #219 RESOLVED (focus-reps kept; every-435 = AMPU-2 parking) + #191 FULLY AUTHORED.** debt #87/
> debt #73; game-mechanics §29.5.
> **★ DH-78 TWO-TRAIT AND-COMBO ELECTION-BONUS FIELD → folds into the trait-election-rule cluster (E20b); XS-S,
> gated on #216 (debt #103; `GM⇒App`).** `TraitElectionRule` (`types.ts:727`) has only `opponentConditional` — it
> CANNOT express a same-candidate two-trait AND-combo (Celebrity AND Military Leader → +1, `excelautomate` POST
> 50). Fix = a `requiresAlsoHas: Trait[]` field + a consume-side check in `applyTraitElectionBonus`
> (`electionEffects.ts:16`); **GATED on #216** (which adds `Military Leader` to the `Trait` union); **verify it
> wasn't deliberately cut first.** debt #103; game-mechanics §14.1.3.
> **★ SHARPENINGS (annotate in place, no new scope):** **#20** (gerrymander/gov-action umbrella, debt #74) —
> `legisexecgov` confirms Gov Actions today are FLAT / state-agnostic; the Gov-Action primitive should be
> state-scoped with per-state variation (sharpen #20, no re-size). **DH-77/QW23** — re-scoped (worst trait =
> Incompetent −20, the `lackey −53` was a typo; the floor-OFFSET fix stands). **#114/DH-36** — `excelautomate`
> corroborates the charter from the BUILD side (the *">30 min/phase ⇒ automate"* GM-burden budget; the ① automation
> priority order tallies → trait-lookup → election-scoring → legislative-scoring); no scope change. **NO
> re-sequence; the top-of-queue is UNCHANGED (QW0 → K0/K2 → K3/K4 + scenarioBoot → E1); the ONE delta to the
> cheap-fixes lane is QW24 (`pv*0.1`-removal), and the PV epic moves gated → ready-after-#216.** debt #85/#87/
...
rules-authoring holes. **★ Batches 16–28 add NO new author-before-build items** (all their deltas are
buildable engine work or rulings/annotations within existing epics). **★ Batch 29 (`wilsons1916`/
`welcome2future`) adds NO new author-before-build RULES hole, but surfaces ONE separate CONTENT-authoring
epic → the Era-of-the-Future PROPOSAL LIBRARY (#206, see below):** events for the Future band are
reportedly done, the missing piece is LEGISLATION ("not completely fleshed out… mainly I need more
proposals," `welcome2future#post 6`) — the same "events-before-legislation" content shape as the
...
new one. **★ Batch 30 (`revampPV`/`summer2021`/`ampuData`/`presidents`/`ampuUpdates`) adds NO new author-
before-build RULES hole** — all its deltas are buildable engine/dataset work or fold-ins (the PV overhaul =
QW22/QW23 + a gated balance epic; #217/#218/#219/#221/#222-#227 fold into existing epics; #226 is a `scripts/`
DATASET rule under #120/E18d). **But it records TWO things in THIS bucket: (a) HYGIENE — do NOT schedule** (so
...
#68/#102), the **randomized / Legis-weighted proposer** (REJECTED — the committee system was kept), and the
`revampPV` per-trait **"absence bonus"** idea (points for LACKING a trait — the additive PV model can't
express it, P37-39; a parked design idea, not a scheduled item). **(b) ★ CORPUS-COVERAGE / UN-INGESTED
SOURCES** (candidate future uploads — ask the human for exports before the dependent work): **(i)** the
`presidents` record names **≥7 un-ingested forum playtests** — highest-value is the **★ 1840-start
persistent-Whig ~100-yr run** (Harrison→…→Hearst, ~27 presidents, a Whig major party that never realigns; the
shipped build has **no 1840 scenario analog**) — get this BEFORE any 1840-start / Whig-realignment work; also
...
which set only POINT VALUES) — **this thread GATES the #214/#215 PV-reweight balance epic** (chase it before
that epic builds). **★ Batch-31 RECONFIRMS the same coverage gaps + adds one (`bestmoments` corpus-coverage
cross-refs) — NO new author-before-build RULES hole** (all batch-31 deltas are buildable engine work or fold-ins;
the confirmation subsystem GROWS debt #50, the 15-era table is a K3/K4 annotation, #228/#229/#230/#233 fold into
existing epics, #234/#235 are PARKED): the **1840-start** persistent-Whig run is reconfirmed the highest-value
un-ingested source; the **"Groupthinking the Political Value"** trait-EFFECTS thread remains un-ingested (still
gates #214/#215); and **matthewyoung123-1772** (the rookie founding run, cross-referenced by `bestmoments`) is
added as a candidate founding-variant export — **ask the human for these before any 1840 / PV-effects /
founding-variant work.** A coverage/sourcing note, NOT a build item. **★ Batch 32
(`tomorrowlist`/`businesslabor`/`histpres`/`georgemartha`/`calsleaderboard`) adds NO new author-before-build
RULES hole** — its four new surfaces are buildable engine/dataset work or fold-ins (#236/#237 fold into the
era-content/#221 area; #238/#239/#194 are the dataset/roster schema cluster; #240 is a `scripts/` author/DATA
task). **But it records ONE DATA-AUTHORING HUMAN-CALL in THIS bucket (a judgment the importer must make before
#240's statline fill, not a rules hole): the #240 raw-skills-vs-re-derive call** — vcczar's 47 `histpres`
president statlines are **2021-vintage (pre-#214/#215 PV revamp)**, so the author must decide whether to
**transcribe the raw skills/command/traits verbatim** or **re-derive them under the current #214/#215 PV curve**
before authoring the ~47 tuples into the `ROWS` source array (debt #100; every president maxes a single ability at
3 — no 4s/5s — consistent with #204; ties the un-ingested "Groupthinking the Political Value" trait-EFFECTS thread
that gates #214/#215). Record it so the #240 author doesn't silently pick one; it does NOT block the QW22 remap or
the engine track. game-mechanics §30.22.C, §3.4.1. **★★ BATCH-33 CORPUS-COVERAGE UPDATE — "Groupthinking the
Political Value" is now INGESTED** (`groupthinkpv`, batch 33): it is **REMOVED from the still-needed un-ingested
list above** (item (ii) is RESOLVED). With it ingested, the **PV-reweight balance epic (#214/#215/#220 + the
0%-built trait-EFFECTS-in-PV layer) is UNBLOCKED → READY-AFTER-#216** and **leaves the gated/Author-before-build
bucket** (it is now buildable Phase-1 work; #220 display-scale is RESOLVED to 0–100/50-avg, off Decision-gated;
only the office-magnitude ↔ 0–100 scale reconciliation remains a human-call inside the epic). **★ The still-needed
un-ingested set is now: (1) the 1840-start persistent-Whig ~100-yr run** (still highest-value, no 1840 scenario
...
- **DH-14 — Bill ideology impacts not era-aware (NEW, batch 5; ★ batch-29 REFRAME — it is a
  ...
  short of 2/3): GM: *"This amendment will never pass in a game with CPUs."* **★ Batch-29 (`wilsons1916`)
  REFRAMES this: the SAME 19th Amendment ("Equal Voting Rights for Women") PASSES in the 10-human 1916
  game (House 374-61, Senate 79-17, ratified)** — so the "never passes with CPUs" verdict is a **CPU-AI /
  era-ideology-weighting artifact, NOT a mechanical block** (logged as a ② opponent-half artifact in the
  GM-replacement lens). The fix is TWO already-queued things: **(1) THIS author task** — the era-keyed
  bill-ideology impact table (move Mods off suffrage's negative side in 1916) — AND **(2) a CPU vote model
  ...
  catalog (E14 / E20) + the #208 CPU legislative engine. — DH-14.
...
- **★ Era-of-the-Future PROPOSAL LIBRARY — a CONTENT-authoring epic (NEW, batch 29; #206, debt #78).**
  NOT a rules-authoring hole (the #206 enum-vs-data question is RESOLVED — a K3/K4 annotation, see above)
  but a **content gap**: the terminal Future band's **events are reportedly done while its LEGISLATION is
  a stub** — dev, verbatim: *"not completely fleshed out… mainly I need more proposals"*
  ...
  it is authored LAST (founding → antebellum → modern → future). — #206 (`welcome2future#post 6`; folds
...
path) are buildable in #155/#56. game-mechanics §17.4.** **★★ Batches 25/26/27/28 net 0 — counts
UNCHANGED:** no item entered or left either sub-bucket (batch-28's `GM⇒App` cluster all rides existing
epics WITHIN their own work). **★ Batch 29 (`wilsons1916`/`welcome2future`) nets +1 to (A) User-gated:
#207's COMPLETION-MODEL pick (year-cap vs open sandbox vs score-out) is a genuine human design call** —
...
by the tech-lead — not a fork); #208/DH-76/DH-74/#209/DH-75 fold into E9/9b-9d + K4 + the officer filter;
the #84 smaller asks ride existing epics; #213's exact ON→OFF boundary is designer-gated TUNING within
the #171/#108 work (not a new bucket entry). **New User-gated count: 3 (#52 + delegate-class + #207).**
**★★ Batch 30 nets +1 to (A) User-gated: #220's PV DISPLAY-SCALE pick (raw "pink" vs 0-100/0-200 vs
100-normalized) is a genuine human presentation call left UNDECIDED in both source threads** (added to (A)
below; everything else in batch 30 is buildable engine/dataset work or a K3/K4 annotation). **New User-gated
count: 4 (#52 + delegate-class + #207 + #220).** **★★ Batch 31 nets 0 — counts UNCHANGED** (all batch-31
deltas ride existing epics or are annotations/reconciliations; #230's swing-state cap is a designer-confirm-
before-scheduling gate WITHIN E20, not a new bucket entry). **★★ Batch 32 nets +2 to (A) User-gated: the two
vcczar-authored DESIGN surfaces each carry one explicitly-OPEN canonical-model question the build cannot infer
— #236's transition TRIGGER + win/loss/persistent-branch END-STATE (ties #88/#188), and #237's "no policy"
DEFAULT (flat "left to the states" vs computed-from-state-makeup)** (both added to (A) below; the rest of
batch 32 is buildable dataset/schema work or fold-ins). **New User-gated count: 6 (#52 + delegate-class +
#207 + #220 + #236 + #237).** **★★ Batch 33 nets −1 to (A) User-gated: #220's PV DISPLAY-SCALE pick is
RESOLVED → 0–100 / 50 = average pol (`groupthinkpv` POST 39, 43)**, so it leaves the bucket (folds into the
now-unblocked PV epic; the office-magnitude ↔ 0–100 scale reconciliation that remains is a human-call INSIDE
that epic, not a canonical-model fork). The batch's other deltas are buildable engine/data work or fold-ins
(the PV epic is un-gated; #221/#242/#241/the House model/DH-78 fold into existing areas). **New User-gated
count: 5 (#52 + delegate-class + #207 + #236 + #237).**
...
- **★ #207 — TIMELINE-COMPLETION MODEL: year-cap vs open sandbox vs score-out (NEW
  ...
  decision a human must make** — three live models: (a) a **fixed year-cap** with a
  win/score screen (the cumulative era-bank scoreboard, #102/#68, decides the winner), (b) an
  **open sandbox** (play continues indefinitely past the last authored era — needs #209
  ...
  as one end-state branch + a win/score screen at **E6** (alongside the loss model). — #207
  (`welcome2future#post 20`; `types.ts:1635`, `phaseRunners.ts:2871`, `phases.ts:49-58` →
  end-state at **E6**; pairs the #102/#68 cumulative scoreboard).
- **★ #220 — PV DISPLAY-SCALE: raw ("pink") vs 0-100/0-200 vs 100-normalized (NEW batch 30,
  debt #85) — ★★ BATCH-33 RESOLVED → 0–100, 50 = average pol (`groupthinkpv` POST 39, 43);
  this is no longer a pending User-gated pick.** `groupthinkpv` settled the fork in the designer's
  voice (display = 0–100 with 50 = average pol), so #220 leaves Decision-gated (A) and folds into
  the now-unblocked PV-reweight epic (READY-AFTER-#216); the ONLY remaining human-call inside that
  epic is the office-magnitude ↔ 0–100 scale reconciliation, not the display-scale family choice.
  (Historical fork, kept for the record:) The presentation/normalization side of the PV overhaul, left UNDECIDED in BOTH
  ...
  at length and **leaned raw ("pink") for HUMAN display** (scaled views bunch pols too close —
  ...
  either thread.** So the build faces a live fork — (a) **raw-for-humans** (revampPV lean) vs (b)
  **100-normalized** (summer2021); normalized is acceptable ONLY if PV is hidden/AI-only. **A human
  ...
  `Math.max(0,…)` with a floor-offset so sub-zero pols keep ordering for the CPU) is NOT gated — it
  ...
  chosen. — #220 (`revampPV` §6 D6/D7 / POST 16, 18, 20, 57, 66-91; `summer2021` POST 200-205;
  `pv.ts:88` clamp / `pvCache` raw → display layer at **E20b**; the clamp-vs-offset half is QW23).
- **★ #236 — ALTERNATE-GOVERNMENT-FORM TRANSITION TRIGGER + END-STATE: how the country goes Fascist/
  Communist/Theocratic, and what WINNING/LOSING/PERSISTING in that regime means (NEW batch 32, debt #97).**
  The DESIGNED government-form axis (vcczar-authored, `tomorrowlist`) leaves the two load-bearing rulings OPEN:
  **(a) the transition TRIGGER** — what set of conditions (meter thresholds / scripted event / amendment / a
  player action) flips `game.governmentForm` from `'democracy'` to a non-democratic form — and **(b) the
  END-STATE** — whether a regime is a **win condition**, a **loss condition**, or a **persistent branch** the
  game continues in (with its own menu + pool). This **ties the #88/#188 end-condition work** — the only
  game-overs in the corpus are mid-run meter/scripted LOSSES, so there is no precedent for a regime end-state.
  **A human must pick** both the trigger model and the end-state semantics; the build cannot infer either. The
  rest of #236 is buildable engine work GATED on the #221 content-primitive registry + the #239 religion field
  (Theocracy) + §24 amendments (the empower-executive primitive) — it folds into the era-content/#221 area once
  the rulings land. DISTINCT from #6/#354 movement-factions (those are lobbies, not a government-form branch).
  — #236 (`tomorrowlist`; debt #97; ties #88/#188 → folds into the era-content/#221 area; game-mechanics
  §30.22.A, §24.x).
- **★ #237 — POLICY-GENRE "NO POLICY" DEFAULT: flat "left to the states" placeholder vs computed-from-state-
  makeup (NEW batch 32, debt #98).** The DESIGNED stateful policy-genre framework (vcczar-authored,
  `businesslabor`) leaves ONE canonical-model question OPEN (POST 12): when a genre has no active policy, is the
  `*-Default` baseline a **flat "left to the states" placeholder** (a single neutral state) or **computed from
  each state's political makeup** (so absence-of-policy resolves differently per state)? This shapes both the
  schema (one default flag vs a per-state derivation) and how the `economic` meter reads the baseline. **A human
  must pick.** The rest of #237 is buildable engine work GATED on the #221 content-primitive registry (the
  `L/P/G/S` entries are content primitives) + the SHIPPED EconStab economic engine — it folds into the
  era-content/policy area once the default ruling lands. DISTINCT from interest-cards/lobbies (those nudge state
  industries; this is a national + per-state policy-FLAG axis). — #237 (`businesslabor` POST 12; debt #98 →
  folds into the era-content/policy area; game-mechanics §30.22.B, §29.7).
...
- **★ #234 Fog-of-War hidden/scoutable stats + #235 D6 pre-nomination VETTING roll —
  PARKING-LOT / "AMPU 2" (NEW batch 31; both designer-DEFERRED, do NOT schedule).** Two
  appointment-phase options the `apptdeepdive` deep-dive surfaced and the designers
  explicitly punted: **#234** — a toggleable game OPTION where a nominee's Admin (and in
  a full version hidden internal traits) displays as a RANGE/blurb, not a number —
  scoutable, with the President able to VET to reveal; both Ted AND vcczar liked it but
  tagged it "more programming realm than rules" → AMPU-2 / post-release (`apptdeepdive§H
  POST 94-98`). Same "fog of war" family as #184 (hide the election formula) + #220 (the
  PV display layer). **#235** — a D6 pre-nomination vetting roll (1 = career-ending retire
  / 2 = Controversial / 3-5 = nothing / 6 = Integrity) on a nominee who accepts vetting;
  repeatedly proposed and repeatedly DEFERRED — Ted judged it too slow ("30 hearings…")
...
§9.6 batch-30 + §8 debt #85-#90 + §9 batch-29 lead +
§9.6 batch-29 + §8 debt #78-#84 + §9 batch-28 lead +
...
**★★★★ Batch-33 lead — FIVE DESIGN/DISCUSSION/BUILD threads, NO playtest (the RECONCILE/UNBLOCK batch that
DELIVERED the long-flagged gating threads: `groupthinkpv` [the trait-EFFECTS / office-PV / display-scale GATE —
the sibling of `revampPV`; UNBLOCKS #214/#215/#220, re-scopes DH-77] + `legisexecgov` [the #221 3-PRIMITIVE
content-authoring source] + `ampuelections` [the elections content doc — ORIGIN of #241 variable term-length +
#242 amendable articles] + `housepoll` [the focus-reps-vs-every-seat poll — RESOLVES #219, AUTHORS #191] +
`excelautomate` [the spreadsheet GM-automation progress log — charter corroboration + ORIGIN of DH-78]); 3 NEW
gap rows #241/#242 + DH-78 (max gap #242, max bug DH-78) + a RECONCILE-HEAVY harvest. The two headlines are a
TINY high-leverage REMOVAL and an EPIC becoming buildable; it does NOT move the top-of-queue. Reflect it by:
(1) ★★ ADDING QW24 — REMOVE `pv*0.1` FROM ELECTIONS (debt #85; XS quick-win; `GM⇒App`): the designer + room
RULED PV must NOT feed elections (double-count, `groupthinkpv` POST 36-38), so DELETE `+ pv * 0.1` at
`phaseRunners.ts:3709` — this SUPERSEDES the batch-30/31 "re-tune `pv*0.1`" note; it pairs with QW19 at the SAME
line, and the 2.9.1 primary scorer's raw `pvCache` (`:3733`) is the endorsed AI-utility use and is KEPT; (2) ★★
MOVING THE PV-REWEIGHT EPIC gated → READY-AFTER-#216 (debt #85): `groupthinkpv` IS the previously-un-ingested
trait-EFFECTS thread → the gate is LIFTED; the build = #214 tiers (M-L) + #215 curve (M; Command×15 ·
Legis/Gov/Admin×10 · Mil/Jud×5 per level + expertise +5) + #220 (RESOLVED to 0–100/50-avg) + remove `pv*0.1` +
the 0%-BUILT trait-EFFECTS-in-PV layer (M-L); #216 remap (QW22) FIRST; the only open human-call = office-magnitude
↔ 0–100 scale; (3) ★★ ADDING #221 — THE 3-PRIMITIVE CONTENT-MODEL REGISTRY (L, downstream of K4; `GM⇒App`):
`legisexecgov` delivers the authoring source (Legis-Prop / Pres-Action / Gov-Action + scripted events, era-gated,
importance-curated); VERIFIED 0% shipped; #236/#237 depend on it; stays downstream of K4; (4) ★ ADDING #242
AMENDABLE `ConstitutionalArticles` (M, gated #221) + #241 VARIABLE TERM-LENGTH (M, folds into K3's `year%4`
predicate-replacement, ties #92, cost shared with K3) + DH-78 two-trait AND-combo field (XS-S, gated #216);
(5) ★ THE HOUSE MODEL — #219 RESOLVED (keep focus-reps, every-435 → AMPU-2 parking; EV-derived 1-10 ladder +
vote-power-with-remainder) + #191 FULLY AUTHORED (per-seat deviant-bias engine), S-M, folds into #34/#55/#73/#74;
(6) ★ SHARPENING #20 (Gov Actions FLAT/state-agnostic) + reflecting the DH-77 re-scope (worst trait = Incompetent
−20, not the `lackey −53` typo); (7) ★★ UPDATING THE GM-REPLACEMENT LENS (a VIEW, no re-sequence) — ① referee
gains PV-as-AI-utility (PV is the ② CPU's draft/persuasion metric, NOT an election input — the `pv*0.1` removal),
the #221 content-primitive registry, and the House model; the `excelautomate` ">30 min/phase ⇒ automate" charter
budget is the quantified ① mandate; (8) ★ CORPUS-COVERAGE — "Groupthinking the Political Value" is now INGESTED
(removed from the still-needed list; remaining = 1840-start + matthewyoung123-1772). NO new keystone, NO
re-sequence, NO new author-before-build RULES item (the PV epic UN-GATES OUT of that bucket), Decision-gated
RECOUNT −1 (#220 RESOLVED → 0–100/50-avg). **The order itself is UNCHANGED — the TOP OF THE QUEUE is QW0 → K0/K2
→ K3/K4 + `scenarioBoot`/`BootSheet` → `scenario1788` (E1); the ONE delta to the cheap-fixes lane is QW24
(`pv*0.1`-removal), and the PV epic moves gated → ready-after-#216. Within-batch order: remove `pv*0.1` + #216
remap (cheap-fixes, now) → the PV epic → #221 → #242 + #241 (inside K3) → the House model → DH-78.**
game-mechanics §3.4.1, §15.1, §15.4(c), §29.5, §2.3, §14.2, §14.1.3, §18.1, §30.23 + technical-guide §9 batch-33
lead + §9.6 + §8 debt #101-#103 + updated #85/#87/#73. debt #85/#87/#97-#98/#101-#103.**

**★★★★ Batch-32 lead — FIVE MIXED DATA/DESIGN/CHANGELOG threads, NO playtest (1 founding solo + 4
data/design/changelog: `tomorrowlist` [vcczar's to-do + the alternate-government-form ORIGIN] + `businesslabor`
[vcczar AUTHORS the policy-genre framework] + `histpres` [vcczar's 47-president statlines + the curated-pol
SCHEMA] + `georgemartha` [a 1772 SOLO — the gender model] + `calsleaderboard` [a ranking-record companion to
`summer2021`]); 5 NEW gap rows #236–#240 (max gap #240) + NO new design-hole bug (max bug DH-77). Mostly NEW
designed-but-unbuilt SURFACES + one DATASET-fill action — almost nothing is shipped; three of four design threads
are vcczar-authored. It does NOT move the top-of-queue. Reflect it by: (1) ★★ ADDING #240 — THE `CURATED_ROWS`
DATASET FILL (debt #100, the most ACTIONABLE item, a STANDALONE author/DATA task OFF the engine track, `GM⇒App`):
**★ a CORRECTED FACT — `CURATED_ROWS` is NOT empty** (the prior-role "empty array" claim is WRONG and corrected):
`seedDataset.mjs:275` `= []` is only the INITIALIZER, the loop `:276-289` builds it to **143 entries at runtime**
from the `ROWS` source array (`:38-233`) (+23 `ERA_FIGURES`), runtime-verified; `ROWS` holds the 1772+1856 marquee
roster NOT the 47 presidents → **#240 = "ADD ~47 president statlines to the `ROWS` source array"** (folded into
`CURATED_ROWS` as same-named OVERRIDES that beat the `congress-legislators` real-served rows per the `CLAUDE.md`
merge precedence), then regenerate (`bash scripts/fetchLegislators.sh && node scripts/legislatorsToDataset.mjs &&
npm run build`) — NOT "fill an empty array"; **GATED on the #216 trait-name remap (= QW22, already queued)** (14/50
`histpres` trait names mismatch the shipped `Trait` union — `Charisma`→`Charismatic` + 13 with no analog), the
Misc/Demographics/Dynasty lines additionally wait on the #99 data-model cluster; HUMAN-CALL recorded in
Author-before-build (trust the 2021-vintage raw skills vs re-derive under the #214/#215 curve); folds alongside the
#120 dataset umbrella (E18d); S-M; (2) ★★ ADDING #236 — THE ALTERNATE-GOVERNMENT-FORM SCENARIO AXIS (debt #97,
`GM⇒App`, the REFEREE half; L): Fascist/Communist/Theocratic USA the country transitions INTO + a reusable
authoritarian kit + Theocracy ×5 by denomination, via a `game.governmentForm` flag; VERIFIED wholly unbuilt;
**DEPENDS on the unbuilt #221 per-administration content-primitive registry** (`grep -iE 'presAction|legisProp|
govAction|scriptedEvent' src/` = ZERO) **+ the #239 religion field (Theocracy) + §24 amendments**; the transition
TRIGGER + win/loss/branch END-STATE are a HUMAN ruling (ties #88/#188 — added to (A) User-gated); DISTINCT from
#6/#354 movement-factions; folds into the era-content/#221 area, NOT a new keystone; (3) ★★ ADDING #237 — THE
STATEFUL POLICY-GENRE FRAMEWORK (debt #98, `GM⇒App`, the REFEREE half; M-L): discrete toggleable prerequisite-
gated policies in named GENRES (Business/Labor is ONE; Currency + Copyright siblings) with `*-Default` baselines +
`L/P/G/S` prefixes + era-band tags + `Preq:` chains that read/feed the SHIPPED `economic` meter; VERIFIED no
policy-flag store; **DEPENDS on #221** (the `L/P/G/S` entries are content primitives) + couples to the SHIPPED
EconStab economic engine; the "no policy" default is a HUMAN ruling (added to (A) User-gated); DISTINCT from
interest-cards/lobbies; folds into the era-content/policy area alongside #236; (4) ★ ADDING the #238/#239/#194
CURATED DATA-MODEL CLUSTER (debt #99, `GM⇒App`; M): `Politician.gender` + suffrage gate + spousal Key-Advisor
(#238); the curated-override flags `southernUnionist`/`canBeIndependent`/`partyFlipBudget`/`canMoveStates` + the
race×religion×sex demographics triple (#239); the dynasty/family graph wired from `histpres`'s turnkey ~37-dynasty
seed table (#194); VERIFIED absent on both `Politician`/`ImportedDraftee`; **build the relationship-edge + the
demographics tuple TOGETHER — the religion field gates #236-Theocracy**; the SCHEMA HALF of #240; needs a
`DB_VERSION` bump; folds into the dataset/roster work; (5) ★ ANNOTATING the SHARPENINGS (no re-doc): #194 gains a
turnkey dynasty seed table; #216/#214/#215 — `calsleaderboard` CONFIRMS office-weighted PV is RIGHT but raw command
is OVER-CREDITED early (the decreasing-Command-returns target of the #215 curve at `pv.ts:74`; skill-cap-3 per
#204) → folds into the debt #85 PV-reweight epic; #206/#221 Era-of-Future NON-tech content from 2 threads
(reconfirms Future DOUBLY UNBUILT); #115/#24 curated "what-if" draft-pool (women + Toussaint, ties #238) → #115;
(6) ★ UPDATING THE GM-REPLACEMENT LENS (a VIEW, no re-sequence) — ① referee gains the government-form axis (#236)
+ the policy-genre framework (#237) as referee-content systems, and the data-model cluster (#238/#239) + dataset
(#240) are flagged FOUNDATIONAL (the #239 religion field gates #236-Theocracy; the #99 schema is the half #240's
statline lines wait on); (7) RECORDING the within-batch order + the human-calls (the #240 raw-skills-vs-re-derive
DATA-authoring call → Author-before-build; the #236 trigger/end-state + #237 default DESIGN calls → (A)
User-gated). NO new keystone, NO re-sequence, NO new author-before-build RULES item (one DATA-authoring human-call
recorded), Decision-gated RECOUNT +2 (#236 + #237 model picks). **The TOP OF THE QUEUE is UNCHANGED: QW0 → K0/K2 →
K3/K4 + `scenarioBoot`/`BootSheet` → `scenario1788` (E1) — #240 is a STANDALONE author/DATA task runnable as soon
as the #216 remap lands (it does not touch the engine track), and #236/#237 fold into the era-content/#221 area
(neither lands before #221, #236-Theocracy also needs #239 religion).** game-mechanics §30.22.A, §30.22.B,
§30.22.C, §30.18.E, §29.7, §24.x, §3.1, §3.4.1 + technical-guide §9 batch-32 lead + §9.6 + §8 debt #97-#100. debt
#97-#100.**
**The order itself is UNCHANGED — the TOP OF THE QUEUE is QW0 → K0/K2 → K3/K4 +
`scenarioBoot`/`BootSheet` → `scenario1788` (E1).**

**★★★★ Batch-31 lead — FIVE RULES/FEEDBACK/TEST/DATA threads, NO playtest (`rulebook` [the CANONICAL
RULEBOOK, ~24 systems, designer @vcczar — the single most authoritative mechanics spec in the corpus] +
`apptdeepdive` [the Sept-2022 DESIGN-ORIGIN of the appointment/confirmation ruleset, @MrPotatoTed-led] +
`primary2020` [a May-2022 balance/edge-case STRESS-TEST] + `playtestdraft` [a founding-era DRAFT-PROCESS
record] + `bestmoments` [a HIGHLIGHTS/design-intent digest]); 8 NEW gap rows #228–#235 (max gap #235) + NO new
design-hole bug (max bug DH-77). A RULES/FEEDBACK/TEST batch — NO playtest exercise; RECONCILE-HEAVY (#199
SUPERSEDED, #183 RESOLVED, #182 CONFIRMED, #172/DH-76 SHARPENED, #50/#51 + #179/#134 canonical algos pinned,
POST-143 designed→built CLOSURE). It does NOT move the top-of-queue. Reflect it by: (1) ★★★ GROWING THE
EXISTING CABINET/APPOINTMENTS EPIC (debt #50, E16+E14c) S-M → M-L with the FULLY-SPECCED + WHOLLY-UNBUILT
CONFIRMATION SUBSYSTEM (debt #91; the batch headline; `GM⇒App`, the REFEREE half) — `runPhase_2_3_1_Cabinet`
(`phaseRunners.ts:2158`) AND SCOTUS (`:3667`) auto-confirm with NO vote/gate/block/filibuster/cloture/
nuclear-option/Top-Four/promise (`grep -iE 'nuclearOption|cloture|filibuster|topFour' src/` = ZERO); the
game-master pinned the canonical 12-rule §9.3.11 flow from the two PRIMARY sources `rulebook`+`apptdeepdive`
(auto-confirm gate Admin≥3∧¬Controversial∧¬IronFist-SML∧¬Top-Four → hearing at the #172 era-keyed threshold →
SML party-wide block [5 reasons] → Puritan filibuster + Manipulation-cloture [the DH-76 half] → president's
tools [compel / convert N=Legislative senators] → vote [Admin-1 50/50-flip = the DH-23 root cause; trait→vote
75%] → **#199 33/33/34 fallout, SUPERSEDING the Controversial→Incompetent cascade** → SML 5-name recovery →
office-vacancy cascades → #181 promise) — the two "Top-Four" sets DIFFER (always-vote = State/Defense/Treasury/
War; cross-party-cap = State/War-Def/AG/Treasury); **do NOT create a duplicate epic — GROW E16; GATED on K2
(the SML enact/repeal + president's-tools rows are ActionRegistry entries) + K5 (CPU votes/block/acceptance are
CpuController handlers)**; (2) ANNOTATING K3/K4/#92 with the **15-ERA TRIGGER-YEAR TABLE** (`rulebook#§A POST 2`
— the authoritative band-label DATA the content-band registry consumes; RECONFIRMS the #92 two-level
content-band model over a growing `Era` enum; no re-size); (3) ANNOTATING K2 with **#182 CONFIRMED**
(`rulebook#POST 30` — Command = the per-half-term executive-action budget; was a single-thread inference) →
the exec-action library is one of K2's 6 libraries (replace the flat `chance(0.5)` `phaseRunners.ts:3635` with a
command-sized loop; S budget / M lifecycle; `GM⇒App`); (4) ANNOTATING the lingering/card-assignment epics with
the **#179/#134** (canonical multi-roll lingering pit + decay numbers, `rulebook#POST 20`; M into E6/E16) +
**#50/#51** (canonical plurality + ban-table + 5-pol-floor + cascade card-assignment, `rulebook#POST 10`; M into
E23/E16) algos (debt #93, `GM⇒App`); (5) ADDING the NEW election deltas at E20 (debt #94): **#228** third-party
VOTE-DRAIN (wholly unbuilt; M; `GM⇒App`) + **#230** swing-state CAP **paired with QW19** at the shared
`calcStateVote` `:3711` jitter (seed via `rng.ts`, then gate behind the swing-state set; S-M; designer-confirm-
before-scheduling) + **#231** modern-Moderate over-performance = a balance regression case folded into #18/#51
(not a row); (6) FOLDING the appointment-side + founding deltas (debt #95): **#229** CPU appointee-acceptance
matrix + humans-freely-accept-except-VP (the OPPONENT half; M → E9 9d / E16; distinct from #73 picker + #91
vote) + **#199** 33/33/34 re-spec INSIDE #91 (XS; re-spec before #112/#73) + **#233** founding CC chair-
nominate→delegates-vote→CC-Pres-tiebreak (S → E1); (7) FOLDING the draft/small fold-ins (debt #96): **#232**
cold-start 2-ideology draft restriction → scenario-boot (#115/K4, S) + **#211** baseline = FIXED within-party
round-robin (re-confirmed; no re-size) + **#222** committee 3→11 CORROBORATED (no re-size) + **#234/#235
PARKING** (Fog-of-War hidden stats + D6 vetting — AMPU-2, do NOT schedule); (8) ★★ UPDATING THE GM-REPLACEMENT
LENS (a VIEW, no re-sequence) — ① referee gains the **confirmation subsystem** (#91/#172/DH-76) + the
**spoiler/swing-state** items (#228/#230/#231); ② opponent gains the **CPU acceptance matrix (#229)** + the
**CC-appointment opponent half (#233)**; (9) RECORDING the RECONCILIATIONS (#183 specced-but-unbuilt — reconcile
magnitude; #182 confirmed; POST-143 designed→built closure) + the **CORPUS-COVERAGE** note (1840-start
persistent-Whig run reconfirmed highest-value un-ingested + the un-ingested "Groupthinking the Political Value"
trait-EFFECTS thread that gates #214/#215 + matthewyoung123-1772; ask the human for exports). NO new keystone,
NO re-sequence, NO new author-before-build RULES item, Decision-gated UNCHANGED. **The TOP OF THE QUEUE is
UNCHANGED: QW0 → K0/K2 → K3/K4 + `scenarioBoot`/`BootSheet` → `scenario1788` (E1) — the headline confirmation
subsystem GROWS the existing cabinet/appointments epic (debt #50, S-M → M-L) and is GATED on K2+K5; it is the
referee-half spec finally complete, NOT a new top-of-queue item.** game-mechanics §9.3.11, §27.1.2, §11.2.1,
§22.4.1, §30.21 + technical-guide §9 batch-31 lead + §9.6 + §8 debt #91-#96. debt #91-#96.**
**★★★★ Batch-30 lead — FIVE DESIGN/DATA/changelog threads, NO playtest (`revampPV` [the Feb-2023 PV-SYSTEM
OVERHAUL] + `summer2021`-changelog [the batch-26 run's tier-1 ALL-DECIDED rule-changes, the ORIGIN of DH-76]
+ `ampuData` [the per-administration 1774→2022 content INDEX, an authoring source] + `presidents` [the
cross-playtest president RECORD + corpus-coverage map] + `ampuUpdates` [the earliest `GM⇒App` charter]); 14
NEW gap rows #214–#227 + 1 NEW design-hole bug DH-77 (max gap #227, max bug DH-77). A DESIGN/DATA batch — NO
playtest exercise. It does NOT move the top-of-queue. Reflect it by: (1) ★★★ THE PV-SYSTEM OVERHAUL SPLITS
(debt #85) — `pv.ts`'s `computePV` ships a PRE-REVAMP formula (flat ±4/−5 traits `pv.ts:76-77`, flat
`command×10` `:74`, flat `skill×weight×4` `:70-73`, magic `Kingmaker +6` `:79`, `Math.max(0,…)` clamp `:88`):
add **QW22 #216 the trait-name remap** to the cheap-fixes lane (**S, BLOCKING PREREQUISITE** — ~17 of ~57
forum trait line-items mis-key/drop vs the shipped `Trait` union `types.ts:62-117`; the table is tiers to
RE-MAP, not literal keys; DO FIRST) + **QW23 DH-77 the floor-offset clamp fix** (XS — swap `Math.max(0,…)`
...
VALUES) **AND with a MANDATORY same-PR `calcStateVote` `pv*0.1` re-tune** (`phaseRunners.ts:3699,3709` — `pv=
...
#86, `GM⇒App`; post-legislation success→Blunder pass + expertise-gated genre traits + Delegator/Micromanager +
Crisis-{Manager,Admin,Gov} + Passive/Predictable + Passive+Pliable PUPPET + Magician) → **an L SYSTEM LAYER
riding #179's roll engine `:3260` + #182's action budget `:3632` + §14.1.y/#126, GATED on federalism; folds
into E16/E6** (de-dup vs Efficient/Egghead first); (3) ANNOTATING **K4 with #221 — the era-content registry
needs TWO SCHEMA FIELDS** (debt #88; a scripted-vs-flavor tier flag + per-category first-active-era metadata;
`EraEvent` `types.ts:1466-1482` lacks both → folds into K4; XS-S each; the authoring-side confirmation of the
#92 two-level content-band model, content terminates at Biden/2022); (4) ADDING **#218 Rule-of-Four/CJ
case-refusal (S → E25, shares #217's trait reads) + #219 US-Rep PROPORTIONAL representation (S → E14/E7)**
(debt #87, both ABSENT); (5) ADDING the **SMALL FOLD-INS #222–#227** (debt #89, several `GM⇒App`): #222
committee-size scales w/ state count (S → E7/E14) · #223 chair→25%-enthusiasm (XS → E7/E16) · #224
lowest-satisfaction 25% AUTO-FLIP, DISTINCT from conversion #76 (S → E9/#108) · #225 Celebrity-expiry-before-40
(XS → E18/#136) · #226 Frail/Hale lifespan auto-traits = a NEW `scripts/` DATASET rule, NOT engine (XS →
...
§9.1.9; debt #90, S-M); (7) ★★ UPDATING THE GM-REPLACEMENT LENS (a VIEW, no re-sequence) — adding to ①
referee/calculator the **PV-electability** row (#214/#215/#216/DH-77 — the "who's electable" math the CPU's
...
② opponent-half entry this batch; (8) ★ RECORDING in the Author-before-build / corpus-coverage area the
...
"absence bonus" idea) and the **CORPUS-COVERAGE** note (≥7 un-ingested playtests — esp. the **1840-start**
persistent-Whig ~100-yr run, no 1840 scenario analog ships — and the un-ingested **"Groupthinking the Political
...
TOP OF THE QUEUE is UNCHANGED: QW0 → K0/K2 → K3/K4 + `scenarioBoot`/`BootSheet` → `scenario1788` (E1) — the PV
...
§3.4.1, §20.10.1, §22.7.z, §27.1.1, §30.20 + technical-guide §9 batch-30 lead + §9.6 + §8 debt #85-#90. debt
#85-#90.**
**★★★★ Batch-29 lead — TWO TIMELINE-EDGE PLAYTESTS that bracket the band model end-to-end (`wilsons1916`
[the FIRST multiplayer 1916, a deliberate START-AT-ERA-OPENING experiment fixing the 1960 mid-cycle
failure #186] + `welcome2future` [the DOWNSTREAM-MOST ingest in the corpus — a hands-off all-CPU 2022→
Future run, 8-of-10 factions on CPU rules, the most complete modern CPU-turn sample; planned to 2072 but
never reached it, covering setup + exactly ONE turn]); 8 NEW gap rows #206–#213 + 3 NEW design-hole bugs
DH-74–DH-76. It does NOT move the top-of-queue. Reflect it by: (1) ★★★ ANNOTATING K3/K4/#92 with the
HEADLINE RESOLUTION — #206 confirms the `Era` enum is DOUBLY too coarse from BOTH timeline edges (the
1916 Era-of-Hollywood OPENING + the TERMINAL Future band), and the tech-lead RESOLVES the long-open "grow
the `Era` enum vs. adopt the #92 two-level data-model" question **IN FAVOR of the DATA-MODEL** — band
...
of-the-Future PROPOSAL-LIBRARY content-authoring epic — events reportedly done, legislation a stub, the
same "events-before-legislation" shape as the Federalism gap (added to the Author-before-build content
bucket; folds into E31); (3) ADDING #207 — NO terminal game-over / completion state (debt #79): the ONLY
terminal state is `gameEnded` at ONE event-driven LOSS site (`phaseRunners.ts:2871`), NO `endYear`/
`maxYear` → a **completion-vs-loss DECISION** is owed (year-cap / sandbox / score-out), DISTINCT from the
#88/#188 meter/scripted LOSS endings — added to (A) User-gated (decision-gated on which completion model),
builds at E6; small but architecturally load-bearing (it defines what "done" means); (4) ADDING #208 +
DH-76 — the CPU LEGISLATIVE/CONFIRMATION engine + cloture (debt #80, `GM⇒App`): `grep filibuster|cloture
...
adds the cloture gap from `wilsons1916#P349` → the Filibuster action + an era-keyed Cloture counter tied
to the Nuclear-Option state + CPU override + cabinet caps, all deterministic → **folds into E9 handlers 9b
(legislation) + 9d (cabinet), K5-DEPENDENT**; M; (5) ADDING DH-74 — a `billProposable` bill-proposal
validator (debt #81, `GM⇒App`): `BILL_TEMPLATES` carry NO precondition fields → `wilsons1916` House-voted
bills then found them INVALID mid-second-vote (a 2.6 HALT) → a predicate + filter at
`phaseRunners.ts:3444` that **SHARES the territory/era content-filter with #206/#92 + DH-60 — build the
predicate library ONCE, use it for events AND bills**; S-M; (6) ADDING #209 — boot-time retro-cohort bench
(debt #83, `GM⇒App`): a non-1772 boot has no synthetic bench → `welcome2future` Alabama had no eligible
Republican → a boot-time retro-gen of SUB-FLOOR career-track cohorts stat-dated to the prior ~5 cycles,
routed through `rng.ts` → **folds into the existing `scenarioBoot`/`BootSheet` pipeline (K4, §9.1.9)**; M;
the boot prerequisite for ANY deep-modern / Future-band start; (7) ADDING DH-75 — military-appointment
eligibility (debt #82, `GM⇒App`): the officer filter checks ONLY `military >= 3` → Pancho Villa
appointable as a US general → add citizenship + exp-type→branch gates; XS-S, standalone; (8) ADDING the
SMALLER MECHANIC ASKS (debt #84, ride existing epics, LOWER priority): #210 Ex-President super-Kingmaker
(S, the Kingmaker subsystem), #211 cross-party PV-snake draft MODE (S, LOW — MP setup flavor), #212 the
unbuilt 2.1.5 ideology-shift pass + the per-faction shift-budget decision (S-M, the era's biggest by-hand
load), #213 era-conditional faction-ideology toggle (sunsets 2026, CONSOLIDATES with #171; XS-S); (9) ★★★
UPDATING THE GM-REPLACEMENT LENS — this is the FIRST batch to add LEGISLATIVE-REFEREE items to the lens:
the #208/DH-76 cloture/filibuster + CPU vote/cabinet-cap engine and the DH-74 bill-validator slot under ①
...
condition rows; on the ② opponent half the new entry is a CORRECTION — **DH-14 is a CPU-AI artifact, NOT a
mechanical block** (the 19th Amendment PASSED with humans, House 374-61), so it folds into the era-keyed
bill-ideology table (① data) + the #208 CPU vote model (② opponent); the lens stays a VIEW, NOT a re-
sequence; (10) ★ REFLECTING the REFRAMES — DH-14 (above); #106/WWI optionality + #114/#110 corroborated
...
populated per-year BootSheet — the 1916 BootSheet was "bare," NO starting generals → DH-75 + #209). NO new
keystone, NO re-sequence, NO new author-before-build RULES item (the Future proposal library is a CONTENT
epic), Decision-gated RECOUNT +1 (#207's completion-model pick). The TOP OF THE QUEUE is UNCHANGED: QW0 →
K0/K2 → K3/K4 + `scenarioBoot`/`BootSheet` → `scenario1788` (E1) — but #206 is the strongest single
argument yet that the K3/K4 era keystone must adopt the #92 two-level content-band model (not a growing
enum), and #208/DH-76/DH-74 sharpen the legislative half of the GM referee into concrete, K5-dependent
engine work. game-mechanics §22.11, §26.4, §28.10, §12.1.1, §9.2.1, §8.5.4, §4.4, §30.19 + technical-guide
§9 batch-29 lead + §9.6 + §8 debt #78-#84. debt #78-#84.**
**The order itself is UNCHANGED — the TOP OF THE QUEUE is QW0 → K0/K2 → K3/K4 +
`scenarioBoot`/`BootSheet` → `scenario1788` (E1).**
...
**★★★★ Batch-27 lead — `redbutton` (THE CORPUS'S FIRST Cold-War / Civil-Rights source + the FIRST
MODERN-ERA scenario boot stress-tested AT SCALE: a ~10-human MP run that STARTED MID-CYCLE in the 1960
Elections Phase, ran 1960→1962-64, then "Temporarily Paused" and never resumed solo; alt-timeline GOP
...
gaps [#185/#183/#186/#187] + ONE premise-only loss [#188→#88]. It does NOT move the top-of-queue.
Reflect it by: (1) ★★ SPLITTING the headline — the DH-72 ELECTION-TUNING SUB-FIXES go into the QUICK-WIN
bucket (highest-leverage work in the batch, binding at `calcStateVote` `phaseRunners.ts:3709-3711`,
XS–S) as THREE rows alongside the queued QW3 ±3 cap [debt #1] + the #18 2-layer scorer: QW19 = narrow +
SEED the jitter (raw `Math.random` ±4 → a seeded d3-style roll via `src/rng.ts`, ALSO closing a
...
→ ~281-129 near-tie; (2) ADDING #184 — the +2/+1/−1 PLATFORM meter-cap is NOT a quick-win and NOT a
standalone patch (it clamps a term that DOESN'T EXIST yet — `grep platform|plank in src/engine` = ZERO,
the SAME unbuilt object as #181) → a BUILD-INTO CONSTRAINT on the convention/platform epic (Phase-1 #10
/ #181): build the scorer with the cap + hide-the-math from day one, never ship the uncapped forum
scorer; (3) ADDING #185 — the modern PRIMARY→BROKERED-CONVENTION machine (12 primary states, primaries-
in-GROUPS, the 5-tier Cat-1–5 delegate-weight HOST-RIGGING lever, kingmaker +1, 67%-first-ballot, two-
leaders-force-a-kingmaker-weighted-rule-change-vote, least-delegates-first interballot order, 100%-
delegate rule + WALKOUT, kingmaker-weighted VP vote) → FOLDS INTO Phase-1 #10 convention machinery, L,
the single biggest election subsystem (CONFIRMED unbuilt: `runPhase_2_9_1_Primaries` `:3725-3750` = flat
PV+command*5 SORT); (4) ADDING #183 — the ENDORSEMENT-MOMENTUM hole (withdrawal/endorsement is inert
unless the dropper holds delegates; add a momentum value +2 Minor/+3 Major OR a home-state/region
transfer OR an ideology-penalty waiver, INDEPENDENT of delegate count) → FOLDS INTO #185/Phase-1 #10, S,
DO NOT build standalone (couples to the DH-72 d6→d3 squeeze + DH-57); (5) ADDING #186 — the
DETERMINISTIC LATE/MID-CYCLE BOOT = a FIFTH `BootSheet` shape (a 1960 governance boot forced into an
Elections-Phase start a turn early → cascading manual patches: Senate 50-50 vs real 66-D, no Leadership-
gated Party Leader, on-election rolls skipped, party-fatigue dropped, gov terms-served unknowable,
delegate weights hand-assigned, DC EVs invalid pre-1961, NO Command runway → roster frozen) → RIDES K4's
...
`careerTrackBackfill` (DH-25) — no new keystone, it SHARPENS the bar; (6) ADDING #187 — PARTY-LEADER
FATIGUE (`party preference −1` after 6yr of one party leader → ~8-yr presidential rotation; its absence
at the mid-cycle start COMPOUNDED the #184 landslide) → S, pairs with #15, do the boot retro-apply WITH
#186 (CONFIRMED unbuilt: `Party` `types.ts:1306-1311` has NO tenure field; `leadershipStartYear` `:1302`
written, NEVER read for a penalty); (7) ADDING #188 — the "BIG RED BUTTON" NUCLEAR LOSS (the campaign's
whole PREMISE never fired; Cold-War content that DID fire is event/legislation FLAVOR on the generic
engine) → FOLDS INTO #88 (meter-floor game-end + APOCALYPSE clock, Phase-1 #6), XS-S, a configured
end-condition ROW not a MAD/arms-race engine (corroborates §28.2 negative-scope #106); (8) ★★ ESCALATING
IN PLACE — NO new scope, NO re-sequence — #88 (first MODERN end-condition datum; the corpus still shows
exactly ONE proven game-over, `terror2000`'s coup → confidence ↑ on the meter-floor model), #86/#115/
#173/E1 (the mid-cycle modern boot is now CONCRETE — the 5th `BootSheet` shape with explicit holes), #14
(the designer's platform/election nerf DIRECTION is now set), #114/DH-36 (re-confirmed as the GM-WORKLOAD
DEATH from the MODERN angle — paused, never resumed solo; E9/K5 carries this too). NO new keystone, NO
re-sequence, NO new author-before-build or Decision-gated items (batch 27 nets 0 — the DH-72 sub-fixes
...
epics). The TOP OF THE QUEUE is UNCHANGED: QW0 → K0/K2 → K3/K4 + `scenarioBoot`/`BootSheet` →
`scenario1788` (E1) — but the DH-72 tuning sub-fixes JOIN the QUICK-WIN bucket (QW19-QW21) and the modern
primary→convention cluster (#185/#183 + the #184 platform cap) is now the KB's richest forward-played
election spec inside Phase-1 #10. game-mechanics §15.3.5, §8.5.3, §28.1, §28.2, §28.6, §30.17 +
technical-guide §9 batch-27 lead + §9.6. debt #64-#70.**
**The order itself is UNCHANGED — the TOP OF THE QUEUE is QW0 → K0/K2 → K3/K4 +
`scenarioBoot`/`BootSheet` → `scenario1788` (E1).**

**★★★★★ Batch-26 lead — `summer2021` (THE CORPUS'S LARGEST SOURCE + the FIRST run to PLAY FEDERALISM
FORWARD [1772→~1810, the whole president/cabinet/diplomacy TURN LOOP run end-to-end, NOT just
...
by: (1) ADDING #179 — the cabinet LINGERING-ROLL meter engine, a REDESIGN of the shipped deterministic
drift (`runPhase_2_5_1_Lingering` `phaseRunners.ts:3260` = `drift(skill)`-keyed, NO roll/backfire) into
a roll engine + ≥6 sub-rules across ≥2 systems (per-portfolio ±roll w/ backfire + Efficient auto-+2;
Pliable-Pres decider override; cabinet-bill double-points; ideology-gated confirmation; regional-rep
penalty; exec-action blunder rolls) → folds into E6 (Lingering) + E16 (cabinet) + §10.4.1 blunder
layer, L, ★ SPLIT into (i)+(f) meter/blunder engine · (b)+(d) appointment/confirmation · (c)
bill-coupling · (e) election-feedback (different deps); GATED on #177 (federalism era existing) + the
diplomacy/EconStab meters it moves, couples to #67 + #116/#160 — Open Q (designer): how to split;
(2) ADDING #182 — `command` = the President's PER-TURN EXECUTIVE-ACTION BUDGET ("Osgood can do up to 4
actions… he has 4 command"; each action implemented by a cabinet member with a blunder roll) → folds
into the §14 executive-action build (K2 ActionRegistry) + #179, S once the exec system is real; GATED
on `runPhase_2_8_1_Executive` (`:3632`, today ONE flat `chance(0.5)` action) becoming a real K2
ActionRegistry chooser — Open Q (designer): applies to governors too?; (3) ADDING #181 — the
...
kept/broken → §22.2 enthusiasm) → PART OF the #19/#20 convention cluster, M, DO NOT build standalone —
it rides the convention epic, gated on conventions existing; (4) ADDING #180 — the monument/legacy-trait
achievement layer (named historical-legacy traits to whoever performs each in-game FIRST) → rides the
federalism/founding era-content build, S–M, LOWER PRIORITY than #177-#179, gated on #177, Fed-Papers
authoring rule needed first; (5) ADDING DH-71 — faction-handover DUPLICATE-politician = a single-
ownership CONSTRAINT on the unbuilt #1 faction-handover (the transfer fn must MOVE not COPY — mutate
the SINGLE `factionId`), XS, NOT a shipped regression (the handover feature is itself unbuilt), folds
into #1; (6) ★★ ESCALATING IN PLACE — NO new scope, NO re-sequence — #177/#178/#114: #177 is now
DESIGNED-AND-PLAYED-FORWARD (first source to run the federalism turn loop forward, §20.9) → the
...
from the MULTIPLAYER end (Ted's meta-math POST 2671: ~50 CPU govs × ~3 actions × ~20% only move a crisis
meter ~1-2 spaces → solo human is the design target; the 3-GM relay re-confirms DH-36 without a clean
burnout quit); raise confidence, do NOT move them. NO new keystone, NO re-sequence, NO new
author-before-build or Decision-gated items (batch 26 nets 0 — #179 is split-but-buildable in E6/E16;
#181 rides #19/#20; #180/DH-71 ride the era-content/handover builds; #182 is gated on K2; the
#179-split, #178 ≤3-fires, #182-governors questions are designer-gated WITHIN their own work). The TOP
OF THE QUEUE is UNCHANGED: QW0 → K0/K2 → K3/K4 + `scenarioBoot`/`BootSheet` → `scenario1788` (E1) — but
the federalism era-content cluster (#177/#178 + the four new #179/#180/#181/#182) is now the KB's
richest, most-forward-played next-era build target. game-mechanics §20.9-§20.15.1, §30.16 + technical-
guide §9 batch-26 lead + §9.6. debt #57/#58 + #59-#63.**
**The order itself is UNCHANGED — the TOP OF THE QUEUE is QW0 → K0/K2 → K3/K4 +
`scenarioBoot`/`BootSheet` → `scenario1788` (E1).**
